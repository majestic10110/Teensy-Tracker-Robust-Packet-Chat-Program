...
class ChatApp(QMainWindow):
    ...
    def _append_rx_ui_only(self, text: str):
        ...
        try:
            ...
            item = QListWidgetItem(self._ts_prefix() + str(text))
...
# ===== File Transfer (Upload → META/OK/PART/END with PING/PONG probes) =====
                ...
                if s:
                    ...
                    if s.startswith('[KISS]') or s.startswith('[APRS]') or is_monitor or aprs_like:
                        if hasattr(self, '_append_rx_ui_only'):
                            self._append_rx_ui_only(s if s.startswith('[') else '[MON] ' + s)
                        return
        except Exception:
            pass
    ...
    def _ui_serial_section(self):
        ...
        try:
            ...
            pass
        self.monitor_all_check = QCheckBox("Monitor All Traffic")

        try:
            self.monitor_all_check.toggled.connect(self._on_monitor_all_toggled)
        except Exception:
            pass
        row2.addWidget(self.monitor_all_check)
        row2.addStretch(1)
        v.addLayout(row2)
...
# (removed duplicate Monitor All checkbox creation)
    ...
    def load_settings(self):
        ...
        try:
            ...
            pass
        # Ensure Monitor All Traffic default is ON for diagnostics (non-destructive)
        try:
            if hasattr(self, 'monitor_all_check'):
                self.monitor_all_check.setChecked(True)
        except Exception:
            pass
