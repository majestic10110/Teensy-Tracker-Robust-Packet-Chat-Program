...
"""
Teensy Tracker Chat v1.2.8 ACK-RED — HOTFIX 4 (2025-09-12)
Fix: adds the missing _save_debug_log() used by GPS SEND preflight logging.
Also includes:
- Mandatory ACK IDs appended to outgoing messages
- Sent line turns GREEN ➜ RED on ACK reception
- Auto-ACK replies for inbound messages with [ACK:ID]
- Console filter for non-beacon dot-lines like ".X0"/".X1"
- Heartbeat list and crash guards
"""

import sys, re, time, os, math, tempfile, traceback
...
class ChatApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Teensy Tracker Robust Packet Chat v1.2.8 ACK-RED — HOTFIX 4")
    ...
    def _on_line(self, line: str):
        ...
        # -- First: treat non-beacon dot-lines (e.g., ".X0") as device lines
    ...
    def _rebuild_chat_view(self):
        ...
        for item in self.chat_items:
            ...
            text = item["text"]
            if kind == "received" and LINE_RE.match(text or ""):
                m = LINE_RE.match(text)
                to = (m.group('to') or '')
                frm = (m.group('frm') or '')
                msg = (m.group('msg') or '')
                esc = lambda s: (s or "").replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")
                frm_link = f'<a href="callsign:{esc(frm)}" style="color:{COLOR_RECV}; text-decoration:none; border:1px solid {COLOR_RECV}; padding:2px; border-radius:3px;">{esc(frm)}</a>'
                rendered = f"{esc(to)} DE {frm_link}" + (f" {esc(msg)}" if msg else "")
            else:
                rendered = (text or "").replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")
    ...
    def _mark_ack_received(self, ack_id: str, from_callsign: str = ""):
        ...
        if item and not item.get("ack"):
            ...
            self._rebuild_chat_view()
            if from_callsign:
                self.status_label.setText(f"ACK {ack_id} received from {from_callsign}.")
            else:
                self.status_label.setText(f"ACK {ack_id} received.")
        else:
...
def main():
    print("Teensy Tracker Chat v1.2.8 ACK-RED — HOTFIX 4 (2025-09-12)")
