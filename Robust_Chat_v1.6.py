...
from PyQt5.QtWidgets import (QScrollArea, QFrame, 
    ...
    QGraphicsScene, QCheckBox, QDockWidget
, QScrollArea, QFrame)

# --- TX-echo suppression helpers (F26a) ---
import re as _rc_re

_RC_MON_FRAME_RE = _rc_re.compile(
    r'^\s*fm\s+[A-Z0-9/+\-]+(?:-\d+)?\s+to\s+[A-Z0-9/+\-]+(?:-\d+)?\s+ctl\s+UI\^?(?:\s+pid\s+\w+)?\s*$',
    _rc_re.IGNORECASE
)

def _rc_is_self_call_in_line(self, s: str) -> bool:
    try:
        mc = getattr(self, "mycall", None) or getattr(self, "MYCALL", None)
        if not mc:
            return False
        return _rc_re.search(rf'\b{_rc_re.escape(str(mc))}\b', str(s), _rc_re.IGNORECASE) is not None
    except Exception:
        return False

def _rc_drop_console_echo(self, line: str) -> bool:
    '''Return True if the line looks like our own TX monitor echo.'''
    try:
        if _RC_MON_FRAME_RE.match(str(line)) and _rc_is_self_call_in_line(self, line):
            return True
    except Exception:
        pass
    return False
...
class FleetManager:
    def __init__(self, path):
        ...
        self.load()
        # Ensure KISS Mode default-checked if present
        try:
            for cand in dir(self):
                try:
                    w = getattr(self, cand)
                    if getattr(w, 'text', None) and callable(w.text):
                        t = w.text().strip()
                        if t.startswith('KISS Mode') and hasattr(w, 'setChecked'):
                            w.setChecked(True)
                except Exception:
                    pass
        except Exception:
...
DE_MSG_RE = re.compile(r"^\s*([A-Z0-9]{1,6}(?:-[0-9]{1,2})?)\s+DE\s+([A-Z0-9]{1,6}(?:-[0-9]{1,2})?)\s+(.+?)(?:\s+\[ACK[:\s]?([A-Z0-9]{1,10})\])?\s*$", re.I)
...
# === Robust Chat Secure Crypto Helpers (AES-256-GCM preferred) ===
import os, hmac, hashlib, base64
def _kdf_hkdf_sha256(passphrase: str, salt: bytes, length: int = 32) -> bytes:
    try:
        from cryptography.hazmat.primitives.kdf.hkdf import HKDF
        from cryptography.hazmat.primitives import hashes
        from cryptography.hazmat.backends import default_backend
        hkdf = HKDF(algorithm=hashes.SHA256(), length=length, salt=salt, info=b"RC-SecurePad", backend=default_backend())
        return hkdf.derive(passphrase.encode("utf-8", "ignore"))
    except Exception:
        out = b""; prev = b""; block = 1
        while len(out) < length:
            prev = hmac.new(salt, prev + passphrase.encode("utf-8","ignore") + b"RC-SecurePad" + bytes([block]), hashlib.sha256).digest()
            out += prev; block += 1
        return out[:length]

def _xor_stream(data: bytes, key: bytes) -> bytes:
    out = bytearray(len(data))
    for i,b in enumerate(data): out[i] = b ^ key[i % len(key)]
    return bytes(out)

def rc_encrypt(plaintext: str, passphrase: str, force_aes: bool=False) -> str:
    if not passphrase: return plaintext
    try:
        from cryptography.hazmat.primitives.ciphers.aead import AESGCM
        salt = os.urandom(16); key = _kdf_hkdf_sha256(passphrase, salt, 32)
        aesgcm = AESGCM(key); nonce = os.urandom(12)
        ct = aesgcm.encrypt(nonce, plaintext.encode("utf-8","ignore"), b"RCv2")
        blob = b"\x02" + salt + nonce + ct
        return "[ENC2]" + base64.b64encode(blob).decode("ascii")
    except Exception:
        if force_aes:
            raise
        salt = os.urandom(16); key = _kdf_hkdf_sha256(passphrase, salt, 32)
        x = _xor_stream(plaintext.encode("utf-8","ignore"), key)
        blob = b"\x01" + salt + x
        return "[ENC1]" + base64.b64encode(blob).decode("ascii")

def rc_decrypt(ciphertext: str, passphrase: str) -> str:
    if not (ciphertext.startswith("[ENC2]") or ciphertext.startswith("[ENC1]") or ciphertext.startswith("[ENC]")): return ciphertext
    if not passphrase: return ciphertext
    try:
        raw = base64.b64decode(ciphertext.split("]",1)[1].encode("ascii"))
    except Exception: return ciphertext
    try:
        if ciphertext.startswith("[ENC2]") and len(raw) > 1+16+12:
            salt = raw[1:17]; nonce = raw[17:29]; ct = raw[29:]
            from cryptography.hazmat.primitives.ciphers.aead import AESGCM
            key = _kdf_hkdf_sha256(passphrase, salt, 32)
            pt = AESGCM(key).decrypt(nonce, ct, b"RCv2")
            return pt.decode("utf-8","ignore")
        elif ciphertext.startswith("[ENC1]") and len(raw) > 1+16:
            salt = raw[1:17]; x = raw[17:]; key = _kdf_hkdf_sha256(passphrase, salt, 32)
            return _xor_stream(x, key).decode("utf-8","ignore")
        elif ciphertext.startswith("[ENC]"):
            key = hashlib.sha256(passphrase.encode("utf-8","ignore")).digest()
            x = base64.b64decode(ciphertext[5:].encode("ascii"))
            return _xor_stream(x, key).decode("utf-8","ignore")
    except Exception: return ciphertext
    return ciphertext
# === End Secure Crypto Helpers ===

# ======== Secure Pad Dialog (bright red) ========
from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QTextEdit, QPushButton, QMessageBox, QCheckBox
from PyQt5.QtCore import Qt

class SecurePadDialog(QDialog):
    def __init__(self, owner, initial_text: str = "", from_call: str = None, show_to: bool = True):
        super().__init__(owner)
        self.owner = owner
        self.setWindowTitle("Secure Pad")
        # Bright flat red background, white text
        self.setStyleSheet("QWidget{background-color:#B00020;color:white;} "
                           "QGroupBox{border:1px solid rgba(255,255,255,0.2); margin-top:6px;} "
                           "QLabel{color:white;} "
                           "QLineEdit, QTextEdit{background:white; color:black;} "
                           "QPushButton{background:white; color:black; border-radius:6px; padding:6px 10px;} "
                           "QPushButton:disabled{opacity:0.6;}")
        self.setMinimumSize(900, 620)
        v = QVBoxLayout(self)

        # Receive-mode banner and TO visibility
        if from_call:
            lab_from = QLabel(f"From: {from_call}")
            lab_from.setStyleSheet("QLabel{background:transparent;color:#fff;font-weight:bold;}")
            v.addWidget(lab_from)
        try:
            if not show_to:
        ...
        # Warning
        warn = QLabel("⚠️ Warning: Using Encryption may breach your license conditions.")
        try: warn.setWordWrap(True)
        except Exception: pass
        warn.setStyleSheet("QLabel{background:transparent;color:#ffef8a;font-weight:bold;}")
        v.addWidget(warn)
        ...
        try:
            t = (owner.to_edit.text() or "").strip().upper()
        except Exception:
            t = ""
        self.to_edit.setText(t); row.addWidget(self.to_edit)
        ...
        # Force AES-256 row (default OFF so we never block sends unexpectedly)
        row2 = QHBoxLayout()
        self.force_aes_chk = QCheckBox("Force AES-256 Encryption")
        self.force_aes_chk.setChecked(True)
        row2.addWidget(self.force_aes_chk); row2.addStretch(1)
        v.addLayout(row2)

        # Text area
        self.txt = QTextEdit()
        self.txt.setPlaceholderText("Type your message here. Use Encrypt/Decrypt as needed. Click SEND to transmit.")
        ...
        br.addStretch(1); br.addWidget(self.btn_enc); br.addWidget(self.btn_dec); br.addWidget(self.btn_send); br.addWidget(self.btn_close)
        v.addLayout(br)

        self.btn_close.clicked.connect(self.reject)
        self.btn_enc.clicked.connect(self._on_encrypt)
        self.btn_dec.clicked.connect(self._on_decrypt)
        self.btn_send.clicked.connect(self._on_send)

    def _on_encrypt(self):
        txt = self.txt.toPlainText(); pw = self.pass_edit.text()
        if not txt.strip(): return
        try:
            out = rc_encrypt(txt, pw, force_aes=self.force_aes_chk.isChecked())
        except Exception:
            QMessageBox.warning(self, "Secure Pad", "AES-256 required but not available. Install 'cryptography' or uncheck Force AES-256.")
            return
    ...
    def _on_decrypt(self):
        txt = self.txt.toPlainText(); pw = self.pass_edit.text()
        if not txt.strip(): return
        self.txt.setPlainText(rc_decrypt(txt, pw))

    def _on_send(self):
        to = (self.to_edit.text() or "").strip().upper()
        if not to:
            QMessageBox.information(self, "Secure Pad", "Please provide a Target/TO callsign.")
            return
        msg = (self.txt.toPlainText() or "").strip()
        if not msg: return
        # Enforce AES if requested
        if self.force_aes_chk.isChecked():
            try:
                from cryptography.hazmat.primitives.ciphers.aead import AESGCM as _AES_TEST
                _ = _AES_TEST
            except Exception:
                QMessageBox.warning(self, "Secure Pad", "AES-256 is required but not available. Install 'cryptography' or uncheck Force AES-256.")
                return
        ok = False
        try:
            ok = bool(self.owner._send_securepad(to, msg))
        except Exception:
            ok = False
        if ok:
            self.accept()
# ======== End Secure Pad Dialog ========
...
class ChatApp(QMainWindow):
    ...
    def _is_encrypted_line(self, s: str) -> bool:
        try:
            t = str(s or "")
        except Exception:
            t = ""
        return ("[ENC2]" in t) or ("[ENC1]" in t) or ("[ENC]" in t)
    def _mirror_positions_to_geojson_from_store(self):
        """
        G-style mirror (clean):
          - Reads base_dir/store/positions.json
          - Injects MYCALL from UI/settings if fixed coords available (settings-fixed) when not already present
          - Writes BOTH base_dir/store/maps/positions.geojson and position.geojson
          - GeoJSON uses [lon, lat], includes generated_at and version.
        """
        try:
            import os, json, datetime, time
            base = app_base_dir()
            store_dir = os.path.join(base, "store")
            maps_dir = os.path.join(store_dir, "maps")
            os.makedirs(maps_dir, exist_ok=True)

            src_path = os.path.join(store_dir, "positions.json")
            positions = {}
            ...
            # --- Inject MYCALL with fixed coords if available and not present ---
            try:
                # Prefer UI widgets
                mycall = ""
                if hasattr(self, 'mycall_edit') and self.mycall_edit is not None:
                    mycall = (self.mycall_edit.text() or "").strip().upper()
                # Fallback to settings dict
                if not mycall:
                    for attr in ('settings','_settings','config','_config'):
                        d = getattr(self, attr, None)
                        if isinstance(d, dict):
                            for key in ('MYCALL','mycall','callsign','my_call'):
                                if d.get(key):
                                    mycall = str(d[key]).strip().upper()
                                    break
                        if mycall:
                            break
                ...
                flat = flon = None
                # UI fixed lat/lon widgets if present
                if hasattr(self, 'fixed_lat_edit') and hasattr(self, 'fixed_lon_edit'):
                    flat = _try_float(getattr(self.fixed_lat_edit, 'text', lambda: None)())
                    flon = _try_float(getattr(self.fixed_lon_edit, 'text', lambda: None)())
                ...
                if flat is None or flon is None:
                    for attr in ('settings','_settings','config','_config'):
                        d = getattr(self, attr, None)
                        if isinstance(d, dict):
                            lat_key = next((k for k in ('my_lat','home_lat','fixed_lat','lat','latitude') if k in d), None)
                            lon_key = next((k for k in ('my_lon','home_lon','fixed_lon','lon','longitude') if k in d), None)
                            if lat_key and lon_key:
                                flat = _try_float(d.get(lat_key))
                                flon = _try_float(d.get(lon_key))
                                break

                if mycall and flat is not None and flon is not None and mycall not in positions:
                    positions[mycall] = {
                        'lat': float(flat),
                        'lon': float(flon),
                        'last_update': int(time.time()),
                        'source': 'settings-fixed',
                        'history': []
                    }
            except Exception:
                pass
            ...
            feats = []
            for call, ent in (positions.items() if isinstance(positions, dict) else []):
                try:
                    lat = float(ent.get("lat"))
                    lon = float(ent.get("lon"))
                except Exception:
                    continue
                last_heard = _to_iso(ent.get("last_heard") or ent.get("ts") or ent.get("last_update"))
                last_update = _to_iso(ent.get("last_update") or ent.get("ts") or ent.get("last_heard"))
                src = ent.get("src") or ent.get("source") or "beacon"
                hist = []
                try:
                    raw_hist = ent.get("history") or []
                    if isinstance(raw_hist, list):
                        for h in raw_hist[-50:]:
                            if not isinstance(h, dict): continue
                            try:
                                hlat = float(h.get("lat")); hlon = float(h.get("lon"))
                            except Exception:
                                continue
                            ht = h.get("t") or h.get("ts") or h.get("time")
                            hist.append({"t": ht, "lat": hlat, "lon": hlon})
                except Exception:
                    pass
            ...
            for outname in ("positions.geojson", "position.geojson"):
                tmp = os.path.join(maps_dir, outname + ".tmp")
                with open(tmp, "w", encoding="utf-8") as f:
                    json.dump(gj, f, ensure_ascii=False)
                os.replace(tmp, os.path.join(maps_dir, outname))
            return True
        except Exception:
            return False

    # --- Connect button theming helpers ---
    def _theme_name(self):
        '''Best-effort theme name from theme_combo or settings.'''
        try:
            if hasattr(self, 'theme_combo') and hasattr(self.theme_combo, 'currentText'):
                n = (self.theme_combo.currentText() or '').strip()
                if n:
                    return n
        except Exception:
            pass
        try:
            ...
            return st.get('theme', '') or ''
        except Exception:
            return ''
    
    def _find_connect_button(self):
        '''Try to locate the Connect button instance.'''
        # Common attribute names first
        for cand in ('connect_button', 'serial_connect_btn', 'btn_connect', 'connectBtn'):
            try:
                btn = getattr(self, cand, None)
                if btn:
                    return btn
            except Exception:
                pass
        # Fallback: scan by visible text
        try:
    ...
    def _style_connect_button(self, state='normal'):
        '''Apply themed styling to the Connect button; no text changes.'''
        try:
            btn = self._find_connect_button()
            if not btn:
                return
            theme = (self._theme_name() or '').lower()
            if state == 'connected' and 'dark green' in theme:
                # Invert for Dark Green: lime background, black text
                btn.setStyleSheet('background: lime; color: black;')
            else:
                # Revert to theme default
                btn.setStyleSheet('')  # clears inline style, letting theme/app-wide stylesheet take over
        except Exception:
            pass
    # --- end helpers ---

    # --- Minimal serial button handlers (no external dependencies) ---
    def _on_connect_click(self):
        try:
            # get selected port (do not depend on external getters)
            port = ""
            try:
                port = (self.serial_combo.currentText() or "").strip()
            except Exception:
                try:
                    port = (self.combobox_port.currentText() or "").strip()
                except Exception:
                    pass
            if not port:
                try:
                    ...
                    pass
                return
            baud = getattr(self, "_serial_baud_default", 38400)
            # open using existing method if available
            ok = False
            try:
                ok = self._open_serial(port, baud)
            except Exception:
                ok = False
            if ok:
                # start reader thread if defined
                try:
                    if not hasattr(self, "reader_thread") or self.reader_thread is None:
                ...
                try:
                    ...
                    pass
                try:
                    # optional button text feedback, but don't rely on specific names
                    from PyQt5.QtWidgets import QPushButton
                    for btn in self.findChildren(QPushButton):
                        try:
            ...
            # never crash from a click
            pass

    def _on_disconnect_click(self):
        try:
            # stop reader thread if present
            try:
                if hasattr(self, "reader_thread") and self.reader_thread:
                    try: self.reader_thread.stop()
                    except Exception: pass
                    try: self.reader_thread.wait(500)
                    except Exception: pass
                    self.reader_thread = None
            except Exception:
                pass
            # close serial using existing method if available
            try:
                self._close_serial()
            except Exception:
                pass
            ...
            try:
                ...
                for btn in self.findChildren(QPushButton):
                    try:
                        if btn.text().strip().lower() in ("connected...", "connected …"):
                            btn.setText("Connect")
                    except Exception:
                        pass
            except Exception:
            ...
            pass
    # --- end minimal serial handlers ---

    def _on_monitor_all_toggled(self, state: int):
        try:
            on = bool(state)
            # Persist monitor_all in settings
            save_json('settings.json', self.settings)
        except Exception:
            pass
        '''UI-only toggle; no TNC commands.'''
        try:
            on = bool(state)
            msg = "🟠 Monitor All: ON (showing raw RX; filters off)" if on else "🟢 Monitor All: OFF (applying filters: MYCALL/CQ/Fleet)"
            if hasattr(self, "_append_sys"):
                self._append_sys(msg)
            # Optional: refresh beacons UI if available
            try:
                if hasattr(self, "_refresh_beacons_ui"):
    ...
    def _append_rx_ui_only(self, text: str):
        '''Append a line to the Messages list without persisting to messages_v1.json.'''
        try:
            from PyQt5.QtWidgets import QListWidgetItem
            txt = str(text)
            # Strip any leading [MON] tag universally
            if txt.startswith('[MON] '):
                txt = txt[6:]
            elif txt.startswith('[MON]'):
                txt = txt[5:]
            item = QListWidgetItem(self._ts_prefix() + txt)

            # === Cyan highlight for messages addressed to MYCALL in "<MYCALL> DE <callsign> ..." format ===
            try:
                from PyQt5.QtGui import QBrush, QColor
                mycall = (self.mycall_edit.text() or "").strip().upper() if hasattr(self, "mycall_edit") else "MYCALL"
                # Match: "<MYCALL> DE <callsign> Message [ACKnnnn]" (ACK optional, case-insensitive)
                if re.match(rf'^\s*{re.escape(mycall)}\s+DE\s+[A-Z0-9]{1,6}(?:-[0-9]{1,2})?\s+.+?(?:\s+\[ACK[:\s]?[A-Z0-9]{4,10}\])?\s*$', txt, re.IGNORECASE):
                    item.setForeground(QBrush(QColor("cyan")))
            except Exception:
                pass
            # === end cyan highlight ===
            # Optional: color for monitor-only
            # from PyQt5.QtGui import QBrush, QColor
    ...
    def _ensure_color_attrs(self):
        ...
        defaults = {
            ...
            "COLOR_SENT": QColor("#088F8F"),      # bright green
    ...
    def _my_position(self):
        ...
        return (None, None)
        try:
            if hasattr(self, 'linkmap') and hasattr(self.linkmap, 'draw_graph'):
...
# ===== File Transfer (Upload → META/OK/PART/END with PING/PONG probes) =====
    ...
    def _save_positions(self):
        try:
            ...
            try:
                self._mirror_positions_to_geojson_from_store()
            except Exception:
                pass
            try:
                self._mirror_positions_to_geojson_from_store()
    ...
    def _load_serial_from_settings(self):

        s = self._settings_get()

        sd = s.get("serial") or {}

        self._serial_port_default = sd.get("port", "")

        try:

            self._serial_baud_default = int(sd.get("baud", 38400))

        except Exception:

            self._serial_baud_default = 38400

        try:

            if hasattr(self, "serial_combo") and self._serial_port_default:

                ix = self.serial_combo.findText(self._serial_port_default)

                if ix >= 0:

                    self.serial_combo.setCurrentIndex(ix)
    ...
    def _save_serial_to_settings(self, port: str, baud: int):

        s = self._settings_get()

        s["serial"] = {"port": str(port or ""), "baud": int(baud or 38400)}
    ...
    def _append_rx(self, line: str):
        # F26a: drop self-TX monitor echoes
        if _rc_drop_console_echo(self, line):
            return

        # Sanitize: remove any stray [MON] tags on RX lines
        if isinstance(line, str) and line.startswith('[MON]'):
            line = line.replace('[MON] ', '').replace('[MON]', '')
        ...
        try:
            ...
            my = ''
            try:
                my = (self.get_mycall() or '').strip().upper()
            except Exception:
                my = ''
            to_cs = ''
            try:
                m = DE_MSG_RE.match(_s)
                if m:
                    to_cs = (m.group(2) or '').strip().upper()
            except Exception:
                to_cs = ''
            if to_cs and my and to_cs != my:
                c = QColor('#088F8F')
            elif to_cs and my and to_cs == my:
                try:
                    c = QColor(self.COLOR_RX)
                except Exception:
                    c = QColor('#00FF00')
            else:
                c = QColor('#088F8F')
            try:
                ...
                it.setData(Qt.ForegroundRole, c)
            except Exception:
                pass
        except Exception:
            pass
        ...
        try:
            try:
                ...
                _enc_guard = (line if "line" in locals() else text if "text" in locals() else msg if "msg" in locals() else "")

                if self._is_encrypted_line(_enc_guard):
    ...
    def _on_serial_thread_line(self, line: str):
        ...
        # PRIORITY: <FROM> DE <TO> <MESSAGE> [ACK nnnn]
        try:
            _s = (line or '').strip()
            m = DE_MSG_RE.match(_s)
            if m:
                frm_cs = (m.group(1) or '').upper()
                to_cs  = (m.group(2) or '').upper()
                msg    = (m.group(3) or '').strip()
                ackid  = (m.group(4) or '').upper() if m.group(4) else None
                my = ''
                try:
                    my = (self.get_mycall() or '').upper()
                except Exception:
                    my = (getattr(self, 'mycall', '') or '').upper()
                # Ignore self-echo just in case
                if frm_cs and my and frm_cs == my:
                    return
                # If addressed to me or broadcast-like, accept immediately
                if not to_cs or (my and to_cs == my):
                    # Update ACK state if present
                    if ackid:
                        st = (getattr(self, '_ack_states', {}) or {}).get(ackid)
                        if st:
                            st.on_ack_received()
                    try:
                        self._append_rx(_s)
        ...
        # Monitor-only raw display (no persistence) when checkbox is ON
        try:
                s = (line or '').strip()
                if s:
                    is_monitor = s.lower().startswith('fm ') or ' pid ' in s.lower()
                    aprs_like = s[:1] in ('!', '/', '=', '@')
                    if s.startswith('[KISS]') or s.startswith('[APRS]') or is_monitor or aprs_like:
                        if hasattr(self, '_append_rx_ui_only'):
                            self._append_rx_ui_only(s)
                        return
        except Exception:
            pass


        # Always parse beacons so Beacons Heard updates regardless of monitor state
        try:
            if hasattr(self, "_scan_for_beacon_line"):
                self._scan_for_beacon_line(line)
        except Exception:
            pass

        # ACK detection FIRST (works in both monitor modes)
        try:
            s_ack = (line or "")
            m_ack = ACK_TAG_RE.search(s_ack)
            if m_ack:
                ack_id = (m_ack.group(1) or "").upper()
                st = getattr(self, "_ack_states", {}).get(ack_id) if hasattr(self, "_ack_states") else None
                if st:
                    st.on_ack_received()
                    try:
                        self._append_sys(f"✔ Ack {ack_id} received")
                    except Exception:
                        pass
                    return
        except Exception:
            pass
        # Monitor-only raw KISS/APRS display (no persistence; only when Monitor All is ON)
        try:
                s = (line or '').strip()
                if s:
                    # If the TNC is already outputting monitor text (e.g., 'fm SRC to DST ... pid F0')
                    is_monitor = s.lower().startswith('fm ') or ' pid ' in s.lower()
                    # Also catch APRS info lines that start with position/time symbols
                    aprs_like = s.startswith('!') or s.startswith('/') or s.startswith('=') or s.startswith('@')
                    if s.startswith('[KISS]') or s.startswith('[APRS]') or is_monitor or aprs_like:
                        if not s.startswith('[KISS]') and (is_monitor or aprs_like):
                            s = '[KISS] ' + s
                        self._append_rx_ui_only(s)
                        return  # don't let it fall into normal chat/ACK logic
        except Exception:
            pass
        # MONITOR-ALL BYPASS: show any non-empty line immediately
        try:
                if (line or '').strip():
                    self._append_rx((line or '').strip())
                    return
        except Exception:
            pass
    ...
    def _open_serial(self, port: str, baud: int = 38400):
        ...
        try:
            ...
            self._status(f'Opened {port} @ {baud} bps'); diag_log(f"[OPEN] port='{port}' baud={baud}")
            return True
        except Exception as e:
    ...
    def _serial_write_bytes(self, data: bytes):
        ...
        try:
            ...
            try: self._status(f'Serial write failed: {e}')
            except Exception: pass
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Robust Chat v1.6")
        ...
        # Secure Pad padlock (between main content and Beacons; bottom-aligned)
        self.secure_btn = QPushButton("🔒")
        self.secure_btn.setToolTip("Open Secure Pad")
        self.secure_btn.setFlat(True)
        self.secure_btn.setFixedWidth(64)
        self.secure_btn.clicked.connect(self.open_secure_pad)
        self.content_layout.addWidget(self.secure_btn, alignment=Qt.AlignBottom)
        ...
        try:
            ...
            try:
                ...
                pass
        # Global colors
        self.COLOR_SENT = QColor("#088F8F"); self.COLOR_RX = QColor(255,165,0)
    ...
    def _build_linkmap_left_view(self):
        ...
        def _get_live():
            ...
            try:
                ...
                parents = {}
            # normalize children (accept schemas: list, {"children": {...}}, {"last":ts,"children":{...}})
            children = {}
            for p, v in (parents.items() if isinstance(parents, dict) else []):
                if isinstance(v, (list,tuple)):
                    children[p] = [str(x) for x in v if isinstance(x,(str,))]
                elif isinstance(v, dict):
                    vc = v.get("children") if isinstance(v.get("children"), dict) else {}
                    children[p] = [str(k) for k in vc.keys() if isinstance(k,(str,))]
                else:
                    children[p] = []
            edges = []
    ...
    def _ui_serial_section(self):
        ...
        self.connect_button = QPushButton("Connect"); self.connect_button.clicked.connect(self._on_connect_click); row.addWidget(self.connect_button)
        refresh_btn = QPushButton("Disconnect"); refresh_btn.clicked.connect(self.refresh_serial_ports); row.addWidget(refresh_btn)
        
        refresh_btn.clicked.connect(lambda: self._style_connect_button('normal'))
        disc_btn = QPushButton("Exit"); disc_btn.clicked.connect(lambda: __import__('PyQt5.QtWidgets').QtWidgets.QApplication.instance().quit()); row.addWidget(disc_btn)
        v.addLayout(row)
        row2 = QHBoxLayout()
        self.kiss_check = QCheckBox('KISS Mode (must be unchecked for Chat)'); row2.addWidget(self.kiss_check)
        self.kiss_check.setChecked(True)
...
# (removed duplicate Monitor All checkbox creation)
    ...
    def toggle_serial(self):
        ...
        if self.connect_button.text() == 'Connect':
            ...
            if not port:
                QMessageBox.warning(self, 'Serial', 'Please select a COM port first.')
                return
            if self._open_serial(port, getattr(self, '_serial_baud_default', 38400)):
                self.connect_button.setText("Connect")
                self._status('Connected')
                try:
                    self._save_serial_to_settings(port, getattr(self, '_serial_baud_default', 38400))
                except Exception:
                    pass
                try:
                    self.reader_thread = SerialReaderThread(self)
    ...
    def load_settings(self):
        ...
        try:
            ...
            pass
        # Load/persist serial default baud=38400 if none set
        try:
            self._load_serial_from_settings()
            if not getattr(self, '_serial_baud_default', None):
                self._serial_baud_default = 38400
        except Exception:
            self._serial_baud_default = 38400
    ...
    def apply_theme(self, name: str):
        ...
        # --- HUD (status bar) text color per theme ---
        try:
            sb = None
            try:
                sb = self.status_bar if hasattr(self, "status_bar") else self.statusBar()
            except Exception:
                sb = getattr(self, "status_bar", None)
            if sb:
                theme_name = (name or "").lower()
                if "dark green" in theme_name:
                    sb.setStyleSheet("color: lime;")
                else:
                    # fall back to theme color or default black
                    try:
                        c = getattr(self, "theme_mgr", None).color("status_text", "#000000")
                    except Exception:
                        c = "#000000"
                    sb.setStyleSheet(f"color: {c};")
        except Exception:
            pass
        # --- end HUD theme ---
    ...
    def _load_link_graph(self):
        '''Load graph from store/link_graph.json; preserve dict schema (parents -> {last, children{...}}).'''
        try:
            data = load_json(os.path.join('store','link_graph.json')) or {}
            parents = data.get('parents') if isinstance(data, dict) else {}
            fixed = {}
            if isinstance(parents, dict):
                for pk, pv in parents.items():
                    if isinstance(pv, dict):
                        fixed[str(pk)] = {
                            'last': int(pv.get('last', 0)) if isinstance(pv.get('last', 0), int) else pv.get('last', 0),
                            'children': dict(pv.get('children') or {})
                        }
                    elif isinstance(pv, (list, tuple)):
                        fixed[str(pk)] = {'last': 0, 'children': {str(x): {'last': 0} for x in pv if isinstance(x,(str,))}}
                    else:
                        fixed[str(pk)] = {'last': 0, 'children': {}}
                self._graph_parents = fixed
            else:
                self._graph_parents = {}
    ...
    def _save_link_graph(self):
        ...
        try:
            def _valid_graph(g):
                ...
                for pv in g.values():
                    if not isinstance(pv, dict): return False
                    ch = pv.get('children', {})
                    if not isinstance(ch, dict): return False
                return True
            graph = self._graph_parents if _valid_graph(getattr(self, '_graph_parents', {})) else {}
            payload = {"parents": graph}
            if getattr(self, '_last_beacon_heard_ts', None):
                payload["last_beacon_ts"] = self._last_beacon_heard_ts.isoformat(timespec="seconds")
            save_json(os.path.join('store','link_graph.json'), payload)
        except Exception:
            pass
    ...
    def _scan_for_beacon_line(self, line: str):
        '''Parse one-line beacons: '..PARENT [Lat xx.xxxxx Lon yy.yyyyy|POS:lat,lon] [/CHILD1 [/CHILD2 ...]]'.
        Updates link_graph (parent->children), positions.json, and refreshes UI/map in one pass.
        '''
        try:
            raw = (line or "").strip()
            if not raw:
                return
            txt = raw.upper()
            # Parent must be at the start
            m_parent = re.match(r'^\.{2}\s*([A-Z0-9/]+)', txt)
            if not m_parent:
                return
            parent = m_parent.group(1).strip().upper()
            self._record_parent_beacon(parent)

            # Position: try POS:<lat>,<lon> first; else LAT/LON tokens anywhere
            lat = lon = None
            m_pos = re.search(r'POS\s*[:=]?\s*([+-]?\d+(?:\.\d+)?)[,;\s]+([+-]?\d+(?:\.\d+)?)', raw, flags=re.I)
            if not m_pos:
                m_lat = re.search(r'LAT\s*[:=]?\s*([+-]?\d+(?:\.\d+)?)', raw, flags=re.I)
                m_lon = re.search(r'LON\s*[:=]?\s*([+-]?\d+(?:\.\d+)?)', raw, flags=re.I)
                if m_lat and m_lon:
                    lat, lon = m_lat.group(1), m_lon.group(1)
            else:
                lat, lon = m_pos.group(1), m_pos.group(2)

            try:
                if lat is not None and lon is not None:
                    self._positions_upsert(parent, float(lat), float(lon), source='beacon-rx')
                    self._save_positions()
            except Exception:
                pass

            # Children: collect all '/CHILD' tokens anywhere on the line
            kids = re.findall(r'/\s*([A-Z0-9/]+)', txt)
            for ch in kids:
                try:
                    self._record_child_beacon(ch.strip().upper())
                except Exception:
                    pass

            # Refresh UI and map + persist
            try:
                self._save_link_graph()
            except Exception:
                pass
...
# ---- Messages storage (store/messages_v1.json) ----
    ...
    def send_message(self):
        ...
        try:
            ...
            pass
    # --- Secure Pad integration ---
    def open_secure_pad(self, initial_text: str = "", from_call: str = None, show_to: bool = True):
        try:
            dlg = SecurePadDialog(self, initial_text=initial_text, from_call=from_call, show_to=show_to)
            dlg.setModal(True)
            dlg.resize(1000, 700)
            return dlg.exec_()
        except Exception as e:
            try: self._status(f"Secure Pad failed: {e}")
            except Exception: pass
            return 0

    def _send_securepad(self, to: str, msg: str) -> bool:
        try:
            me = (self.mycall_edit.text() if hasattr(self, 'mycall_edit') else '').strip().upper()
        except Exception:
            me = ''
        if not me:
            try: QMessageBox.information(self, 'Send', 'Please fill your From/MYCALL first.')
            except Exception: pass
            return False
        try:
            ack_id = self._next_ack_id()
        ...
        try:
            ...
            it = QListWidgetItem(self._fmt_disp_prefix(now_iso) + self._strip_ack_id(first) + self._ticks_for_status('attempt 1/3'))
            from PyQt5.QtGui import QBrush
            it.setForeground(QBrush(self.COLOR_SENT))
            it.setData(Qt.UserRole,'sent')
            try:
        ...
        try:
            st = AckState(self, ack_id, base_line, pause)
            self._ack_states[ack_id] = st; st.start(); ok = True
        except Exception:
            ok = bool(self.send_user_text(base_line))
        try: self._touch_last_tx()
        except Exception: pass
        return ok
    ...
    def add_received_message(self, line: str):
        ...
        # Intercept encrypted messages and divert to Secure Pad (no list, no JSON)
        try:
            s = str(line)
        except Exception:
            s = ""
        if ("[ENC2]" in s) or ("[ENC1]" in s) or ("[ENC]" in s):
            from_call = None
            enc_text = s
            try:
                import re as _re
                m = _re.search(r'^\s*(\S+)\s+DE\s+(\S+)\s+(.*)$', s)
                if m:
                    from_call = (m.group(2) or "").strip().upper()
                    rest = m.group(3) or ""
                else:
                    rest = s
                p = min([q for q in [rest.find("[ENC2]"), rest.find("[ENC1]"), rest.find("[ENC]")] if q != -1] or [-1])
                enc_text = rest[p:] if p >= 0 else rest
            except Exception:
                pass
            try:
                self.open_secure_pad(initial_text=enc_text, from_call=from_call, show_to=False)
            except Exception:
                pass
            return
...
# ======== Linked/Unlinked Beacon Pane + Link Graph Helpers ========
try:
...
LINK_WINDOW_SEC = 17 * 60          # 17 minutes for Linked/Unlinked status
MAIN_BEACON_WINDOW_SEC = 60 * 60   # Visibility window for beacon pane

def _rc_base_callsign(cs: str) -> str:
    try:
        return base_callsign(cs)
    except Exception:
        return (cs or "").strip().upper()

# BeaconPane widget: fixed top/bottom split (Linked top 1/3, Unlinked bottom 2/3)
class BeaconPane(QWidget):
    def __init__(self, owner):
        super().__init__()
        self.owner = owner
        ...
        self.lbl_linked = QLabel("Linked")
        f = self.lbl_linked.font(); f.setBold(True); self.lbl_linked.setFont(f)
        self.v.addWidget(self.lbl_linked)

        self.list_linked = QListWidget()
        self.list_linked.setSelectionMode(QListWidget.NoSelection)
        self.v.addWidget(self.list_linked, 1)  # top third

        self.lbl_unlinked = QLabel("Unlinked")
        fu = self.lbl_unlinked.font(); fu.setBold(True); self.lbl_unlinked.setFont(fu)
        self.v.addWidget(self.lbl_unlinked)

        self.list_unlinked = QListWidget()
        self.list_unlinked.setSelectionMode(QListWidget.NoSelection)
        self.v.addWidget(self.list_unlinked, 2)  # bottom two-thirds

        self._set_placeholders()

    def _set_placeholders(self):
        try:
            self.list_linked.clear(); self.list_unlinked.clear()
            dim = QBrush(getattr(self.owner, "COLOR_SYS", QColor(160,160,160)))
            it1 = QListWidgetItem("")
            it1.setForeground(dim); it1.setFlags(it1.flags() & ~Qt.ItemIsSelectable)
            self.list_linked.addItem(it1)
            it2 = QListWidgetItem("")
            it2.setForeground(dim); it2.setFlags(it2.flags() & ~Qt.ItemIsSelectable)
            self.list_unlinked.addItem(it2)
        except Exception:
            pass

    def _linked_color(self):
        try:
            return self.owner._linked_icon_color()
        except Exception:
            return QColor(0, 200, 255)

    def refresh(self):
        try:
            if hasattr(self.owner, "_load_link_graph"):
                self.owner._load_link_graph()
            gp = getattr(self.owner, "_graph_parents", {}) or {}
            my = self.owner._my_base() if hasattr(self.owner, "_my_base") else "MYCALL"

            linked, unlinked = [], []
            now = int(time.time())
            for p, ent in gp.items():
                last = int(ent.get("last") or 0)
                # Only show parents inside main window
                if (now - last) > MAIN_BEACON_WINDOW_SEC:
                    continue
                is_link = False
                try:
                    is_link = self.owner._is_parent_linked(p)
                except Exception:
                    pass
                (linked if is_link else unlinked).append((p, ent))

            key_last = lambda e: int(e[1].get("last") or 0)
            linked.sort(key=key_last, reverse=True)
            unlinked.sort(key=key_last, reverse=True)

            self.list_linked.clear(); self.list_unlinked.clear()
            col_link = self._linked_color()
            col_unlk = QColor(128,128,128)

            def _fmt_age(ts):
                d = max(0, int(time.time()) - int(ts or 0))
                m, s = divmod(d, 60); h, m = divmod(m, 60)
            ...
            if linked:
                for p, ent in linked:
                    it = QListWidgetItem(f"🔗 {p}   · {_fmt_age(ent.get('last'))} ago")
                    it.setForeground(QBrush(col_link))
                    self.list_linked.addItem(it)
                    kids = sorted((ent.get("children") or {}).items(),
                                  key=lambda kv: int(kv[1].get("last") or 0),
                                  reverse=True)
                    for k, meta in kids:
                        if _rc_base_callsign(k) == my:
                            continue
                        child_linked = False
                        try:
                            child_linked = self.owner._is_station_linked(k)
                        except Exception:
                            pass
                        prefix = "  🔗 " if child_linked else "    "
                        kit = QListWidgetItem(f"{prefix}{k}   · {_fmt_age(meta.get('last'))} ago")
                        kit.setForeground(QBrush(col_link if child_linked else col_unlk))
                        self.list_linked.addItem(kit)
            else:
                it = QListWidgetItem("")
                it.setForeground(QBrush(col_unlk))
                it.setFlags(it.flags() & ~Qt.ItemIsSelectable)
                self.list_linked.addItem(it)

            # Unlinked section
            if unlinked:
                for p, ent in unlinked:
                    it = QListWidgetItem(f"○ {p}   · {_fmt_age(ent.get('last'))} ago")
                    it.setForeground(QBrush(col_unlk))
                    self.list_unlinked.addItem(it)
                    kids = sorted((ent.get("children") or {}).items(),
                                  key=lambda kv: int(kv[1].get("last") or 0),
                                  reverse=True)
                    for k, meta in kids:
                        ...
                        kit = QListWidgetItem(f"{prefix}{k}   · {_fmt_age(meta.get('last'))} ago")
                        kit.setForeground(QBrush(col_link if child_linked else col_unlk))
                        self.list_unlinked.addItem(kit)
            else:
                it = QListWidgetItem("")
                it.setForeground(QBrush(col_unlk))
                it.setFlags(it.flags() & ~Qt.ItemIsSelectable)
...
# Monkey-patched helpers bound to ChatApp at runtime (avoid editing class body)
def _rc_link_graph_load(self):
    d = load_json('link_graph.json') or {}
    g = d.get('parents') or {}
    self._graph_parents = g if isinstance(g, dict) else {}

def _rc_link_graph_save(self):
    d = getattr(self, "_graph_parents", {}) or {}
    try:
        # Guard: never wipe an existing file with empty content
        if not d:
            p = os.path.join('store', 'link_graph.json') if os.path.isdir('store') else 'link_graph.json'
            if os.path.exists(p):
                return
        save_json('link_graph.json', {"parents": d})
    except Exception:
        pass

def _rc_link_graph_update(self, parent: str, children: list, ts: int = None):
    try:
        import time as _t
        p = _rc_base_callsign(parent)
        if not p: return
        ts = int(ts or _t.time())
        gp = getattr(self, "_graph_parents", {})
        if not isinstance(gp, dict): gp = {}
        ent = gp.get(p) or {"last": 0, "children": {}}
        ent["last"] = ts
        ch = ent.get("children") or {}
        for c in (children or []):
            cc = _rc_base_callsign(c)
            if not cc: continue
            ce = ch.get(cc) or {"last": 0}
            ce["last"] = ts
            ch[cc] = ce
        ent["children"] = ch
        gp[p] = ent
        self._graph_parents = gp
        _rc_link_graph_save(self)
    except Exception:
        pass

def _rc_is_fresh(self, ts: int, window: int):
    try:
        if not ts: return False
        import time as _t
        return (int(_t.time()) - int(ts)) <= int(window)
    except Exception:
        return False

def _rc_my_base(self):
    try:
        return _rc_base_callsign(self.mycall_edit.text().strip().upper())
    except Exception:
        return "MYCALL"

def _rc_is_parent_linked(self, parent: str) -> bool:
    try:
        gp = getattr(self, "_graph_parents", {}) or {}
        ent = gp.get(_rc_base_callsign(parent)) or {}
        my = _rc_my_base(self)
        child = (ent.get("children") or {}).get(my) or {}
        return _rc_is_fresh(self, child.get("last"), LINK_WINDOW_SEC)
    except Exception:
        return False

def _rc_is_station_linked(self, callsign: str) -> bool:
    return _rc_is_parent_linked(self, callsign or '')

def _rc_linked_icon_color(self):
    try:
        nm = (getattr(self, "theme_name", "") or "").lower()
    except Exception:
        nm = ""
    if "dark green" in nm: return QColor(124,252,0)
    if "dark red" in nm:   return QColor(255,77,77)
    return QColor(0,128,255)

def _rc_install_beacon_pane(self):
    # bind helpers if not already
    for name, fn in {
        "_load_link_graph": _rc_link_graph_load,
        "_save_link_graph": _rc_link_graph_save,
        "_update_link_graph": _rc_link_graph_update,
        "_is_fresh": _rc_is_fresh,
        "_my_base": _rc_my_base,
        "_is_parent_linked": _rc_is_parent_linked,
        "_is_station_linked": _rc_is_station_linked,
        "_linked_icon_color": _rc_linked_icon_color,
    }.items():
        if not hasattr(self, name):
            try: setattr(self, name, fn.__get__(self, self.__class__))
            except Exception: pass

    # Create the pane if not present
    try:
        if not hasattr(self, "beacon_pane") or self.beacon_pane is None:
            self.beacon_pane = BeaconPane(self)
            # Try to embed in an existing left column; if we can't find it, dock it.
            dock = QDockWidget("Beacons", self)
            dock.setFeatures(QDockWidget.DockWidgetMovable | QDockWidget.DockWidgetFloatable)
            dock.setAllowedAreas(Qt.LeftDockWidgetArea | Qt.RightDockWidgetArea)
            dock.setWidget(self.beacon_pane)
            self.addDockWidget(Qt.LeftDockWidgetArea, dock)
            self._beacon_dock = dock
    except Exception:
        pass

    # Refresh timer
    try:
        if not hasattr(self, "_beacon_refresh_timer"):
            self._beacon_refresh_timer = QTimer(self)
            self._beacon_refresh_timer.timeout.connect(lambda: getattr(self.beacon_pane, "refresh", lambda: None)())
            self._beacon_refresh_timer.start(15000)  # 15s  # 60s
    except Exception:
        pass
    ...
    try:
        self._load_link_graph()
        self.beacon_pane.refresh()
    except Exception:
        pass

    # Try to patch the map with badges after it has been created
    def _try_patch_linkmap():
        try:
            lm = getattr(self, "linkmap", None)
            if lm is None:
                return
            # attach an is_linked callable
            try:
                lm.is_linked = lambda cs: _rc_is_station_linked(self, cs)
            except Exception:
                pass

            # Wrap draw_graph to add badges after drawing
            if not hasattr(lm, "_rc_badge_wrapped"):
                orig = lm.draw_graph
                def _wrapped():
                    res = orig()
                    try:
                        # Nothing else to do here; LinkMapWidget should use is_linked when creating labels.
                        pass
                    except Exception:
                        pass
                    return res
                lm.draw_graph = _wrapped
                lm._rc_badge_wrapped = True
            # trigger a redraw now
            try: lm.draw_graph()
            except Exception: pass
        except Exception:
            pass
    ...
    try:
        # Try now and again in a moment in case map constructs late
        _try_patch_linkmap()
        QTimer.singleShot(1500, _try_patch_linkmap)
    except Exception:
        pass

# Bind installer to ChatApp on import
try:
    ChatApp._install_beacon_pane = _rc_install_beacon_pane
except Exception:
    pass
# ======== Patch: Beacons Heard list with Linked icons & priority ordering (no headers) ========
def _rc_refresh_beacons_icons(self):
    '''
    Draw the 'Beacons Heard' list as a single list:
      - Parents ordered: Linked (fresh) first, then Unlinked, each newest-first
      - Prefix each parent with 🔗 if linked (within 17 min), else ○
      - Under each parent, list children (excluding /MYCALL); prefix child 🔗 if independently linked
    '''
    try:
        # Find a likely list widget for Beacons Heard
        lst = None
        for attr in ('beacons_list', 'beacon_list', 'beaconsHeardList', 'list_beacons', 'beacons_list_widget'):
        ...
        # Ensure graph loaded
        try: self._load_link_graph()
        except Exception: pass
        gp = getattr(self, '_graph_parents', {}) or {}

        my = self._my_base() if hasattr(self, '_my_base') else 'MYCALL'

        # Partition parents and sort
        now = int(time.time())
        def fresh(ts, win): 
            try: return (now - int(ts or 0)) <= int(win)
            except Exception: return False

        linked, unlinked = [], []
        for p, ent in gp.items():
            last = int(ent.get('last') or 0)
            if not fresh(last, MAIN_BEACON_WINDOW_SEC):
                continue
            (linked if self._is_parent_linked(p) else unlinked).append((p, ent))
        ...
        # Theme colours
        from PyQt5.QtGui import QBrush, QColor
        col_link = self._linked_icon_color() if hasattr(self, '_linked_icon_color') else QColor(0,200,255)
        col_unlk = QColor(128,128,128)
        col_parent = getattr(self, 'COLOR_RX', QColor(255,165,0))  # fall back to orange for text
        ...
        # Render a group (first linked, then unlinked)
        def render_group(group, linked_flag):
            for p, ent in group:
                # parent line
                icon = "🔗" if linked_flag else "○"
                it = QListWidgetItem(f"{icon} {p}   · {fmt_age(ent.get('last'))} ago")
                it.setForeground(QBrush(col_link if linked_flag else col_parent))
                lst.addItem(it)
                # children
                kids = sorted((ent.get('children') or {}).items(), key=lambda kv: int(kv[1].get('last') or 0), reverse=True)
                for k, meta in kids:
                    if base_callsign(k) == my:
                        continue
                    child_linked = False
                    try:
                        child_linked = self._is_station_linked(k)
                    except Exception:
                        pass
                    prefix = "  🔗 " if child_linked else "     "
                    kit = QListWidgetItem(f"{prefix}{k}   · {fmt_age(meta.get('last'))} ago")
                    kit.setForeground(QBrush(col_link if child_linked else col_unlk))
                    lst.addItem(kit)

        render_group(linked, True)
        render_group(unlinked, False)

        # If empty, leave list blank (no placeholder)
    except Exception:
        # never break UI
        try:
            # minimal fallback: clear
            lst = getattr(self, 'beacons_list', None)
            if lst: lst.clear()
        except Exception:
            pass
            if lst: lst.clear()
...
def _rc_install_beacon_icons_refresh(self):
    ...
    # Install refresher
    try:
        self._refresh_beacons_ui = _rc_refresh_beacons_icons.__get__(self, self.__class__)
    except Exception:
        pass

    # Timer for auto-refresh each 60s
    try:
        if not hasattr(self, "_beacon_refresh_timer"):
            from PyQt5.QtCore import QTimer
            self._beacon_refresh_timer = QTimer(self)
            self._beacon_refresh_timer.timeout.connect(lambda: getattr(self, "_refresh_beacons_ui", lambda: None)())
            self._beacon_refresh_timer.start(15000)  # 15s
    except Exception:
        pass
...
# Bind installer
try:
    ChatApp._install_beacon_icons_refresh = _rc_install_beacon_icons_refresh
except Exception:
    pass
# ======== End Patch ========

# ======== End Beacon Pane block ========

# ======== Disable BeaconPane dock (use only right-side Beacons Heard list) ========
def _rc_disable_beacon_pane(self):
    try:
        # If a previous build created a dock, close and remove it
        dock = getattr(self, "_beacon_dock", None)
        if dock is not None:
            try:
                dock.close()
            except Exception:
            ...
            try:
                ...
                pass
            try:
                delattr(self, "_beacon_dock")
            except Exception:
                pass
        # Also drop any beacon_pane widget reference
        if hasattr(self, "beacon_pane"):
            try:
                w = getattr(self, "beacon_pane", None)
                if w is not None and hasattr(w, "setParent"):
                    w.setParent(None)
            except Exception:
                pass
            try:
                delattr(self, "beacon_pane")
            except Exception:
                pass
    except Exception:
        pass
...
try:
    ChatApp._install_beacon_pane = _rc_disable_beacon_pane
except Exception:
    pass
# ======== End disable block ========
...
def main():
    ...
    try:
        ...
        w = ChatApp()
        try:
            w._install_beacon_pane()
        except Exception:
            pass
        try:
            w._install_beacon_icons_refresh()
        except Exception:
            pass
        try:
            from PyQt5.QtCore import QTimer
            QTimer.singleShot(500, lambda: getattr(w, '_refresh_beacons_ui', lambda: None)())
        except Exception:
            pass
        try:
            from PyQt5.QtCore import QTimer
            if hasattr(w, '_link_graph_prune'):
                w._link_graph_prune(expiry_sec=3600)
                w._lg_prune_timer = QTimer(w)
                w._lg_prune_timer.timeout.connect(lambda: w._link_graph_prune(expiry_sec=3600))
                w._lg_prune_timer.start(10 * 60 * 1000)
        except Exception:
            pass
...
# ======== Monkeypatch: F3_LOOSE_RX (no monitor/console in Messages; accept any RX) ========
try:
    import time as _t, re as _re, json as _json

    def _f3_persist_message(kind, text_line):
        try:
            entry = {
                "dir": "TX" if kind == "tx" else ("SYS" if kind == "sys" else "RX"),
                "text": str(text_line or ""),
                "ts": __import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat(timespec='seconds').replace('+00:00','Z')
            }
            data = load_json('messages_v1.json') or {}
            msgs = data.get('messages') if isinstance(data, dict) else None
            if not isinstance(msgs, list): msgs = []
            msgs.append(entry)
            save_json('messages_v1.json', {"version":1, "messages":msgs})
        except Exception:
            pass

    def _f3_register_sent(app, text):
        try:
            if not hasattr(app, 'recent_sent'): app.recent_sent = []
            now = _t.time()
            # keep 3s window
            win = getattr(app, 'echo_window_s', 3.0)
            app.recent_sent = [it for it in app.recent_sent if now - it.get('ts', 0) <= win]
            app.recent_sent.append({'ts': now, 'full': (text or '').strip()})
        except Exception:
            pass

    def _f3_is_self_echo(app, line):
        try:
            now = _t.time()
            lin = (line or '').strip()
            win = getattr(app, 'echo_window_s', 3.0)
            for it in list(getattr(app, 'recent_sent', []) or []):
                if now - it.get('ts', 0) <= win:
                    if lin == (it.get('full') or '').strip():
                        return True
        except Exception:
            pass
        return False

    # Wrap send to register echo + persist TX (no change to your actual send mechanics)
    if hasattr(ChatApp, 'send_user_text'):
        _F3_ORIG_SEND = ChatApp.send_user_text
        def _F3_SEND_WRAP(self, text):
            try: _f3_persist_message('tx', text)
            except Exception: pass
            ok = _F3_ORIG_SEND(self, text)
            try: _f3_register_sent(self, text)
            except Exception: pass
            return ok
        ChatApp.send_user_text = _F3_SEND_WRAP

    # Helper to append to Messages (RX) and persist
    def _f3_append_rx(self, line):
        try:
            if hasattr(self, '_append_rx'): self._append_rx(line)
            elif hasattr(self, '_append_rx_ui_only'): self._append_rx_ui_only(line)
        except Exception:
            pass
        try: _f3_persist_message('rx', line)
        except Exception: pass

    # Console/monitor heuristics (exclude from Messages)
    _F3_MONITOR_PAT = _re.compile(
        r'^(\\[MON\\]|\\[KISS\\]|\\[APRS\\]|\\[RX\\]|fm\\s+\\w+\\s+to\\s+\\w+\\s+ctl\\s+.*?\\s+pid\\s+F0)', _re.I)

    def _F3_on_serial_thread_line(self, line: str):
        # de-dup within 300 ms
        try:
            nowm = _t.monotonic()
            raw = (line or '')
            if getattr(self, '_last_recv_line', None) == raw and (nowm - getattr(self, '_last_recv_mono', 0.0)) <= 0.3:
                return
            self._last_recv_line = raw; self._last_recv_mono = nowm
        except Exception:
            pass
        ...
        # Drop self-echo
        try:
            if _f3_is_self_echo(self, s):
                return
        except Exception:
            pass

        # Ignore console/monitor frames entirely for Messages
        try:
            if _F3_MONITOR_PAT.search(s):
                return
        except Exception:
            pass

        # Loose accept: any other line is a received chat line
        self._f3_append_rx(s)

    # Patch it in
    ChatApp._on_serial_thread_line = _F3_on_serial_thread_line

except Exception as _e_f3:
    try:
        diag_log(f"F3_LOOSE_RX patch failed: {_e_f3}")
    except Exception:
        pass

# ======== End Monkeypatch F3_LOOSE_RX ========



# ======== Monkeypatch: F4_REMOVE_MONITOR_ALL ========
try:
    def _f4_disable_monitor_all(self):
        try:
            # Hide the checkbox if it exists
            if hasattr(self, 'monitor_all_check') and None:
                try:
                    None.hide()
                except Exception:
                    pass
            # Remove any menu action named "Monitor All" if present
            for attr in dir(self):
                try:
                    obj = getattr(self, attr)
                    name = getattr(obj, 'text', None)
                    if callable(name) and 'monitor' in (obj.text() or '').lower():
                        try: obj.setVisible(False)
                        except Exception: pass
                except Exception:
                    pass
        except Exception:
    ...
    # Replace any toggle handler with a no-op
    def _f4_on_monitor_all_toggled(self, checked):
        # Intentionally disabled to prevent confusion
        try:
            self._status("Monitor All removed in v1.4.81.F4", 2000)
        except Exception:
            pass
        return

    # Patch onto ChatApp
    ChatApp._on_monitor_all_toggled = _f4_on_monitor_all_toggled
    # Call the disable routine once the window is constructed; try to run soon after class load
    _orig_init = getattr(ChatApp, "__init__", None)
    if callable(_orig_init):
        def _init_wrap(self, *a, **kw):
            _orig_init(self, *a, **kw)
            try: _f4_disable_monitor_all(self)
            except Exception: pass
        ChatApp.__init__ = _init_wrap

except Exception as _e_f4:
    try:
        diag_log(f"F4_REMOVE_MONITOR_ALL patch failed: {_e_f4}")
    except Exception:
        pass

# ======== End Monkeypatch F4_REMOVE_MONITOR_ALL ========



# ======== Monkeypatch: F5_NO_MONITOR_NO_BEACON ========
# Goals:
#  - Absolutely no monitor/console frames in Messages.
#  - Beacons (TX or RX) never appear in Messages.
#  - Messages shows only TX (green) and RX (orange) chat lines.
try:
    import time as _t, re as _re

    # Patterns to exclude from Messages
    _F5_MON_PAT = _re.compile(
        r'^(\\[[0-9]{4}-[0-9]{2}-[0-9]{2}[^\\]]*\\]\\s*)?\\[MON\\]|'    # [timestamp] [MON] ...
        r'^(\\[KISS\\]|\\[APRS\\]|\\[RX\\])\\b|'                        # [KISS] [APRS] [RX]
        r'\\bfm\\s+[A-Z0-9/+-]+\\s+to\\s+[A-Z0-9/+-]+\\s+ctl\\s+.*?\\bpid\\b', _re.I)

    _F5_BEACON_PAT = _re.compile(
        r'^(\\.\\.|!|/|=)|\\[position\\b|\\bPOS:', _re.I)  # ..PARENT ... , or APRS-like !/=, or [position]/POS:
    
    def _f5_is_monitor_frame(s: str) -> bool:
        return bool(_F5_MON_PAT.search(s or ""))

    def _f5_is_beacon_line(s: str) -> bool:
        return bool(_F5_BEACON_PAT.search((s or "").strip()))

    # Ensure UI-only appender is a NO-OP so nothing leaks in from console paths
    def _f5_append_rx_ui_only_noop(self, s):
        return  # do not display console text in Messages

    # Replace UI-only appender if present
    if hasattr(ChatApp, '_append_rx_ui_only'):
        ChatApp._append_rx_ui_only = _f5_append_rx_ui_only_noop

    # Wrap send to skip persisting/echo-register for beacon text
    if hasattr(ChatApp, 'send_user_text'):
        _F5_ORIG_SEND = ChatApp.send_user_text
        def _F5_SEND_WRAP(self, text):
            s = (text or "").strip()
            is_beacon = _f5_is_beacon_line(s)
            if not is_beacon:
                try:
                    # register for echo suppression if earlier patches use it
                    if hasattr(self, 'recent_sent'):
                        now = _t.time()
                        win = getattr(self, 'echo_window_s', 3.0)
                        self.recent_sent = [it for it in self.recent_sent if now - it.get('ts', 0) <= win]
                        self.recent_sent.append({'ts': now, 'full': s})
                except Exception:
                    pass
            return _F5_ORIG_SEND(self, text)
        ChatApp.send_user_text = _F5_SEND_WRAP

    # Override RX handler to filter strictly
    _F5_OLD_RX = getattr(ChatApp, "_on_serial_thread_line", None)
    def _F5_RX(self, line: str):
        s = (line or "").strip()
        if not s:
            return
        # Drop duplicates within 300ms, if base stored those attrs
        try:
            nowm = _t.monotonic()
            if getattr(self, "_last_recv_line", None) == s and (nowm - getattr(self, "_last_recv_mono", 0.0)) <= 0.3:
                return
            self._last_recv_line = s; self._last_recv_mono = nowm
        except Exception:
            pass
        # Hard filters
        if _f5_is_monitor_frame(s):
            return  # never show monitor/console
        if _f5_is_beacon_line(s):
            # still allow internal beacon scanners to run if present
            try:
                if hasattr(self, "_scan_for_beacon_line"):
                    self._scan_for_beacon_line(s)
            except Exception:
                pass
            return  # do not print beacons in Messages
        # Echo suppression if recent_sent exists
        try:
            now = _t.time()
            for it in list(getattr(self, "recent_sent", []) or []):
                if now - it.get("ts", 0) <= getattr(self, "echo_window_s", 3.0):
                    if s == (it.get("full") or ""):
                        return
        except Exception:
            pass
        # Finally append as RX
        try:
            if hasattr(self, "_append_rx"):
                self._append_rx(s)
        except Exception:
            pass
        # Optional: if persistence helpers exist from previous patches, they will run inside _append_rx

    ChatApp._on_serial_thread_line = _F5_RX

except Exception as _e_f5:
    try:
        diag_log(f"F5_NO_MONITOR_NO_BEACON patch failed: {_e_f5}")
    except Exception:
        pass

# ======== End Monkeypatch F5_NO_MONITOR_NO_BEACON ========



# ======== Monkeypatch: F6_MON_ON_HIDDEN_SANITIZE ========
# Goals:
#  - Force "Monitor All Traffic" ON but hide/disable its UI so user isn't confused.
#  - Keep receiving as before, but Messages shows ONLY TX (green) and RX (orange) chat.
#  - Strip any "[MON]" tokens if they slip through.
#  - Never show console/instruction frames (KISS/APRS/fm ... ctl ... pid F0, tnc/cmd lines).
#  - Never show beacons in Messages.
try:
    import time as _t, re as _re, json as _json

    def _f6_force_monitor_on_and_hide(self):
        try:
            # Persist monitor_all true in settings
            try:
                s = self._settings_get()
    ...
    # Install wrapper in __init__ to force ON and hide
    _F6_ORIG_INIT = getattr(ChatApp, "__init__", None)
    if callable(_F6_ORIG_INIT):
        def _F6_INIT_WRAP(self, *a, **kw):
            _F6_ORIG_INIT(self, *a, **kw)
            try: _f6_force_monitor_on_and_hide(self)
            except Exception: pass
        ChatApp.__init__ = _F6_INIT_WRAP

    # Make any monitor toggled handler a no-op
    def _f6_on_monitor_all_toggled(self, checked):
        return
    ChatApp._on_monitor_all_toggled = _f6_on_monitor_all_toggled


    # Exclusion patterns (console/monitor + beacons)
    _F6_MON_PAT = _re.compile(
        r'^(\\[[0-9]{4}-[0-9]{2}-[0-9]{2}[^\\]]*\\]\\s*)?\\[MON\\]|'    # [timestamp] [MON]
        r'^(\\[KISS\\]|\\[APRS\\]|\\[RX\\])\\b|'                        # [KISS]/[APRS]/[RX]
        r'\\bfm\\s+[A-Z0-9/+-]+\\s+to\\s+[A-Z0-9/+-]+\\s+ctl\\s+.*?\\bpid\\b|'  # fm ... to ... ctl ... pid
        r'^(tnc[:>]|cmd[:>])', _re.I)

    _F6_BEACON_PAT = _re.compile(r'^(\\.\\.|!|/|=)|\\[position\\b|\\bPOS:', _re.I)

    def _f6_is_monitor_frame(s: str) -> bool:
        return bool(_F6_MON_PAT.search(s or ""))

    def _f6_is_beacon_line(s: str) -> bool:
        return bool(_F6_BEACON_PAT.search((s or "").strip()))

    # Sanitize helper: remove literal "[MON]" tokens if any made it into message text
    def _f6_sanitize_text(s: str) -> str:
        try:
            s = s.replace('[MON] ', '').replace('[MON]', '')
        except Exception:
            pass
        return s

    # Override RX handler to sanitize and filter
    _F6_OLD_RX = getattr(ChatApp, "_on_serial_thread_line", None)
    def _F6_RX(self, line: str):
        s = (line or '').strip()
        if not s:
            return
        # de-dup brief repeats
        try:
            nowm = _t.monotonic()
            if getattr(self, "_last_recv_line", None) == s and (nowm - getattr(self, "_last_recv_mono", 0.0)) <= 0.3:
                return
            ...
            pass
        # Drop console/monitor frames from Messages but still allow scanners to run
        if _f6_is_monitor_frame(s):
            # Allow beacon scanner to update internals if needed, but do not print
            try:
                if hasattr(self, "_scan_for_beacon_line"):
                    self._scan_for_beacon_line(s)
            except Exception:
                pass
            return
        # Drop beacons entirely from Messages (still scan)
        if _f6_is_beacon_line(s):
            try:
                if hasattr(self, "_scan_for_beacon_line"):
                    self._scan_for_beacon_line(s)
            except Exception:
                pass
            return
        # Echo suppression (3s window if recent_sent exists)
        try:
            now = _t.time()
            for it in list(getattr(self, "recent_sent", []) or []):
                if now - it.get("ts", 0) <= getattr(self, "echo_window_s", 3.0):
            ...
            pass
        # Sanitize and append as RX
        s = _f6_sanitize_text(s)
        try:
            if hasattr(self, "_append_rx"):
                self._append_rx(s)
        except Exception:
    ...
    ChatApp._on_serial_thread_line = _F6_RX

except Exception as _e_f6:
    try:
        diag_log(f"F6_MON_ON_HIDDEN_SANITIZE patch failed: {_e_f6}")
    except Exception:
        pass

# ======== End Monkeypatch F6_MON_ON_HIDDEN_SANITIZE ========



# ======== Monkeypatch: F7_ALLOW_APRS_MESSAGES_RAW ========
try:
    import re as _re

    # Detect APRS text messages (not positions): look for '::TOCALL :text' pattern
    _F7_APRS_MSG_PAT = _re.compile(r'(::\s*[A-Z0-9\-]{1,9}\s*:[^\r\n]+)')

    def _f7_is_aprs_message(s: str) -> bool:
        ss = (s or "")
        # Most logs prefix APRS frames with [APRS]; accept both with and without
        if "[APRS]" in ss or ss.upper().startswith("APRS:") or " APRS " in ss.upper():
            return bool(_F7_APRS_MSG_PAT.search(ss))
        # Fallback: naked APRS message pattern without any tags
        return bool(_F7_APRS_MSG_PAT.search(ss))

    # Extend sanitizer to strip [APRS] token as well
    def _f7_sanitize_aprs_tokens(s: str) -> str:
        try:
            s = s.replace('[APRS] ', '').replace('[APRS]', '')
            s = s.replace('APRS:', '')
        except Exception:
            pass
        return s

    # Patch into the RX override installed in F6
    _F7_OLD_RX = getattr(ChatApp, "_on_serial_thread_line", None)
    def _F7_RX(self, line: str):
        s = (line or '').strip()
        if not s:
            return
        # If it's a console/monitor or beacon frame, we normally drop in F6.
        # BUT: allow APRS *message* frames through as raw RX (sanitized).
        try:
            is_monitor = False
            is_beacon = False
            # Reuse F6 helpers if they exist
            if ' _f6_is_monitor_frame' in globals() or True:
                try:
                    is_monitor = _f6_is_monitor_frame(s)  # from F6 patch
                except Exception:
                    pass
            if ' _f6_is_beacon_line' in globals() or True:
                try:
                    is_beacon = _f6_is_beacon_line(s)    # from F6 patch
                except Exception:
                    pass

            if (is_monitor or is_beacon) and _f7_is_aprs_message(s):
                # Sanitize and append as RX
                ss = _f7_sanitize_aprs_tokens(s)
                try:
                    ss = _f6_sanitize_text(ss)  # also strip [MON] if present
                except Exception:
                    pass
                try:
                    if hasattr(self, "_append_rx"):
        ...
        # Otherwise, defer to existing F6 RX handler (which filters monitor/beacons)
        if callable(_F7_OLD_RX):
            return _F7_OLD_RX(self, line)

    ChatApp._on_serial_thread_line = _F7_RX

except Exception as _e_f7:
    try:
        diag_log(f"F7_ALLOW_APRS_MESSAGES_RAW patch failed: {_e_f7}")
    except Exception:
        pass

# ======== End Monkeypatch F7_ALLOW_APRS_MESSAGES_RAW ========



# ======== Monkeypatch: F8_HARD_REMOVE_MONITOR_UI ========
# Completely remove "Monitor All Traffic" UI from view and from layouts.
# Also hide any menu/toolbar actions that reference Monitor.
try:
    from PyQt5.QtWidgets import QWidget, QCheckBox, QAction

    def _f8_remove_widget_hard(widget):
        try:
            if widget is None:
                return
            # Hide immediately
    ...
    def _f8_scrub_monitor_ui(self):
        # 1) Known attribute
        try:
            w = getattr(self, 'monitor_all_check', None)
            if isinstance(w, QCheckBox) or isinstance(w, QWidget):
                _f8_remove_widget_hard(w)
                try: delattr(self, 'monitor_all_check')
                except Exception: pass
        except Exception:
            pass
        # 2) Scan attributes for any checkbox with "monitor" in text/name
        try:
            for attr in dir(self):
                try:
                    obj = getattr(self, attr)
                    if isinstance(obj, QCheckBox) or isinstance(obj, QWidget):
                        txt = ""
                        try:
                            if hasattr(obj, 'text'): txt = (obj.text() or '')
                        except Exception: pass
                        name = ""
                        try: name = (obj.objectName() or '')
                        except Exception: pass
                        hay = f"{txt} {name}".lower()
                        if 'monitor' in hay:
                            _f8_remove_widget_hard(obj)
            ...
            pass
        # 3) Hide any QAction mentioning Monitor
        try:
            for attr in dir(self):
                try:
                    ...
                    if isinstance(act, QAction):
                        try:
                            label = (act.text() or '').lower()
                            if 'monitor' in label:
                                try: act.setVisible(False)
                                except Exception: pass
    ...
    # Wrap __init__ to scrub after UI is constructed
    _F8_ORIG_INIT = getattr(ChatApp, "__init__", None)
    if callable(_F8_ORIG_INIT):
        def _F8_INIT_WRAP(self, *a, **kw):
            _F8_ORIG_INIT(self, *a, **kw)
            try: _f8_scrub_monitor_ui(self)
            except Exception: pass
        ChatApp.__init__ = _F8_INIT_WRAP

    # Make any monitor toggled handlers inert
    def _f8_on_monitor_all_toggled(self, checked):
        # fully removed
        return
    ChatApp._on_monitor_all_toggled = _f8_on_monitor_all_toggled

except Exception as _e_f8:
    try:
        diag_log(f"F8_HARD_REMOVE_MONITOR_UI patch failed: {_e_f8}")
    except Exception:
        pass

# ======== End Monkeypatch F8_HARD_REMOVE_MONITOR_UI ========


# ======== F13_NO_MON_PREFIX: sanitize RX/TX display text ========
try:
    import re as _re

    def _f13_sanitize_text(s: str) -> str:
        if not isinstance(s, str):
            s = str(s or '')
        # Remove leading timestamp then tags like [MON]/[KISS]/[RX]/[APRS]
        s = _re.sub(r'^\s*(\[[^\]]+\]\s*){0,1}\[(MON|KISS|RX|APRS)\]\s*', '', s, flags=_re.I)
        # Remove any residual tokens inside line
        for tag in ('[MON]', '[KISS]', '[RX]', '[APRS]'):
            s = s.replace(tag, '')
        # Collapse extra whitespace
        s = ' '.join(s.split())
        return s

    # Wrap _append_rx to sanitize text before it hits the Messages view
    if hasattr(ChatApp, '_append_rx'):
        _F13_ORIG_APPEND_RX = ChatApp._append_rx
        def _F13_APPEND_RX(self, line):
            try:
                line = _f13_sanitize_text(line)
            except Exception:
                pass
            return _F13_ORIG_APPEND_RX(self, line)
        ChatApp._append_rx = _F13_APPEND_RX

    # (Optional) also sanitize system text if it ever surfaces
    if hasattr(ChatApp, '_append_sys'):
        _F13_ORIG_APPEND_SYS = ChatApp._append_sys
        def _F13_APPEND_SYS(self, text):
            try:
                text = _f13_sanitize_text(text)
            except Exception:
                pass
            return _F13_ORIG_APPEND_SYS(self, text)
        ChatApp._append_sys = _F13_APPEND_SYS

except Exception as _e_f13:
    try:
        diag_log(f"F13 sanitize patch failed: {_e_f13}")
    except Exception:
        pass

# ======== End F13_NO_MON_PREFIX ========



# ======== F14_RX_ALL: show every received line in Messages (no filters) ========
try:
    # Preserve original handler (if needed for rollback)
    _F14_ORIG_RX = getattr(ChatApp, "_on_serial_thread_line", None)

    def _F14_RX_ALL(self, line: str):
        # Append any non-empty line to Messages as RX, unfiltered.
        try:
    ...
    ChatApp._on_serial_thread_line = _F14_RX_ALL

except Exception as _e_f14:
    try:
        diag_log(f"F14_RX_ALL patch failed: {_e_f14}")
    except Exception:
        pass
...
# ======== F15_RX_ALL_NL_SAFE: universal newline + flush timer + hex diagnostics ========
try:
    import time as _t, binascii as _binascii, re as _re
    from PyQt5.QtCore import QTimer

    # Create / append to rx_trace.log with hexdump
    def _f15_diag_trace(app, stage, **fields):
        try:
            from pathlib import Path
            import json, time as _tx
            base = app_base_dir() if 'app_base_dir' in globals() else '.'
            p = Path(base) / 'store' / 'rx_trace.log'
            p.parent.mkdir(parents=True, exist_ok=True)
            rec = {'t': int(_tx.time()), 'stage': stage}
            rec.update(fields)
    ...
    # Hold buffer across calls when no newline is present
    def _f15_init_rx_hold(self):
        if not hasattr(self, '_f15_hold'):
            self._f15_hold = {'buf': '', 'ts': 0.0}

    # Convert string to a best-effort hex preview for diagnostics
    def _f15_hex_preview(s: str, limit=64):
        try:
            if isinstance(s, bytes):
                b = s
            else:
                b = s.encode('utf-8', errors='backslashreplace')
            hx = _binascii.hexlify(b).decode('ascii')
            return hx[:limit*2] + ('...' if len(hx) > limit*2 else '')
        except Exception:
            return ''

    # Start a timer that flushes the hold buffer if no newline ever arrives
    def _f15_start_flush_timer(self):
        try:
            if getattr(self, '_f15_timer', None):
                return
            self._f15_timer = QTimer(self)
            def _tick():
                try:
                    _f15_init_rx_hold(self)
                    buf = self._f15_hold.get('buf', '')
                    ts = self._f15_hold.get('ts', 0.0)
                    if buf and (_t.monotonic() - ts) >= 0.25:  # 250 ms idle -> flush as a line
                        s = buf
                        self._f15_hold['buf'] = ''
                        self._f15_hold['ts'] = 0.0
                        _f15_diag_trace(self, 'flush_timer', flushed=s, hex=_f15_hex_preview(s))
                        if hasattr(self, '_append_rx'):
                            self._append_rx(s)
                except Exception:
                    pass
            self._f15_timer.timeout.connect(_tick)
            self._f15_timer.start(50)  # check every 50 ms
        except Exception:
            pass

    # Wrap __init__ to start timer
    _F15_ORIG_INIT = getattr(ChatApp, '__init__', None)
    if callable(_F15_ORIG_INIT):
        def _F15_INIT_WRAP(self, *a, **kw):
            _F15_ORIG_INIT(self, *a, **kw)
            try:
                _f15_init_rx_hold(self)
                _f15_start_flush_timer(self)
            except Exception:
                pass
        ChatApp.__init__ = _F15_INIT_WRAP

    # Replace RX handler: universal newline splitting + buffer hold + diagnostic tracing
    def _F15_RX(self, line: str):
        # F26a: drop self-TX monitor echoes before persisting
        if _rc_drop_console_echo(self, line):
            return
        ...
        try:
            ...
            s = str(line or '')
        # Trace raw input
        _f15_diag_trace(self, 'entry', raw=s, hex=_f15_hex_preview(s))

        _f15_init_rx_hold(self)

        # Normalize to string and split on ANY of CR, LF, CRLF, or NL in middle
        s = s.replace('\r\n', '\n').replace('\r', '\n')
        parts = s.split('\n')

        # Prepend any previous hold to the first chunk
        if self._f15_hold['buf']:
            parts[0] = self._f15_hold['buf'] + parts[0]
            self._f15_hold['buf'] = ''
            self._f15_hold['ts'] = 0.0

        # If the last part has no terminating newline, hold it for up to 250ms
        if len(parts) >= 1:
            tail = parts[-1]
            complete = parts[:-1]
        else:
        ...
        # Emit each complete line (non-empty after strip of CR/LF)
        for item in complete:
            itm = item.strip('\r')
            if itm != '':
                _f15_diag_trace(self, 'emit', line=itm, hex=_f15_hex_preview(itm))
                try:
                    if hasattr(self, '_append_rx'):
                        self._append_rx(itm)
                except Exception:
        ...
        # Update hold buffer with tail (could be empty)
        if tail:
            self._f15_hold['buf'] = tail
            self._f15_hold['ts'] = _t.monotonic()
        else:
            self._f15_hold['buf'] = ''
            self._f15_hold['ts'] = 0.0

    ChatApp._on_serial_thread_line = _F15_RX

except Exception as _e_f15:
    try:
        diag_log(f"F15_RX_ALL_NL_SAFE failed: {_e_f15}")
    except Exception:
        pass

# ======== End F15_RX_ALL_NL_SAFE ========



# ======== F16_MSGS_PARITY_WITH_1_4_1B ========
# Goal: Make Messages behave like 1.4.1.B
#  - Persist to store/messages_v1.json on every new UI message
#  - Load from disk on startup into self.chat_items and rebuild view
#  - Ensure _append_rx/_append_tx route through _add_chat_item-compatible path
...
try:
    ...
    def _f16_store_dir():
        import os
        d = os.path.join(_f16_base_dir(), 'store')
    ...
    def _f16_store_path():
        import os
        return os.path.join(_f16_store_dir(), 'messages_v1.json')
    ...
    def _f16_write_disk(arr):
        p = _f16_store_path()
        try:
            os.makedirs(os.path.dirname(p), exist_ok=True)
            payload = {'version': 1, 'messages': arr}
            with open(p, 'w', encoding='utf-8') as f:
                json.dump(payload, f, ensure_ascii=False, indent=2)
        except Exception:
            pass

    # Normalize between 1.4.1.B item and compact list format
    def _f16_item_to_disk(it):
        ts = it.get('ts')
        try:
    ...
    def _f16_disk_to_item(d):
        ts = d.get("ts")
        try:
            from datetime import datetime as _dt
            ts = _dt.fromisoformat(ts.rstrip('Z')) if isinstance(ts, str) else ts
        except Exception:
            pass
        return {
            "kind": d.get("kind"),
    ...
    def _f16_save_now(self):
        items = list(reversed(getattr(self, "chat_items", []) or []))
        if not items:
            return
        arr = [_f16_item_to_disk(it) for it in items]
        _f16_write_disk(arr)
        try:
            self.status_label.setText(f"Saved ({len(arr)})")
        except Exception:
            pass

    # Hook ChatApp: on init load, on _add_chat_item save
    _F16_ORIG_INIT = getattr(ChatApp, "__init__", None)
    def _F16_INIT(self, *a, **kw):
        if _F16_ORIG_INIT: _F16_ORIG_INIT(self, *a, **kw)
        # ensure attrs
        try:
            if not hasattr(self, "chat_items") or self.chat_items is None:
                self.chat_items = []
        except Exception:
            try: self.chat_items = []
            except Exception: pass
        ...
        # load from disk
        try:
            disk = _f16_read_disk()  # oldest-first (dict schema)
            mem = [_f16_disk_to_item(d) for d in disk]
            self.chat_items = list(reversed(mem))  # newest-first in memory
            try:
                self._rebuild_chat_view()
            except Exception:
                # Simple list fallback if no HTML view available
                if hasattr(self, "messages_list"):
                    try:
                        ...
                        for it in self.chat_items:
                            ...
                            line = it.get("text") or ""
                            role = (it.get("kind") or "").lower()
                            item = QListWidgetItem(f"[{ts_str}] {line}")
                            if role == "received":
                                item.setForeground(QBrush(self.COLOR_RX))
                            elif role == "sent":
                                item.setForeground(QBrush(self.COLOR_SENT))
                            self.messages_list.insertItem(0, item)
                        self.messages_list.scrollToTop()
                    except Exception:
                        pass
        ...
        # autosave
        try:
            self._f16_autosave = QTimer(self)
            self._f16_autosave.setInterval(300000)
            self._f16_autosave.timeout.connect(lambda: _f16_save_now(self))
            self._f16_autosave.start()
        except Exception:
            pass

    ChatApp.__init__ = _F16_INIT

    # Wrap _add_chat_item to save eagerly (parity with 1.4.1.B)
    if hasattr(ChatApp, "_add_chat_item"):
        _F16_ORIG_ADD = ChatApp._add_chat_item
        def _F16_ADD(self, *a, **kw):
            r = None
            try:
                r = _F16_ORIG_ADD(self, *a, **kw)
            finally:
                try: _f16_save_now(self)
                except Exception: pass
            return r
        ChatApp._add_chat_item = _F16_ADD

    # Ensure _append_rx and _append_tx route through _add_chat_item if present
    def _F16_RX_APPEND_WRAP(self, line: str):
        try:
            txt = str(line or '').strip()
            if not txt:
                return
            if hasattr(self, "_add_chat_item"):
                self._add_chat_item(kind="received", text=txt, to="", frm="", ack_id=None, ack=False)
            else:
                ...
                if hasattr(self, "messages_list"):
                    try:
                        ...
                        pass
                # also touch disk via chat_items if present
                try:
                    self.chat_items.insert(0, {"kind":"received","text":txt,"ts":_dt.now(),"ack":False,
                                               "ack_id":None,"to":"","frm":"","attempt":None,"max_attempts":None,
                                               "failed":False,"style":None})
                    _f16_save_now(self)
                except Exception:
                    pass
        except Exception:
    ...
    def _F16_TX_APPEND_WRAP(self, line: str):
        try:
            txt = str(line or '').strip()
            if not txt: return
            if hasattr(self, "_add_chat_item"):
                self._add_chat_item(kind="sent", text=txt, to="", frm="", ack_id=None, ack=False)
            else:
                if hasattr(self, "messages_list"):
                    try:
                        from datetime import datetime as _dt
                        ...
                        pass
                try:
                    self.chat_items.insert(0, {"kind":"sent","text":txt,"ts":_dt.now(),"ack":False,
                                               "ack_id":None,"to":"","frm":"","attempt":None,"max_attempts":None,
                                               "failed":False,"style":None})
                    _f16_save_now(self)
                except Exception:
    ...
    if hasattr(ChatApp, "_append_rx"):
        _F16_ORIG_ARX = ChatApp._append_rx
        def _F16_WRAP_ARX(self, line): 
            try: _F16_RX_APPEND_WRAP(self, line)
            except Exception: pass
            return _F16_ORIG_ARX(self, line)
        ChatApp._append_rx = _F16_WRAP_ARX
    else:
        ChatApp._append_rx = _F16_RX_APPEND_WRAP

    if hasattr(ChatApp, "_append_tx"):
        _F16_ORIG_ATX = ChatApp._append_tx
        def _F16_WRAP_ATX(self, line):
            try: _F16_TX_APPEND_WRAP(self, line)
            except Exception: pass
            return _F16_ORIG_ATX(self, line)
        ChatApp._append_tx = _F16_WRAP_ATX
    else:
        ChatApp._append_tx = _F16_TX_APPEND_WRAP

except Exception as _e_f16:
    try:
        diag_log(f"F16 parity patch failed: {_e_f16}")
    except Exception:
        pass

# ======== End F16_MSGS_PARITY_WITH_1_4_1B ========



# ======== F17: RX tap (QSerialPort or pyserial) + unconditional persist/display ========
# Purpose: if the original RX path isn't firing, tap the serial source directly and
#          route bytes into Messages + messages_v1.json with robust newline handling.
try:
    import os, json, time as _t, binascii as _binascii
    from datetime import datetime as _dt
    from PyQt5.QtCore import QTimer
    try:
    ...
    def _f17_store_path(self):
        return os.path.join(_f17_store_dir(self), 'messages_v1.json')

    def _f17_save_append(self, kind, text):
        try:
            p = _f17_store_path(self)
            data = {}
            if os.path.exists(p):
                with open(p, 'r', encoding='utf-8') as f:
            ...
            if not isinstance(msgs, list): msgs = []
            msgs.append({
                "dir": "TX" if kind=="sent" else "RX",
                "kind": "sent" if kind=="sent" else "received",
                "text": text,
                "ts": _dt.utcnow().isoformat(timespec='seconds') + 'Z'
            })
            data = {"version": 1, "messages": msgs}
            with open(p, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        except Exception:
            pass
    ...
    # ---------- UI helper ----------
    def _f17_ui_append(self, self_app, kind, text):
        # F26a: drop self-TX monitor echoes
        if _rc_drop_console_echo(self, text):
            return

        try:
            # Prefer existing appenders to keep colour
            if kind == 'received' and hasattr(self_app, '_append_rx'):
                self_app._append_rx(text)
                return
            if kind == 'sent' and hasattr(self_app, '_append_tx'):
                self_app._append_tx(text)
                return
        except Exception:
            pass
        # Fallback to messages_list if available
        try:
            from PyQt5.QtWidgets import QListWidgetItem
            from PyQt5.QtGui import QBrush
            from PyQt5.QtCore import Qt
            if hasattr(self_app, 'messages_list') and self_app.messages_list:
                ts = _dt.now().strftime("%H:%M:%S")
                item = QListWidgetItem(f"[{ts}] {text}")
                if kind == 'received' and hasattr(self_app, 'COLOR_RX'):
                    item.setForeground(QBrush(self_app.COLOR_RX))
                elif kind == 'sent' and hasattr(self_app, 'COLOR_SENT'):
                    item.setForeground(QBrush(self_app.COLOR_SENT))
                self_app.messages_list.insertItem(0, item)
                self_app.messages_list.scrollToTop()
        except Exception:
            pass

    # ---------- line processing ----------
    def _f17_process_lines(self_app, s):
        # normalize and split; accept CR, LF, CRLF; also flush partial
        try:
        ...
        s = s.replace('\r\n', '\n').replace('\r', '\n')
        parts = s.split('\n')
        buf = getattr(self_app, '_f17_buf', '')
        if buf:
            parts[0] = buf + parts[0]
            self_app._f17_buf = ''
            self_app._f17_buf_ts = 0.0
        # emit complete
        for itm in parts[:-1]:
            line = itm.strip()
            if line:
                _f17_save_append(self_app, 'received', line)
                _f17_ui_append(self_app, 'received', line)
        # hold tail
        tail = parts[-1]
        if tail:
            self_app._f17_buf = tail
            self_app._f17_buf_ts = _t.monotonic()
        else:
            self_app._f17_buf = ''
            self_app._f17_buf_ts = 0.0

    def _f17_timer_flush(self_app):
        try:
            if getattr(self_app, '_f17_buf', '') and (_t.monotonic() - getattr(self_app, '_f17_buf_ts', 0.0)) >= 0.25:
                line = self_app._f17_buf
                self_app._f17_buf = ''
                self_app._f17_buf_ts = 0.0
                line = str(line).strip()
                if line:
    ...
    # ---------- QSerialPort path ----------
    def _f17_qserial_ready(self_app):
        try:
            sp = getattr(self_app, 'serial', None) or getattr(self_app, 'serial_port', None)
            if sp is not None and QSerialPort is not None and isinstance(sp, QSerialPort):
                data = bytes(sp.readAll())
                if data:
                    try:
                        s = data.decode('utf-8', errors='replace')
                    except Exception:
                        s = repr(data)
                    _f17_process_lines(self_app, s)
        except Exception:
            pass

    # ---------- pyserial polling path ----------
    def _f17_pyserial_poll(self_app):
        try:
            ser = getattr(self_app, 'ser', None) or getattr(self_app, 'serial', None)
            if ser is not None and hasattr(ser, 'in_waiting'):
                n = 0
                try: n = ser.in_waiting
                except Exception: 
                    try: n = ser.inWaiting()
    ...
    # ---------- install in __init__ ----------
    _F17_ORIG_INIT = getattr(ChatApp, '__init__', None)
    if callable(_F17_ORIG_INIT):
        def _F17_INIT(self, *a, **kw):
            _F17_ORIG_INIT(self, *a, **kw)
            # init buffers
            self._f17_buf = ''
            self._f17_buf_ts = 0.0
            # hook QSerialPort if present
            try:
                sp = getattr(self, 'serial', None) or getattr(self, 'serial_port', None)
                if sp is not None and QSerialPort is not None and isinstance(sp, QSerialPort):
                    try: sp.readyRead.connect(lambda: _f17_qserial_ready(self))
                    except Exception: pass
            except Exception:
                pass
            # start periodic flush + polling
            try:
                self._f17_timer = QTimer(self)
                self._f17_timer.setInterval(50)
                def _tick():
                    try:
                        _f17_timer_flush(self)
                        _f17_pyserial_poll(self)
                    except Exception:
                        pass
                self._f17_timer.timeout.connect(_tick)
                self._f17_timer.start()
            except Exception:
                pass
        ChatApp.__init__ = _F17_INIT

except Exception as _e_f17:
    try:
        diag_log(f"F17_RX_TAP_AND_PERSIST failed: {_e_f17}")
    except Exception:
        pass
...
# ======== F18: Force serial baud = 38400 + stronger diagnostics ========
try:
    ...
    def _f18_diag_write(self, what: str, **fields):
        try:
            base = os.path.dirname(os.path.abspath(__file__))
            p = os.path.join(base, 'store', 'serial_diag.log')
            os.makedirs(os.path.dirname(p), exist_ok=True)
            rec = {"t": _dt.utcnow().isoformat(timespec='seconds') + 'Z', "msg": what}
            rec.update(fields)
    ...
    def _f18_force_38400(self):
        '''Force 38400 on both QtSerialPort and pyserial if present.'''
        try:
            # QtSerialPort path
            sp = getattr(self, 'serial', None) or getattr(self, 'serial_port', None)
            if QSerialPort is not None and isinstance(sp, QSerialPort):
                try:
                    current = int(sp.baudRate())
                except Exception:
                    current = None
                try:
                    if current != 38400:
                        sp.setBaudRate(38400)
                        _f18_diag_write(self, "set QtSerialPort baud", prev=current, now=38400)
                except Exception as e:
                    _f18_diag_write(self, "failed set QtSerialPort baud", error=str(e))
            # pyserial path
            ser = getattr(self, 'ser', None) or getattr(self, 'serial', None)
            try:
                # Avoid double-using same object
                if ser is not None and (QSerialPort is None or not isinstance(ser, QSerialPort)):
                    prev = None
                    try: prev = int(getattr(ser, 'baudrate', None))
                    except Exception: prev = None
                    try:
                        setattr(ser, 'baudrate', 38400)
                        _f18_diag_write(self, "set pyserial baud", prev=prev, now=38400)
                    except Exception as e:
                        _f18_diag_write(self, "failed set pyserial baud", error=str(e))
            except Exception:
                pass
        except Exception:
            pass

    # Install into __init__ right after F17
    _F18_ORIG_INIT = getattr(ChatApp, '__init__', None)
    if callable(_F18_ORIG_INIT):
        def _F18_INIT(self, *a, **kw):
            _F18_ORIG_INIT(self, *a, **kw)
            try:
                _f18_force_38400(self)
            except Exception:
                pass
            # Keep enforcing every 2 seconds (some drivers can reset baud after open)
            try:
                self._f18_timer = QTimer(self)
                self._f18_timer.setInterval(2000)
                self._f18_timer.timeout.connect(lambda: _f18_force_38400(self))
                self._f18_timer.start()
            except Exception:
                pass
            # First snapshot
            try:
                ...
                try:
                    if QSerialPort is not None and isinstance(sp, QSerialPort):
                        qt_baud = int(sp.baudRate())
                except Exception: pass
                try:
                    if ser is not None and (QSerialPort is None or not isinstance(ser, QSerialPort)):
                        py_baud = int(getattr(ser, 'baudrate', None))
                except Exception: pass
                _f18_diag_write(self, "baud snapshot", qt_baud=qt_baud, py_baud=py_baud)
            except Exception:
                pass
        ChatApp.__init__ = _F18_INIT

except Exception as _e_f18:
    try:
        diag_log(f"F18 38400 force failed: {_e_f18}")
    except Exception:
        pass
...
# ======== F19: Strip [MON] everywhere + TX lead/tail timing (250ms/250ms) ========
try:
    import re as _re, time as _t

    # --- universal sanitizer (remove [MON] and similar tags) ---
    def _f19_sanitize(line: str) -> str:
        s = '' if line is None else str(line)
        # remove timestamp [YYYY-mm-dd HH:MM:SS] if present, then a tag like [MON]/[KISS]/[RX]/[APRS]
        s = _re.sub(r'^\s*\[\d{4}-\d{2}-\d{2}[^]]+\]\s*', '', s)
        s = s.replace('[MON] ', '').replace('[MON]', '')
        # optional: strip other tags too
        for tag in ('[KISS]', '[RX]', '[APRS]'):
            s = s.replace(tag + ' ', '').replace(tag, '')
        return s.strip()

    # --- Wrap appenders so any path into the Messages list is sanitized ---
    if hasattr(ChatApp, '_append_rx'):
        _F19_ORIG_ARX = ChatApp._append_rx
        def _F19_ARX(self, line):
            try: line = _f19_sanitize(line)
            except Exception: pass
            return _F19_ORIG_ARX(self, line)
        ChatApp._append_rx = _F19_ARX

    if hasattr(ChatApp, '_append_sys'):
        _F19_ORIG_ASYS = ChatApp._append_sys
        def _F19_ASYS(self, line):
            try: line = _f19_sanitize(line)
            except Exception: pass
            return _F19_ORIG_ASYS(self, line)
        ChatApp._append_sys = _F19_ASYS

    # Ensure any UI-only/monitor path is sanitized and forwarded into main RX appender
    if hasattr(ChatApp, '_append_rx_ui_only'):
        _F19_ORIG_UI = ChatApp._append_rx_ui_only
        def _F19_UI(self, line):
            try: line = _f19_sanitize(line)
            except Exception: pass
            try:
                if hasattr(self, '_append_rx'):
                    return self._append_rx(line)
            except Exception:
                pass
            return _F19_ORIG_UI(self, line)
        ChatApp._append_rx_ui_only = _F19_UI

    # Also sanitize the F17 fallback UI appender if present
    if '_f17_ui_append' in globals():
        _F19_ORIG_UIAPP = _f17_ui_append
        def _f17_ui_append(self_app, kind, text):
            # F26a: drop self-TX monitor echoes
            if _rc_drop_console_echo(self, text):
                return

            try: text = _f19_sanitize(text)
            except Exception: pass
            return _F19_ORIG_UIAPP(self_app, kind, text)

    # --- TX timing: 250 ms lead (before send) and 250 ms tail (after send) ---
    # Applies to send_user_text and low-level serial.write where present.
    def _f19_tx_lead_tail(self):
        # Try toggling RTS/DTR briefly (best-effort; many TNCs ignore it)
        try:
            ser = getattr(self, 'ser', None) or getattr(self, 'serial', None)
            # Lead
            if ser is not None:
                for sig in ('setDTR', 'setRTS'):
                    try:
                        fn = getattr(ser, sig, None)
                        if callable(fn): fn(True)
                    except Exception: pass
            _t.sleep(0.25)  # 250 ms lead
        except Exception:
            pass

    def _f19_tx_tail(self):
        try:
            _t.sleep(0.25)  # 250 ms tail
            ser = getattr(self, 'ser', None) or getattr(self, 'serial', None)
            if ser is not None:
                for sig in ('setRTS', 'setDTR'):
                    try:
                        fn = getattr(ser, sig, None)
                        if callable(fn): fn(False)
                    except Exception: pass
        except Exception:
            pass

    # Wrap send_user_text if available
    if hasattr(ChatApp, 'send_user_text'):
        _F19_ORIG_SEND = ChatApp.send_user_text
        def _F19_SEND(self, text):
            try: _f19_tx_lead_tail(self)
            except Exception: pass
            try:
                r = _F19_ORIG_SEND(self, text)
            except Exception as e:
                r = None
            try: _f19_tx_tail(self)
            except Exception: pass
            return r
        ChatApp.send_user_text = _F19_SEND

    # Wrap low-level serial write if it's exposed as a method on the app (rare)
    if hasattr(ChatApp, 'serial_write'):
        _F19_ORIG_SW = ChatApp.serial_write
        def _F19_SW(self, data):
            try: _f19_tx_lead_tail(self)
            except Exception: pass
            try: r = _F19_ORIG_SW(self, data)
            except Exception: r = None
            try: _f19_tx_tail(self)
            except Exception: pass
            return r
        ChatApp.serial_write = _F19_SW

except Exception as _e_f19:
    try:
        diag_log(f"F19 sanitize/tx timing failed: {_e_f19}")
    except Exception:
        pass
...
# ======== F20: Block self-ACKs and suppress my own monitor echoes ========
try:
    import re as _re

    _F20_ACK_RE  = _re.compile(r'\[ACK:([0-9A-Za-z]{2,12})\]')
    _F20_CHAT_RE = _re.compile(r'^\s*([A-Z0-9/+\-]+)\s+DE\s+([A-Z0-9/+\-]+)\s+(.+?)\s*$', _re.I)
    _F20_MON_RE  = _re.compile(r'^\s*(\[[^\]]+\]\s*)?\[MON\]\s*fm\s+([A-Z0-9/+\-]+)\b', _re.I)

    def _f20_is_self_ack(app, s: str) -> bool:
        '''Return True if line contains an ACK that is from MYCALL or not addressed to MYCALL.'''
        try:
            s2 = '' if s is None else str(s)
        except Exception:
            s2 = str(s or '')
        m_chat = _F20_CHAT_RE.match(s2)
        if not m_chat:
            return False
        to_cs  = (m_chat.group(1) or '').upper().strip()
        frm_cs = (m_chat.group(2) or '').upper().strip()
        body   = (m_chat.group(3) or '')
        if not _F20_ACK_RE.search(body):
            return False
        myc = ''
        try:
            myc = app.mycall_edit.text().strip().upper()
        except Exception:
            myc = ''
        # Block if FROM == MYCALL or TO != MYCALL (only peer-to-me ACKs allowed)
        if myc and (frm_cs == myc or (to_cs and to_cs != myc)):
            return True
        return False

    def _f20_is_my_monitor_echo(app, s: str) -> bool:
        '''Detect [MON] fm <MYCALL> ... echo lines and suppress them from UI/ACK path.'''
        try:
            s2 = '' if s is None else str(s)
        except Exception:
            s2 = str(s or '')
        m = _F20_MON_RE.match(s2)
        if not m:
            return False
        ...
        try:
            ...
            myc = ''
        return bool(myc and frm == myc)

    # Wrap the serial-thread line handler to filter before any other logic runs
    _F20_ORIG_RX = getattr(ChatApp, "_on_serial_thread_line", None)
    def _F20_FILTER(self, line: str):
        try:
            s = '' if line is None else str(line)
        except Exception:
            s = str(line or '')
        # Suppress my own [MON] fm <MYCALL> ... echoes entirely
        if _f20_is_my_monitor_echo(self, s):
            return
        # Filter ACKs that are from me or not to me
        if _f20_is_self_ack(self, s):
            return
        # Otherwise, continue with original handler
        if callable(_F20_ORIG_RX):
            return _F20_ORIG_RX(self, line)

    ChatApp._on_serial_thread_line = _F20_FILTER

except Exception as _e_f20:
    try:
        diag_log(f"F20 self-ack filter failed: {_e_f20}")
    except Exception:
        pass
...
# ======== F21: Kill monitor lines, kill TX echo, strict ACK gating (TO==MYCALL, FROM!=MYCALL) ========
try:
    import re as _re, time as _t
    ...
    def _f21_myc(app):
        try:
            return app.mycall_edit.text().strip().upper()
        except Exception:
            return ""

    _F21_TAG_RE = _re.compile(r'^\s*(\[[^\]]+\]\s*)?\[(MON|KISS|RX|APRS)\]\s*', _re.I)
    _F21_CHAT_RE = _re.compile(r'^\s*([A-Z0-9/+\-]+)\s+DE\s+([A-Z0-9/+\-]+)\s+(.+?)\s*$', _re.I)
    _F21_ACK_RE  = _re.compile(r'\[ACK:([0-9A-Za-z]{2,12})\]')

    def _f21_is_monitor_line(s: str) -> bool:
        return bool(_F21_TAG_RE.match(s or "")) or " pid " in (s or "").lower()

    def _f21_is_self_ack(app, s: str) -> bool:
        '''ACKs are valid ONLY when TO == MYCALL and FROM != MYCALL. Everything else is rejected.'''
        m = _F21_CHAT_RE.match(s or "")
        if not m:
            return False
        to_cs, frm_cs, body = (m.group(1) or "").upper(), (m.group(2) or "").upper(), (m.group(3) or "")
        if not _F21_ACK_RE.search(body):
            return False
        myc = _f21_myc(app)
        if not myc:
            return False
        # Reject if FROM==MYCALL or TO!=MYCALL
        return (frm_cs == myc) or (to_cs and to_cs != myc)

    # Record last TX text and ignore exact echo for 3s
    if not hasattr(ChatApp, "_f21_recent_tx"):
        ChatApp._f21_recent_tx = []  # list of (ts, text)

    # Wrap send_user_text to remember what we sent
    if hasattr(ChatApp, "send_user_text"):
        _F21_ORIG_SEND = ChatApp.send_user_text
        def _F21_SEND(self, text):
            try:
                now = _t.time()
                txt = (text or "").strip()
                # keep 3s window
                ChatApp._f21_recent_tx = [(ts, t) for (ts, t) in ChatApp._f21_recent_tx if now - ts <= 3.0]
                if txt:
                    ChatApp._f21_recent_tx.append((now, txt))
            except Exception:
                pass
            return _F21_ORIG_SEND(self, text)
        ChatApp.send_user_text = _F21_SEND

    def _f21_is_tx_echo(s: str) -> bool:
        try:
            now = _t.time()
            txt = (s or "").strip()
            # prune
            ChatApp._f21_recent_tx = [(ts, t) for (ts, t) in ChatApp._f21_recent_tx if now - ts <= 3.0]
            for ts, t in ChatApp._f21_recent_tx:
                if txt == t:
                    return True
        except Exception:
            pass
    ...
    # 1) Kill the UI-only monitor path entirely
    def _F21_UI_NOOP(self, s): 
        return
    ChatApp._append_rx_ui_only = _F21_UI_NOOP

    # 2) Filter at the earliest RX hook
    _F21_ORIG_RX = getattr(ChatApp, "_on_serial_thread_line", None)
    def _F21_RX_FILTER(self, line: str):
        s = "" if line is None else str(line)
        # Drop monitor/console lines altogether
        if _f21_is_monitor_line(s):
            return
        # Drop exact echo of our TX
        if _f21_is_tx_echo(s):
            return
        # Drop self-ACKs / ACKs not addressed to me
        if _f21_is_self_ack(self, s):
            return
        if callable(_F21_ORIG_RX):
            return _F21_ORIG_RX(self, line)
    ChatApp._on_serial_thread_line = _F21_RX_FILTER

    # 3) If the F17 tap is installed, filter its lines too before appending
    if "_f17_process_lines" in globals():
        _F21_ORIG_F17 = _f17_process_lines
        def _f17_process_lines(self_app, s):
            # Split like original, then filter each line
            try:
                s = '' if s is None else str(s)
            except Exception:
                s = str(s or '')
            ...
            for itm in parts[:-1]:
                ...
                if not line:
                    continue
                if _f21_is_monitor_line(line):
                    continue
                if _f21_is_tx_echo(line):
                    continue
                if _f21_is_self_ack(self_app, line):
                    continue
                # pass through to original UI append path
                try:
                    _f17_save_append(self_app, 'received', line)
                    _f17_ui_append(self_app, 'received', line)
                except Exception:
            ...
            if tail:
                ...
                self_app._f17_buf = ''
                self_app._f17_buf_ts = 0.0
        globals()['_f17_process_lines'] = _f17_process_lines

except Exception as _e_f21:
    try:
        diag_log(f"F21 echo/ack/monitor filter failed: {_e_f21}")
    except Exception:
        pass
...
# ======== F22: ACK reply (hybrid header) + no ticks on received ========
try:
    import re as _re, time as _t

    _F22_CHAT_RE = _re.compile(r'^\s*([A-Z0-9/+\-]+)\s+DE\s+([A-Z0-9/+\-]+)\s+(.+?)\s*$', _re.I)
    _F22_ACK_RE  = _re.compile(r'\[ACK:([0-9A-Za-z]{2,12})\]')

    if not hasattr(ChatApp, "_f22_recent_acks"):
        ChatApp._f22_recent_acks = []  # list of (ts, from_call, ack_id)

    def _f22_myc(self):
        try: return self.mycall_edit.text().strip().upper()
        except Exception: return ""

    def _f22_already_replied(frm, ack_id):
        now = _t.time()
        ChatApp._f22_recent_acks = [(ts,f,a) for (ts,f,a) in ChatApp._f22_recent_acks if now - ts <= 60.0]
        for ts,f,a in ChatApp._f22_recent_acks:
            if f == frm and a == ack_id:
                return True
        return False

    def _f22_mark_replied(frm, ack_id):
        ChatApp._f22_recent_acks.append((_t.time(), frm, ack_id))

    # Hybrid ACK reply: "<THEIRCALL> DE <MYCALL> [ACK:ID]"
    def _f22_maybe_reply_ack(self, s: str):
        m = _F22_CHAT_RE.match(s or "")
        if not m:
            return
        to_cs, frm_cs, body = (m.group(1) or "").upper(), (m.group(2) or "").upper(), (m.group(3) or "")
        m_ack = _F22_ACK_RE.search(body)
        if not m_ack:
            return
        myc = _f22_myc(self)
        if not myc:
            return
        # Only reply when it's addressed to me and not from me
        if to_cs != myc or frm_cs == myc:
            return
        ack_id = (m_ack.group(1) or "").upper()
        if not ack_id or _f22_already_replied(frm_cs, ack_id):
            return
        reply = f"{frm_cs} DE {myc} [ACK:{ack_id}]"
        try:
            if hasattr(self, 'send_user_text'):
                self.send_user_text(reply)
                _f22_mark_replied(frm_cs, ack_id)
        except Exception:
            pass

    # Hook into the earliest RX handler after F21 filtering
    _F22_ORIG_RX = getattr(ChatApp, "_on_serial_thread_line", None)
    def _F22_RX(self, line: str):
        s = "" if line is None else str(line)
        # attempt ACK reply (hybrid) on any incoming chat with ACK token
        try: _f22_maybe_reply_ack(self, s)
        except Exception: pass
        if callable(_F22_ORIG_RX):
            return _F22_ORIG_RX(self, line)
    ChatApp._on_serial_thread_line = _F22_RX

    # Ensure received items never show ticks: coerce flags when added
    if hasattr(ChatApp, "_add_chat_item"):
        _F22_ORIG_ADD = ChatApp._add_chat_item
        def _F22_ADD(self, *a, **kw):
            # normalize args if called with keywords
            try:
                kind = kw.get("kind") if isinstance(kw, dict) else None
            except Exception:
                kind = None
            if (not kind) and len(a) >= 1 and isinstance(a[0], str):
                kind = a[0]
            # Force no-ack visuals for received
            if (kind or "").lower() in ("received", "rx"):
                try:
                    kw = dict(kw or {})
                    kw["ack"] = False
                    kw["failed"] = False
                    kw["attempt"] = None
                    kw["max_attempts"] = None
                except Exception:
                    pass
            return _F22_ORIG_ADD(self, *a, **kw)
        ChatApp._add_chat_item = _F22_ADD

except Exception as _e_f22:
    try:
        diag_log(f"F22 ACK reply patch failed: {_e_f22}")
    except Exception:
        pass
...
# ======== F23: Accept ACK only from the original TO/Target station ========
try:
    import re as _re, time as _t

    # Chat & ACK token (allow with or without colon: [ACK1234] or [ACK:1234])
    _F23_CHAT_RE = _re.compile(r'^\s*([A-Z0-9/+\-]+)\s+DE\s+([A-Z0-9/+\-]+)\s+(.+?)\s*$', _re.I)
    _F23_ACK_RE  = _re.compile(r'\[ACK:?\s*([0-9A-Za-z]{2,12})\]')

    def _f23_mycall(self):
        try: return self.mycall_edit.text().strip().upper()
        except Exception: return ""

    # Track outbound ACK expectations: ack_id -> {"target": "G4ABC", "ts": ...}
    if not hasattr(ChatApp, "_pending_outbox"):
        ChatApp._pending_outbox = {}

    # Wrap send_user_text to record target + ack_id whenever we send with an ACK tag
    if hasattr(ChatApp, "send_user_text"):
        _F23_ORIG_SEND = ChatApp.send_user_text
        def _F23_SEND(self, text):
            try:
                s = (text or "").strip()
                m = _F23_CHAT_RE.match(s)
                if m:
                    to_cs, frm_cs, body = (m.group(1) or "").upper(), (m.group(2) or "").upper(), (m.group(3) or "")
                    m_ack = _F23_ACK_RE.search(body)
                    if m_ack:
                        ack_id = (m_ack.group(1) or "").upper()
                        if ack_id:
                            # Record the intended peer target (TO) for this ack_id
                            tgt = to_cs
                            ChatApp._pending_outbox[ack_id] = {"target": tgt, "ts": _t.time()}
            except Exception:
                pass
            return _F23_ORIG_SEND(self, text)
        ChatApp.send_user_text = _F23_SEND

    # Helper to mark ACK delivered on the UI state machines, if present
    def _f23_mark_ack_received(self, ack_id: str):
        ok = False
        # 1) Try an ack state machine, if used by your build
        try:
            st = getattr(self, "_ack_states", {}).get(ack_id)
            if st:
                try:
                    st.on_ack_received()
                    ok = True
            ...
            pass
        # 2) Update chat_items if present
        if not ok:
            try:
                from datetime import datetime as _dt
                for it in (getattr(self, "chat_items", []) or []):
                    if (it.get("ack_id") or "").upper() == ack_id.upper() and (it.get("kind") or "").lower() == "sent":
                        it["ack"] = True
                        it["ts_ack"] = _dt.utcnow().isoformat() + "Z"
                        ok = True
                # Optional: rebuild view if you have a renderer
                if ok and hasattr(self, "_rebuild_chat_view"):
                    try: self._rebuild_chat_view()
                    except Exception: pass
            except Exception:
                pass
        # 3) Fallback: append a small system line
        if ok:
            try:
                if hasattr(self, "_append_sys"):
                    self._append_sys(f"✔ Ack {ack_id} received")
    ...
    # Accept ACK only when: TO == MYCALL AND FROM == the recorded target for that ack_id
    _F23_ORIG_RX = getattr(ChatApp, "_on_serial_thread_line", None)
    def _F23_RX(self, line: str):
        s = "" if line is None else str(line)
        try:
            m = _F23_CHAT_RE.match(s)
            if m:
                ...
                if m_ack:
                    ack_id = (m_ack.group(1) or "").upper()
                    myc = _f23_mycall(self)
                    # Lookup pending target
                    rec = ChatApp._pending_outbox.get(ack_id, {})
                    expected = (rec.get("target") or "").upper()
                    if myc and to_cs == myc and expected and frm_cs == expected:
                        # Valid ACK: mark and remove from pending
                        try:
                            _f23_mark_ack_received(self, ack_id)
                        except Exception:
                            pass
                        try:
                            ChatApp._pending_outbox.pop(ack_id, None)
                        except Exception:
                            pass
                        # Do NOT return: allow normal RX display of the text (without ✓ on received)
                    else:
                        # Not from the right station or not to me → ignore for ack purposes
                        pass
        except Exception:
            pass
        if callable(_F23_ORIG_RX):
            return _F23_ORIG_RX(self, line)
    ChatApp._on_serial_thread_line = _F23_RX

except Exception as _e_f23:
    try:
        diag_log(f"F23 strict-target ack failed: {_e_f23}")
    except Exception:
        pass
...
# ======== F26e: D_ALPHA-style hardening helpers ========
import re as _rc_re2, time as _rc_t2
from collections import deque as _rc_deque2

def _f26e_norm_text(s: str) -> str:
    try:
        return _rc_re2.sub(r'[\r\n]+', '', (s or '')).strip()
    except Exception:
        return (s or '').strip()

def _f26e_is_recent_echo_norm(self, s: str) -> bool:
    # Return True if 's' matches normalized text of a very recent TX (<= 3 seconds).
    try:
        now = _rc_t2.time()
        txt = _f26e_norm_text(s)
        dq = getattr(ChatApp, '_f24_recent_tx', None)
        if dq is None:
            return False
        newdq = _rc_deque2([], maxlen=getattr(dq, 'maxlen', 20))
        hit = False
        for ts, t, tar in list(dq):
            if now - ts <= 3.0:
                tnorm = _f26e_norm_text(t)
                newdq.append((ts, tnorm, tar))
                if not hit and tnorm == txt:
                    hit = True
        ChatApp._f24_recent_tx = newdq
        return hit
    except Exception:
        return False

def _f26e_drop_own_control_line(self, s: str) -> bool:
    # Drop lines like '<TO> DE <FROM> <MSG>' when FROM == MYCALL and MSG looks like control traffic.
    try:
        norm = _f26e_norm_text(s)
        m = _rc_re2.match(r'^([A-Z0-9/\-]+|CQ)\s+DE\s+([A-Z0-9/\-]+)\s*(.*)$', norm, _rc_re2.I)
        if not m:
            return False
        _to, _frm, _msg = (m.group(1) or '').upper(), (m.group(2) or '').upper(), (m.group(3) or '')
        myc = ''
        try:
            myc = self.mycall_edit.text().strip().upper()
        except Exception:
            myc = (getattr(self, 'mycall', '') or getattr(self, 'MYCALL', '') or '').strip().upper()
        if not myc:
            return False
        if _frm == myc and (_msg.startswith('FILE ') or ('[ACK:' in _msg.upper())):
            return True
        return False
    except Exception:
        return False
# ======== End F26e helpers ========
# ======== F24: Host-mode: no local echo, strict ACK gating, retry-friendly ========
try:
    import time as _t, re as _re
    from collections import deque

    # Remember recent TX (text) for echo suppression window
    if not hasattr(ChatApp, "_f24_recent_tx"):
        ChatApp._f24_recent_tx = deque(maxlen=20)  # (ts, text, target)

    def _f24_mycall(self):
        try: return self.mycall_edit.text().strip().upper()
        except Exception: return ""

    # Detect monitor/console style lines
    _F24_MON_TAG = _re.compile(r'^\s*(\[[^\]]+\]\s*)?\[(MON|KISS|RX|APRS)\]\b', _re.I)
    _F24_MON_PID = _re.compile(r'\bpid\s+[A-F0-9]+\b', _re.I)
    _F24_MON_FM  = _re.compile(r'\bfm\s+([A-Z0-9/+\-]+)\b', _re.I)

    def _f24_is_monitorish(s: str) -> bool:
        ss = s or ""
        return bool(_F24_MON_TAG.match(ss) or _F24_MON_PID.search(ss))

    # Exact TX-text echo check (within 3s)
    def _f24_is_exact_echo(s: str) -> bool:
        now = _t.time()
        txt = (s or "").strip()
        # prune
        ChatApp._f24_recent_tx = deque([(ts,t,tar) for (ts,t,tar) in ChatApp._f24_recent_tx if now - ts <= 3.0], maxlen=20)
        for ts, t, _tar in ChatApp._f24_recent_tx:
            if t == txt:
                return True
        return False

    # Self-monitor echo check: [MON] fm <MYCALL> ...
    def _f24_is_my_monitor_echo(self, s: str) -> bool:
        m = _F24_MON_FM.search(s or "")
        if not m:
            return False
        frm = (m.group(1) or "").upper()
        return frm == _f24_mycall(self)

    # Record TX before sending (for echo suppression)
    if hasattr(ChatApp, "send_user_text"):
        _F24_ORIG_SEND = ChatApp.send_user_text
        def _F24_SEND(self, text):
            try:
                target = ""
                try: target = (self.target_edit.text() or "").strip().upper()
                except Exception: target = ""
                ChatApp._f24_recent_tx.append((_t.time(), (text or "").strip(), target))
            except Exception:
                pass
            return _F24_ORIG_SEND(self, text)
        ChatApp.send_user_text = _F24_SEND

    # Kill UI monitor-path appender
    def _F24_UI_NOOP(self, s): 
        return
    ChatApp._append_rx_ui_only = _F24_UI_NOOP

    # Strict filter at the earliest RX hook: drop monitor + echoes
    _F24_ORIG_RX = getattr(ChatApp, "_on_serial_thread_line", None)
    def _F24_RX(self, line: str):
        s = "" if line is None else str(line)
        # Drop local monitor/console lines entirely
        if _f24_is_monitorish(s) and _f24_is_my_monitor_echo(self, s):
            return
        # Drop exact TX echo
        if _f24_is_exact_echo(s):
            return
        if callable(_F24_ORIG_RX):
            return _F24_ORIG_RX(self, line)
    ChatApp._on_serial_thread_line = _F24_RX

    # Also filter the F17 byte-tap path if active
    if "_f17_process_lines" in globals():
        _F24_ORIG_PROC = _f17_process_lines
        def _f17_process_lines(self_app, s):
            # normalize and split
            try:
                s = '' if s is None else str(s)
            ...
            for itm in parts[:-1]:
                ...
                if not line:
                    continue
                # Drop monitor line that is from MYCALL
                if _f24_is_monitorish(line) and _f24_is_my_monitor_echo(self_app, line):
                    continue
                # Drop exact text echo
                if _f24_is_exact_echo(line):
                    continue
                # pass through
                try:
                    _f17_save_append(self_app, 'received', line)
    ...
    # Add a minimal "cooldown gate": ignore any ACK parsed within 300 ms of a send
    # (host echoes can be near-instant)
    if not hasattr(ChatApp, "_f24_last_send_ts"):
        ChatApp._f24_last_send_ts = 0.0
    if hasattr(ChatApp, "send_user_text"):
        _F24_ORIG_SEND2 = ChatApp.send_user_text
        def _F24_SEND2(self, text):
            ChatApp._f24_last_send_ts = _t.time()
            return _F24_ORIG_SEND2(self, text)
        ChatApp.send_user_text = _F24_SEND2

    _F24_ORIG_RX2 = getattr(ChatApp, "_on_serial_thread_line", None)
    def _F24_RX2(self, line: str):
        s = "" if line is None else str(line)
        # After initial echo/monitor filters, enforce 300ms no-ACK window
        try:
            if (_t.time() - getattr(ChatApp, "_f24_last_send_ts", 0.0)) <= 0.30:
                # strip any ACK marks added by early paths by not letting it through
                pass  # we don't actively drop here; the earlier filters should have already caught echoes
        except Exception:
            pass
        if callable(_F24_ORIG_RX2):
            return _F24_ORIG_RX2(self, line)
    ChatApp._on_serial_thread_line = _F24_RX2

except Exception as _e_f24:
    try:
        diag_log(f"F24 no-local-echo patch failed: {_e_f24}")
    except Exception:
        pass
...
# ======== F25_BEACON_PIPE: Beacons -> Heartbeat + link_graph.json (+positions), never Messages ========
try:
    import re as _re, json, os
    from datetime import datetime as _dt
    from pathlib import Path as _Path

    # Pattern for beacon/heard lines:
    # ..PARENT [Lat 51.934 Lon -1.242] /CHILD1 /CHILD2 ...
    _F25_BEACON_RE = _re.compile(
        r'^\s*\.\.(?P<parent>[A-Z0-9/+\-]+)'
        r'(?:\s*\[\s*(?:LAT|Lat|lat)\s*(?P<lat>[-+]?\d+(?:\.\d+)?)\s+'
        r'(?:LON|Lon|lon)\s*(?P<lon>[-+]?\d+(?:\.\d+)?)\s*\])?'
        r'(?P<trail>(?:\s*/[A-Z0-9/+\-]+)*)\s*$'
    )

    def _f25_is_beacon_line(s: str) -> bool:
        try:
            return bool(_F25_BEACON_RE.match(s or ""))
        except Exception:
            return False

    def _f25_parse_beacon(s: str):
        m = _F25_BEACON_RE.match(s or "")
        if not m:
            return None
        parent = (m.group('parent') or '').upper()
        lat = m.group('lat')
        lon = m.group('lon')
        lat = float(lat) if lat is not None else None
        lon = float(lon) if lon is not None else None
        trail = m.group('trail') or ''
        children = [t[1:].upper() for t in _re.findall(r'/[A-Z0-9/+\-]+', trail)]
        return parent, children, lat, lon
    ...
    def _f25_link_graph_path():
        return os.path.join(_f25_store_dir(), 'link_graph.json')

    def _f25_positions_path():
        return os.path.join(_f25_store_dir(), 'positions.json')

    def _f25_read_json_safe(path, fallback):
        try:
            if os.path.exists(path):
                with open(path, 'r', encoding='utf-8') as f:
                    return json.load(f)
    ...
    def _f25_write_json_safe(path, data):
        try:
            with open(path, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        except Exception:
    ...
    def _f25_update_link_graph(parent, children):
        # disabled: handled by unified graph updater
        return

    def _f25_update_positions(parent, lat, lon):
        if lat is None or lon is None:
            return
        p = _f25_positions_path()
        data = _f25_read_json_safe(p, {"version": 1, "positions": {}})
        ...
        _f25_write_json_safe(p, data)

    def _f25_update_heartbeat(self, parent, children):
        try:
            now_iso = _dt.utcnow().isoformat(timespec='seconds') + 'Z'
            hb = getattr(self, "beacon_heard", None)
            if not isinstance(hb, dict):
                hb = {}
            for call in [parent] + children:
                if not call: 
                    continue
                ent = hb.setdefault(call, {})
                ent["last_ts"] = now_iso
            self.beacon_heard = hb
            # optional UI refresh hook
            if hasattr(self, "_refresh_heartbeat_ui"):
                try: self._refresh_heartbeat_ui()
                except Exception: pass
        except Exception:
            pass

    # gate in the earliest RX hook: DO NOT show in messages
    _F25_ORIG_RX = getattr(ChatApp, "_on_serial_thread_line", None)
    def _F25_RX(self, line: str):
        s = "" if line is None else str(line)
        if _f25_is_beacon_line(s):
            parsed = _f25_parse_beacon(s)
            if parsed:
                parent, children, lat, lon = parsed
                try: _f25_update_heartbeat(self, parent, children)
                except Exception: pass
                try: self._rc_link_graph_update(parent, children)
                except Exception: pass
                try: _f25_update_positions(parent, lat, lon)
                except Exception: pass
                try: _f25_update_link_graph(parent, children)
                except Exception: pass
                try: _f25_update_positions(parent, lat, lon)
                except Exception: pass
            return  # never send to Messages
        if callable(_F25_ORIG_RX):
            return _F25_ORIG_RX(self, line)
    ChatApp._on_serial_thread_line = _F25_RX

    # also gate the byte-tap path from F17
    if "_f17_process_lines" in globals():
        _F25_ORIG_TAP = _f17_process_lines
        def _f17_process_lines(self_app, s):
            try:
                s = '' if s is None else str(s)
            except Exception:
            ...
            for itm in parts[:-1]:
                ...
                if not line:
                    continue
                if _f25_is_beacon_line(line):
                    parsed = _f25_parse_beacon(line)
                    if parsed:
                        parent, children, lat, lon = parsed
                        try: _f25_update_heartbeat(self_app, parent, children)
                        except Exception: pass
                        try: _f25_update_link_graph(parent, children)
                        except Exception: pass
                        try: _f25_update_positions(parent, lat, lon)
                        except Exception: pass
                    continue  # do not show in Messages
                # non-beacon lines flow to the pre-existing F17 path
                try:
                    _F25_ORIG_TAP(self_app, line + "\n")  # hand back chunk
                except Exception:
                    pass
            tail = parts[-1]
            if tail:
                self_app._f17_buf = tail
                self_app._f17_buf_ts = __import__('time').monotonic()
            else:
                self_app._f17_buf = ''
                self_app._f17_buf_ts = 0.0
        globals()['_f17_process_lines'] = _f17_process_lines

except Exception as _e_f25:
    try:
        diag_log(f"F25_BEACON_PIPE failed: {_e_f25}")
    except Exception:
        pass
...
# ======== F17_INSTANT_RX: tighter flush + immediate paint ========
try:
    import time as _t
    from PyQt5.QtCore import QTimer
    from PyQt5.QtWidgets import QApplication

    # 1) Tighten partial-line flush from 250ms -> 50ms
    if '_f17_timer_flush' in globals():
        _F17_ORIG_FLUSH = _f17_timer_flush
        def _f17_timer_flush(self_app):
            try:
                # Force a much faster flush if a tail is waiting
                if getattr(self_app, '_f17_buf', '') and (_t.monotonic() - getattr(self_app, '_f17_buf_ts', 0.0)) >= 0.05:
                    line = self_app._f17_buf
                    self_app._f17_buf = ''
                    self_app._f17_buf_ts = 0.0
                    line = str(line).strip()
                    if line and '_f17_save_append' in globals() and '_f17_ui_append' in globals():
                        _f17_save_append(self_app, 'received', line)
                        _f17_ui_append(self_app, 'received', line)
                else:
                    # fallback to the original logic for any other cases
                    _F17_ORIG_FLUSH(self_app)
            except Exception:
                # never crash the UI on flush
                pass
        globals()['_f17_timer_flush'] = _f17_timer_flush

    # 2) Speed up the polling/flush tick from 50ms -> 20ms
    _F17_ORIG_INIT_FOR_SPEED = getattr(ChatApp, '__init__', None)
    if callable(_F17_ORIG_INIT_FOR_SPEED):
        def _F17_INIT_SPEED(self, *a, **kw):
            _F17_ORIG_INIT_FOR_SPEED(self, *a, **kw)
            try:
                # If our F17 timer exists, retune it
                t = getattr(self, '_f17_timer', None)
                if isinstance(t, QTimer):
                    t.setInterval(20)
            except Exception:
                pass
        ChatApp.__init__ = _F17_INIT_SPEED

    # 3) After each UI append, force an immediate paint so the line appears now
    if '_f17_ui_append' in globals():
        _F17_ORIG_UI_APPEND = _f17_ui_append
        def _f17_ui_append(self_app, kind, text):
            try:
                r = _F17_ORIG_UI_APPEND(self_app, kind, text)
                try:
                    # Try to force a quick paint; small, safe pump
                    QApplication.processEvents()
                except Exception:
                    pass
                return r
                ...
                # fall back quietly
                return None
        globals()['_f17_ui_append'] = _f17_ui_append

except Exception as _e_inst_rx:
    try:
        diag_log(f"F17_INSTANT_RX patch failed: {_e_inst_rx}")
    except Exception:
        pass
# ======== End F17_INSTANT_RX ========