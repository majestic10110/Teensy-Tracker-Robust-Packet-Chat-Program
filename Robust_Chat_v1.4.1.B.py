...
# =====================
# Robust Chat v1.4.1.b — Alpha Release
# =====================
...
# ---- Link Graph persistence (store/link_graph.json) ----
def _lg_path():
    try:
        store = _rc_store()
    except Exception:
        ...
        try:
            ...
            pass
    return os.path.join(store, "link_graph.json")
...
def _lg_save(data):
    import json, os
    try:
        os.makedirs(os.path.dirname(_lg_path()), exist_ok=True)
        with open(_lg_path(), "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        return True
    except Exception:
        return False

def _lg_merge_parent_child(data, parent_base: str, child_base: str = None, snr: int = None):
    parent = (parent_base or "").strip().upper()
    if not parent:
        return data
    heard = data.setdefault("heard", {})
    node = heard.setdefault(parent, {})
    if child_base:
        ch = node.setdefault("children", {})
        child = (child_base or "").strip().upper()
        if child:
            entry = ch.setdefault(child, {})
...
def _lg_prune_with_hb(self, data):
    import time
    try:
        now = time.time()
        win = float(getattr(self, "hb_window_sec", getattr(self, "HEARTBEAT_TTL_S", 1800)))
        heard = data.get("heard", {})
        parents_seen = getattr(self, "hb_parent_seen", {}) or {}
        keep_parents = set([p.split("-",1)[0].upper() for p, ts in parents_seen.items() if (now - float(ts or 0)) <= win])
        hb_children = getattr(self, "hb_children", {}) or {}
        new_heard = {}
        for p, node in list(heard.items()):
            base_p = (p or "").split("-",1)[0].upper()
            fresh_children = {}
            for c_name, meta in (node.get("children") or {}).items():
                base_c = (c_name or "").split("-",1)[0].upper()
                ts = float(((hb_children.get(base_p, {}) or {}).get(base_c, 0)) or 0)
                if (now - ts) <= win:
                    fresh_children[base_c] = meta if isinstance(meta, dict) else {}
            if base_p in keep_parents or fresh_children:
                new_node = dict(node)
                if fresh_children:
                    new_node["children"] = fresh_children
                else:
                    new_node.pop("children", None)
                new_heard[base_p] = new_node
        data["heard"] = new_heard
    except Exception:
        pass
    return data

def _lg_save_now(self, touch_parent=None, touch_child=None, snr=None):
    try:
        data = _lg_load()
        try:
            ...
            my = ""
        if my:
            data["mycall"] = my.split("-",1)[0]
        if touch_parent:
            data = _lg_merge_parent_child(data, touch_parent, touch_child, snr)
        # Merge all current hb dicts
        for p in list((getattr(self, "hb_parent_seen", {}) or {}).keys()):
            data = _lg_merge_parent_child(data, p, None, None)
            for c in ((getattr(self, "hb_children", {}) or {}).get(p, {}) or {}).keys():
                data = _lg_merge_parent_child(data, p, c, None)
        data = _lg_prune_with_hb(self, data)
        _lg_save(data)
    except Exception:
        pass
# ---- End Link Graph persistence ----



def _lg_clear_now(self, also_memory=True):
    """Clear link_graph.json (preserving mycall) and optionally flush hb_* memory. Also posts a status message."""
    try:
        try:
            ui_my = (self.mycall_edit.text() or "").strip().upper().split("-", 1)[0]
        except Exception:
            ui_my = ""
        data = _lg_load()
        my = ui_my or (data.get("mycall") or "").strip().upper().split("-", 1)[0] or ""
        if also_memory:
            try:
                self.hb_parent_seen = {}
                self.hb_children = {}
                ...
                pass
        _lg_save({"mycall": my, "heard": {}})
        # status message
        try:
            import time as _t
            ts = _t.strftime("%H:%M:%S")
            self.statusBar().showMessage(f"[{ts}] Link map cleared (20m policy)", 6000)
        except Exception:
            pass
    except Exception:
        pass
...
BEACON_INTERVAL_MS = 10 * 60 * 1000  # 10 minutes
...
"""
LiNK500 Teensy Chat v1.4.1.b (Explicit Fleet Mode)  (2025-09-19)
...
class FleetManager:
    def __init__(self, base_dir: str):
        ...
        # Clear link graph immediately on startup so the map starts live/empty
        try:
            _lg_clear_now(self, also_memory=True)
        except Exception:
            pass
...
class ChatApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Robust Chat v1.4.1.b")
        ...
        self.beacon_check = QCheckBox("10 Min Beacon")
    ...
    def _toggle_beaconing(self, on: bool):
        if on:
            ...
            self._send_beacon()
            self.status_label.setText(f"Beaconing ON (10 min) | Connected {self.port_combo.currentText()}")
        else:
            self.beacon_timer.stop()
            self.status_label.setText(f"Connected {self.port_combo.currentText()}  |  Reminder: turn KISS OFF to chat")

    def _send_beacon(self):
        # Alternating beacon payload: odd -> `..MYCALL`, even -> `..MYCALL /child1 /child2 ...`
        try:
            self._beacon_counter += 1
        except AttributeError:
            self._beacon_counter = 1
        mycall = (getattr(self, 'mycall_input', None).text() if hasattr(self, 'mycall_input') else '').strip() or getattr(self, 'MYCALL', '').strip()
        heard = []
        try:
            if hasattr(self, 'heard_stations') and isinstance(self.heard_stations, (set, list)):
                heard = [c for c in list(self.heard_stations) if c and c != mycall]
            elif hasattr(self, 'link_graph') and isinstance(self.link_graph, dict):
                heard = [c for c in self.link_graph.keys() if c and c != mycall]
            elif hasattr(self, '_heard_set') and isinstance(self._heard_set, set):
                heard = [c for c in list(self._heard_set) if c and c != mycall]
        except Exception:
            heard = []
        heard = sorted(set(heard))[:20]
        if self._beacon_counter % 2 == 1 or not heard:
            beacon_str = '..' + mycall
        else:
            children = ' '.join('/' + c for c in heard)
            beacon_str = '..' + mycall + ' ' + children
        try:
            if hasattr(self, 'send_line'):
                self.send_line(beacon_str)
            elif hasattr(self, 'transmit'):
                self.transmit(beacon_str)
            else:
                self._serial_write(beacon_str + '\n')
        except Exception:
            pass
        ...
        if ok:
            ...
            suffix = " | Beaconing ON (10 min)" if self.beacon_check.isChecked() else ""
    ...
    def _on_line(self, line: str):
        ...
        if m:
            ...
            try:
                ...
                if _msg_upper.startswith("H*_DISABLED"):
                    ...
                    if my_base and reply_to:
                        ...
                        if ok:
                            ...
                            _lg_save_now(self, touch_parent=my_base)
                        ...
                        for c in children:
                            ...
                            if ok:
                                ...
                                _lg_save_now(self, touch_parent=my_base, touch_child=c)
                                self._heartbeat_update_view()
                        return
                ...
                if _msg_str.startswith('/') and '<' in _msg_str:
                    ...
                    bucket[child] = now_ts
                    _lg_save_now(self, touch_parent=parent, touch_child=child)
                    # prune by window
                    win = float(getattr(self, "hb_window_sec", getattr(self, "HEARTBEAT_TTL_S", 1800)))
            ...
            if action == "hide" and not (my and to == my) and to != "CQ": return
...
def main():
    print("Link500 Teensy Robust Chat v1.4.1.b (2025-09-19)")
...
# ======== AUTO-RELAY + AUTO-ACK bolt-on patch (no UI changes) ========
# This block safely augments ChatApp at runtime. Any error here is swallowed,
# and the original UI continues to work.
try:
    import os, sys, json, re, time, hashlib, random
    from types import MethodType
except Exception:
    pass

# Fallback regexes in case app globals differ
try:
    CALL_RE = r'[A-Z0-9]{1,2}\d[A-Z0-9]{1,3}(?:-[0-9]{1,2})?'
    LINE_RE_FALLBACK = re.compile(rf'^(?P<to>{CALL_RE}|CQ)\s+DE\s+(?P<frm>{CALL_RE})\s*(?P<msg>.*)$', re.I)
    ACK_TAG_RE_FALLBACK = re.compile(r'\[ACK:([0-9A-Z]{3,})\]', re.I)
except Exception:
    pass
...
def _lgp_load():
    ...
    try:
        with open(p, "r", encoding="utf-8") as f:
            d = json.load(f)
        if not isinstance(d, dict): return {"mycall": "", "heard": {}}
        d.setdefault("mycall",""); d.setdefault("heard",{})
        return d
    except Exception:
        return {"mycall": "", "heard": {}}
...
def _lgp_save(d):
    try:
        with open(_lgp_path(), "w", encoding="utf-8") as f:
            json.dump(d, f, ensure_ascii=False, indent=2)
        return True
    except Exception:
        return False

def _arp_patch_chatapp(cls):
    # --- small state maps (non-invasive) ---
    def _ensure_auto_maps(self):
        if not hasattr(self, "_relay_seen"): self._relay_seen = {}
        if not hasattr(self, "_relay_ttl"): self._relay_ttl = 120.0
        if not hasattr(self, "_ack_sent"): self._ack_sent = {}
        if not hasattr(self, "_ack_sent_ttl"): self._ack_sent_ttl = 600.0
        if not hasattr(self, "_outbox_by_ack"): self._outbox_by_ack = {}
        if not hasattr(self, "hb_parent_seen"): self.hb_parent_seen = {}
        if not hasattr(self, "hb_children"): self.hb_children = {}
    cls._ensure_auto_maps = _ensure_auto_maps

    def _mycall_base(self):
        try:
            c = (self.mycall_edit.text() or "").strip().upper()
            return c.split("-",1)[0]
        except Exception:
            return ""
    cls._mycall_base = _mycall_base

    def _is_parent_heard_now(self, base_to: str) -> bool:
        try:
            base = (base_to or "").split("-",1)[0].upper()
            if not base: return False
            now = time.time()
            # Prefer live heartbeats if the app manages them
            win = float(getattr(self, "hb_window_sec", getattr(self, "HEARTBEAT_TTL_S", 1200)))
            ts = float((getattr(self, "hb_parent_seen", {}) or {}).get(base, 0) or 0)
            if (now - ts) <= win:
                return True
            # Fallback to persisted link graph
            heard = (_lgp_load().get("heard") or {})
            return base in { (k or "").split("-",1)[0].upper() for k in heard.keys() }
        except Exception:
            return False
    cls._is_parent_heard_now = _is_parent_heard_now

    def _relay_should_forward(self, raw_line: str) -> bool:
        try:
            self._ensure_auto_maps()
            h = hashlib.sha1((raw_line or "").encode("utf-8","ignore")).hexdigest()
            nowm = time.monotonic()
            last = float((self._relay_seen or {}).get(h, 0) or 0)
            if (nowm - last) < float(getattr(self, "_relay_ttl", 120.0)):
                return False
            if len(self._relay_seen) > 512:
                cutoff = nowm - float(getattr(self, "_relay_ttl", 120.0))
                self._relay_seen = {k:v for k,v in self._relay_seen.items() if v >= cutoff}
            self._relay_seen[h] = nowm
            return True
        except Exception:
            return True
    cls._relay_should_forward = _relay_should_forward

    def _tx_ack(self, dest_call: str, ack_id: str) -> None:
        try:
            my = self._mycall_base()
            if not my or not dest_call or not ack_id: return
            line = f"{dest_call} DE {my} [ACK:{ack_id}]"
            if hasattr(self, "_ptt_guard") and hasattr(self, "_write_line"):
                self._ptt_guard(lambda: self._write_line(line))
        except Exception:
            pass
    cls._tx_ack = _tx_ack

    def _handle_inbound_ack(self, frm: str, ack_id: str) -> None:
        try:
            self._ensure_auto_maps()
            item = (self._outbox_by_ack or {}).get(str(ack_id))
            if not item: return
            try:
                tmr = item.get("timer")
                if tmr: tmr.stop()
            ...
            item["delivered"] = True
            try:
                if hasattr(self, "_update_ui_on_ack"):
                    self._update_ui_on_ack(item)
            except Exception: pass
            try:
                (self._outbox_by_ack or {}).pop(str(ack_id), None)
            except Exception: pass
        except Exception:
            pass
    cls._handle_inbound_ack = _handle_inbound_ack

    def _modern_beacon_parse_and_update(self, raw: str):
        # Only '..PARENT' and '/CHILD' formats
        try:
            s = (raw or "").strip()
            if not s: return False
            now = time.time()
            if s.startswith(".."):
                call = s[2:].strip().split()[0].upper().split("-",1)[0]
                if not call: return False
                self._ensure_auto_maps()
                self.hb_parent_seen[call] = now
                data = _lgp_load()
                heard = data.setdefault("heard", {})
                node = heard.setdefault(call, {"ts": now, "children": {}})
                node["ts"] = now
                _lgp_save(data)
                return True
            if s.startswith("/"):
                call = s[1:].strip().split()[0].upper().split("-",1)[0]
                if not call: return False
                self._ensure_auto_maps()
                # attach child under freshest parent we heard
                if getattr(self, "hb_parent_seen", {}):
                    parent = max(self.hb_parent_seen, key=lambda p: self.hb_parent_seen.get(p,0.0))
                    data = _lgp_load()
                    heard = data.setdefault("heard", {})
                    node = heard.setdefault(parent, {"ts": now, "children": {}})
                    node["ts"] = max(now, node.get("ts", 0))
                    node.setdefault("children", {}).setdefault(call, {})
                    _lgp_save(data)
                return True
            return False
        except Exception:
            return False
    cls._modern_beacon_parse_and_update = _modern_beacon_parse_and_update

    # ---- Wrap _on_line safely (never break original) ----
    old_on_line = getattr(cls, "_on_line", None)
    def _on_line(self, line):
        try:
            self._ensure_auto_maps()
            raw = (line if isinstance(line, str) else str(line or "")).strip("\\r\\n")
            # 1) consume modern beacons
            if self._modern_beacon_parse_and_update(raw):
                return
            # 2) message parsing
            LINE_RE_OBJ = globals().get("LINE_RE", None)
            ACK_TAG_RE_OBJ = globals().get("ACK_TAG_RE", None)
            if LINE_RE_OBJ is None: LINE_RE_OBJ = LINE_RE_FALLBACK
            if ACK_TAG_RE_OBJ is None:  ACK_TAG_RE_OBJ = ACK_TAG_RE_FALLBACK
            m = LINE_RE_OBJ.match(raw) if hasattr(LINE_RE_OBJ, "match") else LINE_RE_FALLBACK.match(raw)
            if m:
                to  = (m.group('to')  or '').upper()
                frm = (m.group('frm') or '').upper()
                body = (m.group('msg') or '').strip()
                ...
                # Bare ACK addressed to me — stop retries, don't print
                ack_m = ACK_TAG_RE_OBJ.search(body) if hasattr(ACK_TAG_RE_OBJ, "search") else ACK_TAG_RE_FALLBACK.search(body)
                if my and to == my and ack_m and body == f"[ACK:{ack_m.group(1)}]":
                    self._handle_inbound_ack(frm, ack_m.group(1))
                    return

                # To me with ACK tag — auto-ACK with small jitter
                if my and to == my and ack_m:
                    ack_id = ack_m.group(1)
                    key = f"{frm}|{ack_id}"
                    nowm = time.monotonic()
                    last = float((self._ack_sent or {}).get(key, 0) or 0)
                    if (nowm - last) > float(getattr(self, "_ack_sent_ttl", 600.0)):
                        self._ack_sent[key] = nowm
                        try:
                            from PyQt5.QtCore import QTimer
                            delay = random.randint(80, 250)
                            QTimer.singleShot(delay, lambda f=frm, a=ack_id: self._tx_ack(f, a))
                        except Exception:
                            self._tx_ack(frm, ack_id)
                    # then fall through to original display/logic

                # Not to me & not from me — relay if TO is a heard parent
                if my and to != my and frm != my:
                    to_base = to.split('-',1)[0] if to else ""
                    enable = True
                    try:
                        if hasattr(self, "auto_hb_relay_check"):
                            enable = bool(self.auto_hb_relay_check.isChecked())
                    except Exception:
                        enable = True
                    if enable and to_base and self._is_parent_heard_now(to_base):
                        if self._relay_should_forward(raw):
                            if hasattr(self, "_ptt_guard") and hasattr(self, "_write_line"):
                                self._ptt_guard(lambda: self._write_line(raw))
                                return  # do not print it, per spec
        except Exception:
            # swallow any errors in the patch layer
            pass

        # Delegate to original handler
        if old_on_line:
            return old_on_line(self, line)

    if old_on_line:
        cls._on_line = _on_line
    else:
        cls._on_line = _on_line

    return cls

# Patch ChatApp immediately if already defined
try:
    if "ChatApp" in globals() and isinstance(ChatApp, type):
        ChatApp = _arp_patch_chatapp(ChatApp)
except Exception:
    pass

# Also hook future class creation just in case
try:
    import builtins as _bi
    _orig_bc = getattr(_bi, "__build_class__", None)
    if _orig_bc:
        def _arp_bc(func, name, *a, **kw):
            cls = _orig_bc(func, name, *a, **kw)
            try:
                if name == "ChatApp":
                    cls = _arp_patch_chatapp(cls)
            finally:
                return cls
        _bi.__build_class__ = _arp_bc
except Exception:
    pass
# ======== END: AUTO-RELAY + AUTO-ACK bolt-on patch ========



# ======== UI STRETCH + MYCALL PERSIST (safe bolt-on) ========
try:
    import os, json
    from PyQt5 import QtCore, QtWidgets
    from PyQt5.QtWidgets import QGroupBox, QSizePolicy, QListWidget
except Exception:
    QGroupBox = None
    QSizePolicy = None
    QListWidget = None
...
def _settings_path():
    ...
    return os.path.join(store, "settings.json")
...
def _apply_ui_stretch(self):
    """Find 'Serial' and 'Fleet' group boxes and make them expand with the window.
       Also expand combos/buttons/list widgets inside."""
    try:
        if QGroupBox is None or QSizePolicy is None:
            return
        boxes = self.findChildren(QGroupBox) if hasattr(self, "findChildren") else []
        for gb in boxes:
            title = (gb.title() or "").strip().lower()
            if not title: 
                continue
            if "serial" in title or "fleet" in title:
                # Make the group box stretch
                gb.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
                lay = gb.layout()
                if lay is not None:
                    try:
                        # Encourage contents to expand
                        if hasattr(lay, "setContentsMargins"):
                            lay.setContentsMargins(8, 8, 8, 8)
                        if isinstance(lay, (QtWidgets.QVBoxLayout, QtWidgets.QHBoxLayout)):
                            lay.addStretch(1)
                        elif isinstance(lay, QtWidgets.QGridLayout):
                            # Make last row/col stretchy
                            try:
                                lay.setRowStretch(lay.rowCount()-1, 1)
                                lay.setColumnStretch(lay.columnCount()-1, 1)
                            except Exception:
                                pass
                    except Exception:
                        pass

                # Expand common children
                children = gb.findChildren(QtWidgets.QWidget)
                for w in children:
                    try:
                        if isinstance(w, (QtWidgets.QComboBox, QtWidgets.QPushButton, QtWidgets.QToolButton)):
        ...
        # Also, if main central layout exists, make it stretchy
        try:
...
def _apply_mycall_persist(self):
    """Load MYCALL on startup and persist on change (debounced)."""
    try:
        st = _settings_load()
        # Load
        try:
            if hasattr(self, "mycall_edit") and self.mycall_edit is not None:
                saved = (st.get("mycall") or "").strip()
                if saved:
                    self.mycall_edit.setText(saved)
        except Exception:
            pass
        ...
        try:
            if hasattr(self, "mycall_edit") and self.mycall_edit is not None:
                self._mycall_save_timer = QtCore.QTimer(self)
                self._mycall_save_timer.setSingleShot(True)
                self._mycall_save_timer.setInterval(600)  # 0.6s debounce
                def _save_now():
                    s = ""
                    try:
                        s = (self.mycall_edit.text() or "").strip().upper()
                    except Exception:
                        s = ""
                    d = _settings_load()
                    d["mycall"] = s
                    _settings_save(d)
                # Connect textChanged → start debounce
                try:
                    self.mycall_edit.textChanged.connect(lambda *_: self._mycall_save_timer.start())
                    self._mycall_save_timer.timeout.connect(_save_now)
                except Exception:
                    pass
        except Exception:
            pass
...
# Wrap ChatApp.__init__ to apply tweaks after UI is built
try:
    if "ChatApp" in globals() and isinstance(ChatApp, type):
        ...
        def _ui_init(self, *a, **kw):
            if _orig_init:
                _orig_init(self, *a, **kw)
            # After UI created, apply tweaks
            try: _apply_ui_stretch(self)
            except Exception: pass
            try: _apply_mycall_persist(self)
            except Exception: pass
        ChatApp.__init__ = _ui_init
except Exception:
    pass
# ======== END UI STRETCH + MYCALL PERSIST ========
