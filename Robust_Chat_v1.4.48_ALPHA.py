# =====================
# Robust Chat v1.4.48 — FileTX OK/PING + 500KB cap
# =====================
...
class ThemeManager:
    ...
    def apply(self, window, name: str):
        ...
        QGroupBox {{ background: {t['panel_bg']}; border: 1px solid {t['border']}; border-radius: 6px; margin-top: 12px; }}
...
class ChatApp(QMainWindow):
    ...
    def _persist_tx(self, obj: dict):
        try:
            _ts = obj.get('ts') or self._utc_now_iso()
    ...
    def _ack_update_ui(self, ack_id: str, new_text: str, status: str):
        ...
        if it is None:
            it = QListWidgetItem(new_text); it.setForeground(QBrush(self.COLOR_SENT)); it.setData(Qt.UserRole,'sent')
            import datetime as _dt
            ...
            it.setText(prefix + new_text)
        self.messages_list.scrollToTop()
        self.last_tx_label.setText('Last TX: just now')
    ...
    def __init__(self):
        super().__init__()
        self.setFont(QFont("VT323", 11))
        self.setWindowTitle("Link500 Teensy Robust Chat v1.4.48")
        ...
        self.last_tx_label = QLabel("Last TX: —"); bv.addWidget(self.last_tx_label)
        ...
        try:
            ...
            if enabled:
                ...
                QTimer.singleShot(180_000, lambda: self._enable_beacon(15, immediate=True))
        except Exception:
            try:
                QTimer.singleShot(180_000, lambda: self._enable_beacon(15, immediate=True))
            except Exception:
        ...
        try:
            ...
            pass
        # Global colors
        self.COLOR_SENT = QColor(0,255,0); self.COLOR_ACK = QColor(255,0,0); self.COLOR_RX = QColor(255,165,0)
...
# ---- Messages storage (store/messages_v1.json) ----
    ...
    def send_message(self):
        ...
        try:
            ...
            it = QListWidgetItem(self._fmt_disp_prefix(now_iso) + first)
            try:
                from PyQt5.QtGui import QBrush, QColor
                fg = getattr(self, 'COLOR_SENT', QColor(0,255,0))
    ...
    def save_fixed_gps(self):
        ...
        try:
            ...
            try:
                ...
                self._send_beacon()
                # last_ts already set to now; next coordinate append in ~30 minutes
            except Exception:
                pass
        except Exception:
            pass
