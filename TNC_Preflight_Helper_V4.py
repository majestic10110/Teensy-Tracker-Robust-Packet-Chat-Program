...
class PreflightWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("TNC Preflight Helper — Echo ON, TXDelay 450ms")
        self.resize(720, 460)
        self.ser = None
        ...
        self._step_texts = [
            ...
            "Monitor ON filtered (ESC M IUSC -MYCALL)",
            "Echo ON (ESC E1)",
    ...
    def _send_monitor_cmd(self):
        # Build 'M IUSC' with optional negative filter for own call (no space after '-')
        base = self.call_edit.text().strip().upper()
        if '-' in base:
            base = base.split('-')[0]
        base = ''.join(ch for ch in base if ch.isalnum())[:6]
        cmd = "M IUSC" + (f" -{base}" if base else "")
