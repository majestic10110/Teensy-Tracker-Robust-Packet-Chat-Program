...
import socket as _h_sock
import subprocess as _h_subp
...
class _H_Servers:
    ...
    def start(self):
        # 1) Best-effort port release on Windows (kill only safe processes)
        try:
        ...
        try:
            if not self._port_free(self.http_port):
                # likely our previous EXE; image will contain 'robust_chat'
                self._kill_port_on_windows(self.http_port, safe_names=('robust_chat', 'python'))
        except Exception:
            pass
        # 2) If still busy, scan small ranges for free ports
        try:
            if not self._port_free(self.pm_port):
        ...
        try:
            ...
            pass
        # 3) Launch pmtiles server (hidden console on Windows)
        if self.pm_proc is None:
            if _h_os.path.isfile(self.pmtiles_exe):
                ...
                try:
                    if self._port_free(self.pm_port):
                        ...
                        try:
                            ...
                            pass
                        self.pm_proc = _h_subp.Popen(cmd, stdout=_h_subp.DEVNULL, stderr=_h_subp.DEVNULL, **kw)
                except Exception:
                    self.pm_proc = None
        ...
        if self.http_thread is None:
            try:
                ...
                self.http_thread = None
        try:
            self.app.status_bar.showMessage(f'Offline map server on http://127.0.0.1:{self.http_port} (tiles {self.pm_port})', 5000)
        except Exception:
            pass
def stop(self):
        try:
...
# ===== File Transfer (Upload → META/OK/PART/END with PING/PONG probes) =====
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Robust Chat v1.4.71.E_ALPHA_MAPPING")
