...
class PreflightWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("TNC Preflight Helper v5 — Echo ON, TXDelay 450ms, Monitor US -MYCALL")
        self.resize(720, 460)
        self.ser = None
        ...
        self._step_texts = [
            ...
            "Set TXDelay 450 ms (ESC T 45)",
            "Set TX Tail 50 ms (ESC %N 5)",
            "Monitor ON (ESC M US -MYCALL) — APRS/UI only, suppress self",
            "Echo ON (ESC E1)",
    ...
    def _start_preflight(self):
        ...
        steps += [
            ...
            (lambda: self._send_cmd("T 45", esc=True), 6),
            (lambda: time.sleep(0.7), 7),
            # Monitoring and hygiene
            (lambda: self._send_monitor_cmd(), 8),
            (lambda: self._send_cmd("E1", esc=True), 9),
    ...
    def _send_monitor_cmd(self):
        # Monitor APRS/UI + Supervisory only; suppress own base call
        base = self.call_edit.text().strip().upper()
        if '-' in base:
            base = base.split('-')[0]
        base = ''.join(ch for ch in base if ch.isalnum())[:6]
        filt = f" -{base}" if base else ""
        cmd = f"M US{filt}"
