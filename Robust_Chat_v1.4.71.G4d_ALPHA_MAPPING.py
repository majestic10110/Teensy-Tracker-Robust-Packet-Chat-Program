# =====================
# Robust Chat v1.4.59 — FileTX OK/PING + 500KB cap
# =====================
...
def diag_log(msg):
    try:
        _ensure_store_dir()
        with open('store/diagnostic.txt', 'a', encoding='utf-8') as _f:
            _f.write(f"[{_dt.datetime.now(_dt.timezone.utc).replace(microsecond=0).isoformat()}Z] {msg}\n")
    except Exception: pass
def _install_excepthook():
    def _hook(exc_type, exc, tb):
        try:
            _ensure_store_dir()
            with open('store/diagnostic.txt', 'a', encoding='utf-8') as _f:
                _f.write(f"[{_dt.datetime.now(_dt.timezone.utc).replace(microsecond=0).isoformat()}Z] UNCAUGHT {exc_type.__name__}: {exc}\n")
                _tb.print_tb(tb, file=_f)
        except Exception: pass
        try: _sys.__excepthook__(exc_type, exc, tb)
        except Exception: pass
...
import os
import warnings as _rc_warnings
# Silence PyQt/SIP spam unless user opts in via env var
if not os.environ.get("RC_SHOW_WARNINGS"):
    try:
        _rc_warnings.filterwarnings("ignore",
                                    message=r".*sipPyTypeDict\(\).*",
                                    category=DeprecationWarning)
        _rc_warnings.filterwarnings("ignore", category=DeprecationWarning)
    except Exception:
        pass
...
from PyQt5.QtWidgets import (
    ...
    QGraphicsScene, QCheckBox, QButtonGroup
)
...
# ===== HARDWIRED OFFLINE MAP (no external imports needed) =====
# Provides: _install_offline_map_embedded(app)
# Behavior:
#  - Uses PyQt5 WebEngine if available to embed the map tab.
#  - If not, shows buttons: "Open in Browser" and (if PyQt6 + WebEngine installed) "Open in PyQt6 Viewer".
#  - Starts pmtiles.exe (dir mode) on 8080 and a static server on 8787 serving store/maps/
...
from PyQt5.QtWidgets import QWidget as _H_QWidget, QVBoxLayout as _H_QVBox, QLabel as _H_QLabel, QPushButton as _H_QPushButton, QHBoxLayout as _H_QHBox
from PyQt5.QtGui import QDesktopServices as _H_Desk

try:
    from PyQt5.QtWebEngineWidgets import QWebEngineView as _H_WebView
    _H_WEB_OK = True
except Exception:
    _H_WebView = None
    _H_WEB_OK = False

def _H_app_base_dir(app):
    try:
        if hasattr(app, 'app_base_dir'):
            return app.app_base_dir()
    except Exception:
        pass
    try:
        return _h_os.path.dirname(_h_os.path.abspath(app.__file__))
    except Exception:
        return _h_os.getcwd()

def _H_maps_dir(app):
    return _h_os.path.join(_H_app_base_dir(app), 'store', 'maps')

def _H_settings_json(app):
    try:
        base = _H_app_base_dir(app)
        return _h_os.path.join(base, 'store', 'settings.json')
    except Exception:
        return _h_os.path.join(_h_os.getcwd(), 'store', 'settings.json')

def _H_pmtiles_path(app):
    # 1) settings.json -> key 'pmtiles_exe'
    try:
        sj = _H_settings_json(app)
        if _h_os.path.isfile(sj):
            import json as _h_json
            with open(sj,'r',encoding='utf-8') as f:
                st = _h_json.load(f)
            p = st.get('pmtiles_exe')
            if p and _h_os.path.isfile(p):
                return p
    except Exception:
        pass
    # 2) default to Downloads
    try:
        dl = _h_os.path.join(_h_os.path.expanduser('~'),'Downloads','pmtiles.exe')
        if _h_os.path.isfile(dl): return dl
    except Exception:
        pass
    # 3) alongside app
    cand = _h_os.path.join(_H_app_base_dir(app),'pmtiles.exe')
    return cand

class _H_HTTP(_h_th.Thread):
...
class _H_Servers:
    def __init__(self, app, maps_dir, pmtiles_exe=None, pm_port=8080, http_port=9797):
        self.app = app
        self.maps_dir = maps_dir or os.path.join(app_base_dir(), 'store', 'maps')
        self.pmtiles_exe = pmtiles_exe or _H_pmtiles_path(app)
        self.pm_port = pm_port
        self.http_port = http_port
        self.pm_proc = None
        self.http_thread = None
    def _port_free(self, port):
        with _h_sock.socket(_h_sock.AF_INET, _h_sock.SOCK_STREAM) as s:
            return s.connect_ex(("127.0.0.1", port)) != 0
    def start(self):
        # 1) Best-effort port release on Windows (safe image names only)
        try:
            if not self._port_free(self.pm_port):
                self._kill_port_on_windows(self.pm_port, safe_names=('pmtiles',))
        except Exception:
            pass
        try:
            if not self._port_free(self.http_port):
                self._kill_port_on_windows(self.http_port, safe_names=('robust_chat','python'))
        except Exception:
            pass

        # 2) If still busy, scan small ranges for a free port
        try:
            if not self._port_free(self.pm_port):
                self.pm_port = self._find_free_port(8080, 8090)
        except Exception:
            pass
        try:
            if not self._port_free(self.http_port):
                self.http_port = self._find_free_port(9797, 9810)
        except Exception:
            pass

        # 3) Launch pmtiles (hidden console on Windows)
        if self.pm_proc is None:
            if os.path.isfile(self.pmtiles_exe):
                import subprocess as _sp, os as _os
                cmd = [self.pmtiles_exe, 'serve', '--port', str(self.pm_port), '--cors', '*', self.maps_dir]
                try:
                    if self._port_free(self.pm_port):
                        kw = {}
                        try:
                            if _os.name == 'nt':
                            ...
                            pass
                        self.pm_proc = _sp.Popen(cmd, stdout=_sp.DEVNULL, stderr=_sp.DEVNULL, **kw)
                except Exception:
                    self.pm_proc = None
            else:
                try:
                    self.app.status_bar.showMessage(f'pmtiles.exe not found: {self.pmtiles_exe}')
                except Exception:
                    pass
        ...
        if self.http_thread is None:
            try:
                import os as _os
                if not _os.path.isdir(self.maps_dir):
                    self.maps_dir = _os.path.join(app_base_dir(), 'store', 'maps')
                self.http_thread = _H_HTTP(self.maps_dir, self.http_port)
                self.http_thread.start()
            except Exception:
                self.http_thread = None

        # Status message and, if embedded pane exists, refresh URL
        try:
            self.app.status_bar.showMessage(f'Offline map: dir={self.maps_dir}  URL=http://127.0.0.1:{self.http_port}/map_pmserve_markers.html  (tiles {self.pm_port})', 5000)
            if hasattr(self.app, '_offline_map_pane') and getattr(self.app._offline_map_pane, 'servers', None) is self:
                self.app._offline_map_pane.url = f'http://127.0.0.1:{self.http_port}/map_pmserve_markers.html'
                if getattr(self.app._offline_map_pane, 'view', None) is not None:
                    self.app._offline_map_pane.view.setUrl(_H_QUrl(self.app._offline_map_pane.url))
        except Exception:
            pass
    def stop(self):
        try:
            if self.http_thread: self.http_thread.stop()
        finally:
            self.http_thread = None
        try:
            if self.pm_proc: self.pm_proc.terminate()
        finally:
            self.pm_proc = None

class _H_OfflinePane(_H_QWidget):
    def __init__(self, app, servers:_H_Servers, parent=None):
        super().__init__(parent)
        self.app = app
        self.servers = servers
        self.url = f"http://127.0.0.1:{servers.http_port}/map_pmserve_markers.html"
        v = _H_QVBox(self)
        # --- Always show an "Open in Browser" button (dual-monitor friendly) ---
        header = _H_QHBox()
        open_btn = _H_QPushButton("Open in Browser")
        def _open_browser():
            self.servers.start()
            try:
                _H_Desk.openUrl(_H_QUrl(self.url))
            except Exception:
                try:
                    ...
                    pass
        open_btn.clicked.connect(_open_browser)
        header.addWidget(open_btn)
        header.addStretch(1)
        v.addLayout(header)

        if _H_WEB_OK and _H_WebView is not None:
            self.view = _H_WebView(self)
            v.addWidget(self.view, 1)
        else:
            self.view = None
            row = _H_QHBox()
            msg = _H_QLabel("Embedded WebEngine not available. Choose:")
            row.addWidget(msg); row.addStretch(1)
            btn_b = _H_QPushButton("Open in Browser")
            def _open_b():
                self.servers.start()
                try: _H_Desk.openUrl(_H_QUrl(self.url))
                except Exception: _h_web.open(self.url)
            btn_b.clicked.connect(_open_b)
            row.addWidget(btn_b)
            # PyQt6 viewer option
            show_pyqt6 = False
            try:
                ...
                show_pyqt6 = False
            if show_pyqt6:
                btn6 = _H_QPushButton("Open in PyQt6 Viewer")
                def _open6():
                    self.servers.start()
                    exe = _h_sys.executable
                    viewer = _h_os.path.join(_H_app_base_dir(self.app), "offline_map_viewer_pyqt6.py")
                    if not _h_os.path.isfile(viewer):
                        viewer = _h_os.path.join(_h_os.path.dirname(__file__), "offline_map_viewer_pyqt6.py")
                    _h_subp.Popen([exe, viewer, self.url])
                btn6.clicked.connect(_open6)
                row.addWidget(btn6)
            v.addLayout(row)
    def showEvent(self, e):
        ...
        self.servers.start()
        try:
            self.url = f'http://127.0.0.1:{self.servers.http_port}/map_pmserve_markers.html'
        except Exception:
            pass
        if self.view is not None:
            try: self.view.setUrl(_H_QUrl(self.url))
            except Exception: pass

def _install_offline_map_embedded(app):
    has_tab = hasattr(app, 'offline_map_tab')
    # patched to mount into pre-created Offline Map tab when available
    has_tab = hasattr(app, 'offline_map_tab')
    maps_dir = _H_maps_dir(app)
    _h_os.makedirs(maps_dir, exist_ok=True)
    servers = _H_Servers(app, maps_dir, http_port=9797)
    pane = _H_OfflinePane(app, servers)
    added = False
    if has_tab:
        try:
            # ensure layout exists
            from PyQt5.QtWidgets import QVBoxLayout
            if app.offline_map_tab.layout() is None:
                app.offline_map_tab.setLayout(QVBoxLayout())
            app.offline_map_tab.layout().addWidget(pane)
            added = True
        except Exception:
            added = False
    if not added:
        try:
            app.tab_widget.addTab(pane, "Offline Map")
            added = True
        except Exception:
            pass
    if added:
        app._offline_map_servers = servers
        app._offline_map_pane = pane
    # Step E: ensure positions GeoJSON exists for the Offline Map (best-effort)
    try:
        if hasattr(app, '_mirror_positions_to_geojson'):
            app._mirror_positions_to_geojson()
    except Exception:
        pass
    old_close = getattr(app, 'closeEvent', None)
    def _patched(ev):
        try: servers.stop()
        finally:
            if callable(old_close): old_close(ev)
    try: app.closeEvent = _patched
    except Exception: pass
    try: app.status_bar.showMessage("Offline Map ready â€” open the tab to view.")
    except Exception: pass
# ===== END HARDWIRED OFFLINE MAP =====

# ===== Offline Map Dock (slide-in) =====
def _install_offline_map_dock(app):
    """Create a slide-in/out dock on the right that shows the Offline Map pane,
    leaving the main left-pane (Beacons Heard, etc.) visible. Also adds a toolbar
    toggle action 'Map Drawer'. Safe if called multiple times."""
    try:
        from PyQt5.QtWidgets import QDockWidget, QAction, QToolBar
        from PyQt5.QtCore import Qt

        # Reuse or create servers + pane
        maps_dir = _H_maps_dir(app)
        try: _h_os.makedirs(maps_dir, exist_ok=True)
        except Exception: pass
        servers = getattr(app, '_offline_map_servers', None)
        if servers is None:
            servers = _H_Servers(app, maps_dir, http_port=9797)
        pane = _H_OfflinePane(app, servers)

        # Build dock
        dock = QDockWidget("Offline Map", app)
        dock.setObjectName("OfflineMapDock")
        ...
        # Wire to app
        app._offline_map_dock = dock
        app._offline_map_servers = servers
        app._offline_map_pane_dock = pane

        # Toolbar action
        try:
            # Try to reuse an existing toolbar; otherwise, create a small one
            tb = None
            for w in app.findChildren(QToolBar):
                tb = w
                break
            if tb is None:
                tb = QToolBar("Main Toolbar", app)
                app.addToolBar(tb)
            act = QAction("Map Drawer", app)
            act.setCheckable(True)
            def _sync_action(visible):
                try: act.setChecked(bool(visible))
                except Exception: pass
            def _toggle():
                try:
                    if dock.isVisible():
                        ...
                        dock.show()
                        # Ensure content is live
                        servers.start()
                except Exception:
                    pass
                _sync_action(dock.isVisible())
        ...
        # Optional: status
        try:
            app.status_bar.showMessage("Offline Map Drawer ready (View → Map Drawer)", 4000)
        except Exception:
            pass
    except Exception:
        pass
...
class ChatApp(QMainWindow):
    # --- Map visibility control (hide everywhere except Offline Map) ---
    def _set_offline_map_visible(self, show: bool):
        try:
            w = getattr(self, 'offmap_view', None)
            c = getattr(self, 'offmap_left', None)
            d = getattr(self, '_offline_map_dock', None)
            p = getattr(self, '_offline_map_pane_dock', None)
            for obj in (w, c, d, p):
                if obj is not None:
                    obj.setVisible(bool(show))
        except Exception:
            pass
...
# ===== File Transfer (Upload → META/OK/PART/END with PING/PONG probes) =====
    ...
    def _save_positions(self):
        try:
            save_json('positions.json', getattr(self, '_positions', {}))  # existing behavior
            try:
                self._mirror_positions_to_geojson()  # safe mirror for offline map
            except Exception:
                # Never allow map mirroring to break normal operation
                pass
        except Exception:
            pass
    ...
    def _mirror_positions_to_geojson(self):
        """Best-effort mirror of store/positions.json -> store/maps/positions.geojson.
        Never raises; silently ignores errors to avoid breaking chat/beacon logic.
        """
        try:
            import os, json, datetime
            base = app_base_dir()
            maps_dir = os.path.join(base, "store", "maps")
            os.makedirs(maps_dir, exist_ok=True)
            pos = getattr(self, "_positions", {}) or {}
            # Step F: also include fixed coords from settings for MYCALL if available
            try:
                myc = (self.mycall_edit.text().strip().upper() if hasattr(self, 'mycall_edit') else '')
            except Exception:
                myc = ''
            try:
                sset = self._settings_get()
                b = sset.get('beacon', {})
                c = b.get('coords', {})
                if myc and c.get('enabled') and 'lat' in c and 'lon' in c and myc not in pos:
                    pos[myc] = {
                        'lat': float(c.get('lat')),
                        'lon': float(c.get('lon')),
            ...
            for cs, e in pos.items():
                try:
                    ...
                    continue
                last = e.get("last_heard") or e.get("last_update") or now_iso
                features.append({
                    "type": "Feature",
                    ...
                    "properties": {
                        ...
                        "lat": lat,
                        "lon": lon,
                        "last_heard": (last if isinstance(last,str) else datetime.datetime.utcfromtimestamp(float(last)).replace(microsecond=0).isoformat()+"Z"),
                        "last_update": (e.get("last_update") if isinstance(e.get("last_update"), str) else (datetime.datetime.utcfromtimestamp(float(e.get("last_update"))).replace(microsecond=0).isoformat()+"Z" if e.get("last_update") else None)),
                        "source": e.get("source"),
                        "history": e.get("history", []),
                    },
                })
            fc = {"type": "FeatureCollection", "features": features, "generated_at": now_iso, "version": "1.4.70"}
            # Write both plural and singular filenames (HTMLs may expect either)
            for fname in ("positions.geojson", "position.geojson"):
                out_path = os.path.join(maps_dir, fname)
                with open(out_path, "w", encoding="utf-8") as f:
                    json.dump(fc, f, ensure_ascii=False)
            # Step C: status + diag
            try:
                self._status(f"Mirrored {len(features)} positions → maps/(positions|position).geojson", 2500)
            except Exception:
                pass
            try:
                diag_log(f"mirror_geojson: wrote {len(features)} features at {now_iso}")
            except Exception:
                pass
        except Exception:
            pass
    ...
    def _on_serial_thread_line(self, line: str):
        ...
        if m:
            aid = m.group(1).upper()
            st = self._ack_states.get(aid)
            if st and (norm in st.echo_texts):
                self._diag(f"[RX] (ignored echo for {aid}) {norm}")
                return
            if st:
                st.on_ack_received()
    ...
    def __init__(self):
        ...
        self.setWindowTitle(f"Link500 Teensy Robust Chat v{VERSION}_ALPHA_MAPPING")
        ...
        self.beacon_5_btn.setCheckable(True)
        self.beacon_5_btn.clicked.connect(lambda: self.set_beacon_interval(5))
        self.beacon_10_btn.setCheckable(True)
        self.beacon_10_btn.clicked.connect(lambda: self.set_beacon_interval(10))
        self.beacon_15_btn.setCheckable(True)
        self.beacon_15_btn.clicked.connect(lambda: self.set_beacon_interval(15))
        self.beacon_btn_group = QButtonGroup(self)
        self.beacon_btn_group.setExclusive(True)
        self.beacon_btn_group.addButton(self.beacon_5_btn)
        self.beacon_btn_group.addButton(self.beacon_10_btn)
        self.beacon_btn_group.addButton(self.beacon_15_btn)
        bl.addWidget(self.beacon_5_btn); bl.addWidget(self.beacon_10_btn); bl.addWidget(self.beacon_15_btn); bl.addStretch()
        
        # Ensure checked beacon buttons show the theme-green background
        try:
            self.beacon_5_btn.setStyleSheet(self.beacon_5_btn.styleSheet() + '\nQPushButton:checked{background-color:#00FF00;color:#000;}')
            self.beacon_10_btn.setStyleSheet(self.beacon_10_btn.styleSheet() + '\nQPushButton:checked{background-color:#00FF00;color:#000;}')
            self.beacon_15_btn.setStyleSheet(self.beacon_15_btn.styleSheet() + '\nQPushButton:checked{background-color:#00FF00;color:#000;}')
        except Exception:
            pass
        ...
        # Offline map tab (selector only; content shown in left-pane view)
        self.offline_map_tab = QWidget()
        self.tab_widget.addTab(self.offline_map_tab, "Offline Map")
        try:
        ...
        self._build_offline_map_left_view()
        self._show_main_view()
        # Install Offline Map pane into Offline Map tab
        try:
            None  # (offline map embed suppressed)  # (legacy embed; no-op if absent)
        except Exception as e:
            pass
    ...
    def _build_offline_map_left_view(self):
        """Build a left-pane view for the Offline Map, mirroring how Network Map uses the left pane."""
        try:
            self.offmap_left = QWidget(); self.left_stack.addWidget(self.offmap_left)
            v = QVBoxLayout(self.offmap_left)
            header = QHBoxLayout()
            open_btn = QPushButton("Open in Browser"); header.addWidget(open_btn); header.addStretch(1)
            v.addLayout(header)
            maps_dir = _H_maps_dir(self)
            try: _h_os.makedirs(maps_dir, exist_ok=True)
            except Exception: pass
            self._offline_map_servers = getattr(self, '_offline_map_servers', _H_Servers(self, maps_dir, http_port=9797))
            self._offline_map_url = f"http://127.0.0.1:{self._offline_map_servers.http_port}/map_pmserve_markers.html"
            def _open_browser():
                try:
                    self._offline_map_servers.start()
                    _H_Desk.openUrl(_H_QUrl(self._offline_map_url))
                except Exception:
                    try:
                        import webbrowser as _wb; _wb.open(self._offline_map_url)
                    except Exception: pass
            open_btn.clicked.connect(_open_browser)
            self.offmap_view = None
            try:
                from PyQt5.QtWebEngineWidgets import QWebEngineView as _H_WebView
                self.offmap_view = _H_WebView(self.offmap_left)
                v.addWidget(self.offmap_view, 1)
                try:
                    self._offline_map_servers.start()
                    self.offmap_view.setUrl(_H_QUrl(self._offline_map_url))
                except Exception: pass
            except Exception:
                note = QLabel("Embedded WebEngine not available. Use 'Open in Browser'."); v.addWidget(note, 1)
            self.offmap_left.hide()
        except Exception as e:
            try: self._status(f"Offline Map pane error: {e}", 5000)
            except Exception: pass

    def _show_offline_map_view(self):
        try:
            self._offline_map_servers.start()
            if getattr(self, 'offmap_view', None) is not None:
                try: self.offmap_view.setUrl(_H_QUrl(self._offline_map_url))
                except Exception: pass
        except Exception: pass
        try: self.offmap_left.show()
        except Exception: pass
        try: self.map_left.hide()
        except Exception: pass
        try: self.main_left.hide()
        except Exception: pass
...
# ---- Messages storage (store/messages_v1.json) ----
    ...
    def on_toolbar_tab_changed(self, idx):
        ...
        if title == "Offline Map":
            try:
                self._set_offline_map_visible(True)
            except Exception:
                pass
            try:
                self._show_offline_map_view()
            except Exception:
                pass
        else:
            try:
                self._set_offline_map_visible(False)
            except Exception:
                pass
            try:
    ...
    try:
        ...
        try:
            ...
            QMessageBox.critical(None, "Startup error",
                                 "An error occurred during startup. A log was written to startup_error.log.\n\n%s" % e)
        except Exception:
            pass
