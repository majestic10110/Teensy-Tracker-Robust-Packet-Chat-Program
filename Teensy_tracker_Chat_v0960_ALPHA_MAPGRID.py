...
# -*- coding: utf-8 -*-
"""
Teensy Tracker Chat v0.9.60 (Region tabs & zooms; World/Europe incl. UK+IE/US/N. America/S. America/UK+IE)
- Pure black MAP GRID background; lime grid; cyan labels; zoom buttons; world/europe/us + N.Am/S.Am/UK+IE presets
...
class GridMapView(QtWidgets.QGraphicsView):
    ...
    # Helper to build a view rect from bounds (lat_min..lat_max, lon_min..lon_max)
    def rect_from_bounds(self, lat_min, lat_max, lon_min, lon_max) -> QRectF:
        x = lon_min * self.scale_deg
        y = -lat_max * self.scale_deg  # top-left y
        w = (lon_max - lon_min) * self.scale_deg
        h = (lat_max - lat_min) * self.scale_deg
    ...
    def zoom_europe(self):
        # Europe (incl. UK & Ireland): lat 35..71, lon -11..40
        r = self.rect_from_bounds(35, 71, -11, 40)
        self.fitInView(r, Qt.KeepAspectRatio)

    def zoom_us(self):
        # Continental US: lat 24..49, lon -125..-66
        r = self.rect_from_bounds(24, 49, -125, -66)
        self.fitInView(r, Qt.KeepAspectRatio)

    def zoom_namerica(self):
        # North America: lat 7..83, lon -168..-52
        r = self.rect_from_bounds(7, 83, -168, -52)
        self.fitInView(r, Qt.KeepAspectRatio)

    def zoom_samerica(self):
        # South America: lat -56..13, lon -82..-34
        r = self.rect_from_bounds(-56, 13, -82, -34)
        self.fitInView(r, Qt.KeepAspectRatio)

    def zoom_ukireland(self):
        # UK & Ireland: lat 49..61, lon -11..2
        r = self.rect_from_bounds(49, 61, -11, 2)
        self.fitInView(r, Qt.KeepAspectRatio)
...
class ChatApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Teensy Tracker Robust Packet Chat v0.9.60")
    ...
    def _build_ui(self):
        ...
        btn_nam = QPushButton("N. America"); btn_nam.clicked.connect(self.map_widget.zoom_namerica)
        btn_sam = QPushButton("S. America"); btn_sam.clicked.connect(self.map_widget.zoom_samerica)
        btn_ukie = QPushButton("UK+Ireland"); btn_ukie.clicked.connect(self.map_widget.zoom_ukireland)
        tb.addWidget(btn_world); tb.addWidget(btn_eu); tb.addWidget(btn_us)
        tb.addWidget(btn_nam); tb.addWidget(btn_sam); tb.addWidget(btn_ukie)
        tb.addStretch(1)
...
def main():
    print("Teensy Tracker Chat v0.9.60 (Region tabs & zooms)")
