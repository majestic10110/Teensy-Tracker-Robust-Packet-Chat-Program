...
"""
Teensy Tracker Chat v1.2.6 BETA (2025-09-09)
...
- Received messages appear at the top of the receive window (inverted scroll).
- Sent and received messages are prefixed with [HH:MM:SS] timestamps.
- From callsigns in received messages are outlined and clickable to populate the To field (using QTextBrowser with anchorClicked signal).
- Fixed missing QTextEdit import for heartbeat_text widget.
- Added PTT guard to load_mycall to prevent unintended transmission (removed in this version).
- Removed Load Callsign button and functionality; MyCALL must now be entered manually.
...
class ChatApp(QMainWindow):
    def __init__(self):
        ...
        self.recv_text.setOpenLinks(False)  # Disable default link handling
    ...
    def _append_chat_line(self, text: str, kind: str):
        ...
        # Check if the message matches the LINE_RE pattern for received messages
        if kind == "received" and LINE_RE.match(text):
            m = LINE_RE.match(text)
            to = m.group('to')
            frm = m.group('frm')
            msg = m.group('msg') or ''
            # Wrap the 'frm' callsign in an <a> tag with an outline
            frm_link = f'<a href="callsign:{frm}" style="color:{COLOR_RECV}; text-decoration:none; border:1px solid {COLOR_RECV}; padding:2px; border-radius:3px;">{frm}</a>'
            text_esc = f"{to} DE {frm_link}" + (f" {msg.replace('&','&amp;').replace('<','&lt;').replace('>','&gt;')}" if msg else "")

        html = f'<span style="color:{color}; font-family:VT323,monospace">[{timestamp}] {text_esc}</span><br>'
    ...
    def _handle_callsign_click(self, url: QUrl):
        """Handle clicks on callsign links in received messages."""
        link = url.toString()
