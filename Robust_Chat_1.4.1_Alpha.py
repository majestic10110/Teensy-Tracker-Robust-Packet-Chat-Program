...
# =====================
# Robust Chat v1.4.1 experimental relay — Production Persistence Pack
# =====================
...
"""
LiNK500 Teensy Chat v1.4.1 experimental relay Consolidated (Explicit Fleet Mode)  (2025-09-17)
...
class ChatApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Robust Chat v1.4.1 experimental relay")
...
def main():
    print("Link500 Teensy Robust Chat v1.4.1 experimental relay (2025-09-15)")
...
# =====================
# Robust Chat v1.4.1 experimental relay — Autosave + Fleet Manager width cap (pre‑main)
...
try:
    ...
    def __rc_write(path, records, fmt):
        try:
            ...
            print(f"[v1.4.1 experimental relay write] {e!r}")
    ...
    def __rc_clear_disk_messages(self):
        ...
        try:
            ...
            print(f"[v1.4.1 experimental relay clear disk] {e!r}")
...
if "ChatApp" in globals() and hasattr(ChatApp, "__init__"):
    ...
    def __rc_patched_init(self, *a, **kw):
        ...
        try:
            ...
            print(f"[v1.4.1 experimental relay menu] {e!r}")
        ...
        try:
            ...
            print(f"[v1.4.1 experimental relay autosave] {e!r}")
        ...
        try:
            ...
            def _shrink_fleet():
                try:
                    ...
                    print(f"[v1.4.1 experimental relay fleet shrink] {e!r}")
            ...
            try:
                ...
                print(f"[v1.4.1 experimental relay serial cap] {e!r}")
            _t.start()
            self._fleet_shrink_timer = _t
        except Exception as e:
            print(f"[v1.4.1 experimental relay fleet shrink init] {e!r}")
        ...
        try:
            ...
            def _shrink_boxes():
                try:
                    ...
                    print(f"[v1.4.1 experimental relay shrink boxes] {e!r}")
            _t2 = QTimer(self)
            ...
            print(f"[v1.4.1 experimental relay shrink init] {e!r}")
        ...
        try:
            ...
            def _shrink_specific_groups():
                try:
                    ...
                    print(f"[v1.4.1 experimental relay compact groups] {e!r}")
            ...
            print(f"[v1.4.1 experimental relay ui compact init] {e!r}")
    ...
    if hasattr(ChatApp, "_clear_receive_window"):
        ...
        def __rc_patched_clear(self):
            ...
            try:
                ...
                print(f"[v1.4.1 experimental relay clear hook] {e!r}")
...
# =====================
# Data Files Self‑Check (v1.4.1 experimental relay)
...
# =====================
# Persistence CORE v1.4.1 experimental relay (canonical & idempotent)
...
# =====================
# Persistence Diagnostics v1.4.1 experimental relay-dbg
...
# =====================
# Schema Compatibility v1.4.1 experimental relay
...
try:
    ...
    # Monkey-patch the core functions if present from v1.4.1 experimental relay
...
# =====================
# LAST-WINS STARTUP HOOK (v1.4.1 experimental relay fixfinal)
...
# =====================
# TOP TOOLBAR ENFORCER (v1.4.1 experimental relay toolbar)
...
# =====================
# UNIVERSAL MENU + TOOLBAR ENFORCER (v1.4.1 experimental relay toolbar2)
...
# =====================
# AUTOSAVE POLICY: single 5‑min timer, NO initial 10‑second save (v1.4.1 experimental relay nokick)
# =====================
try:
    from PyQt5.QtCore import QTimer
...
# =====================
# MINIMAL CLEAN PATCH (v1.4.1 experimental relay minimal_clean)
...
# =====================
# v1.4.1 experimental relay autosave post-init fix (safe override)
# =====================
try:
    from PyQt5.QtCore import QTimer
except Exception:
...
# =====================
# BEACON FILTER (v1.4.1 experimental relay)
# =====================
...
# =====================
# Patch ChatApp._rc_render_ui (installed by ChatGPT on 2025-09-17)
# =====================
# Renders newest-first items into either a QListWidget (preferred) or a text editor fallback.
# - Strips [HOP:x/2] tags for display clarity
# - Shows timestamps, KIND, who, message text, and optional (ACK:ID)
def __patched_rc_render_ui(self, items_newest_first):
    try:
        from PyQt5.QtWidgets import QListWidget, QListWidgetItem, QPlainTextEdit, QTextEdit
    except Exception:
        return
    try:
        ...
        if lst is not None:
            ...
            for it in items_newest_first:
                ...
                text = it.get("text") or ""
                ack = it.get("ack_id") or ""
                # Strip hop tags for display
                import re as _re
                display_text = _re.sub(r'\s*\[HOP:\d+/2\]', '', text)
                line = f"[{ts_str}] {kind.upper()} {who}: {display_text}"
                if ack:
                    line += f" (ACK:{ack})"
                try:
                    lst.addItem(QListWidgetItem(line))
...
# Bind the patch to ChatApp if available
try:
    ChatApp._rc_render_ui = __patched_rc_render_ui
except NameError:
    # If ChatApp isn't defined yet (unusual in this file), defer binding at import-time
    try:
        import builtins as _bi
        _orig_bc = getattr(_bi, "__build_class__", None)
        def _hook(func, name, *a, **k):
            cls = _orig_bc(func, name, *a, **k)
            try:
                if name == "ChatApp":
                    setattr(cls, "_rc_render_ui", __patched_rc_render_ui)
            finally:
                return cls
        if _orig_bc is not None:
            _bi.__build_class__ = _hook
    except Exception:
        pass
# =====================
...
# =====================
# Toolbar/Menu Nuke Patch
# =====================
try:
    from PyQt5.QtWidgets import QToolBar
    def _nuke_menus_and_toolbars(self):
        try:
            ...
            pass
    # Patch ChatApp to nuke menus/toolbars on init
    _orig_init = getattr(ChatApp, "__init__", None)
    if _orig_init:
        def __init_nuke(self, *a, **k):
            _orig_init(self, *a, **k)
            try:
                _nuke_menus_and_toolbars(self)
            except Exception:
                pass
        ChatApp.__init__ = __init_nuke
except Exception:
    pass
