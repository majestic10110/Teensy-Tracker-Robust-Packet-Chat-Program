...
"""
Teensy Tracker Chat v1.1.6 BETA (2025-09-09)
- Serial + Frequencies boxes "hug contents" (no wasted vertical space) and are top-aligned.
- Beacon: send immediately when enabled, then every 15 minutes.
- Heartbeat: each ..CALLSIGN shown for 20 minutes since last heard; resets timer on re-hear; no duplicates.
- Receive pane auto-scrolls; device/console lines filtered; SEND box has permanent green border; maximized on open.
"""
...
class ChatApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Teensy Tracker Robust Packet Chat v1.1.7 BETA")
        ...
        # Hug contents: width expands, height fixed to sizeHint after widgets exist
        ...
        # No stretch inside, to avoid vertical growing
        ...
        self.beacon_check = QCheckBox("15 min Beacon")
        ...
        self.heartbeat_text.setFixedWidth(int(char_w * 17.5))  # ~16 characters
        ...
        # Permanent green border for Send Message box
        ...
        self.status_label = QtWidgets.QLabel("Disconnected  |  Reminder: WARNING Disconnect Radio Data cable when turning KISS on/off! Turn KISS OFF to chat. If Kiss is ON then toggle check ON/OFF")
    ...
    def _toggle_beaconing(self, on: bool):
        if on:
            ...
            # Transmit immediately on enable
            self._send_beacon()
    ...
    def _send_beacon(self):
        ...
        if ok:
            ...
            # Append Beacon sent (green) and auto-scroll
    ...
    def _on_line(self, line: str):
        ...
        if beacon_match:
            ...
            mycall = self.mycall_edit.text().strip().upper()
            if call == mycall:
                # Suppress own beacon from RX
                return
            # Update last-heard time and refresh view (dedup + TTL handled in view update)
        ...
        # Loose echo suppression fallback
        for item in list(self.recent_sent):
    ...
    def _heartbeat_update_view(self):
        """Prune expired heartbeats (TTL) and refresh the Heartbeat column."""
        now = time.time()
    ...
    def _is_device_line(self, s: str) -> bool:
        ...
        pats = [
            ...
            r'^\*\s*[%@].*',
            r'^\*.*',                   # any line starting with an asterisk
            r'^%[A-Za-z].*',
            r'^cmd:\s',
            r'^cmd>\s*',                # some firmwares use 'cmd>'
            r'^(MHEARD|HEARD)\b',
    ...
    def _append_chat_line(self, text: str, kind: str):
        """Append colored line and force auto-scroll to bottom."""
...
def main():
    print("Teensy Tracker Chat v1.1.7 BETA (2025-09-09)")
