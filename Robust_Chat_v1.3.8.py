...
"""
LiNK500 Teensy Chat v1.3.8 Beta (2025-09-12)
...
import re as _re_for_cs
_CALL_RE = _re_for_cs.compile(r"^([A-Z0-9/]+?)(?:-([0-9]{1,2}))?$", _re_for_cs.IGNORECASE)
...
class ChatApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Link500 Teensy Robust Chat v1.3.8 Beta")
        ...
        self._tx_gate = False
        self._tx_count = 0
        # --- ACK state (PTT-driven) ---
        self.ACK_LISTEN_MS = 6000           # exact 6s wait after each PTT OFF
        self.USE_PTT_TIMING = False  # v1.3.8: use word-count timing instead of PTT
        self.TX_MS_PER_WORD = 466      # ~0.30s per word (tweakable)
        self.TX_BASE_MS = 0          # base overhead per message
        self.PTT_TAIL_MS = 0         # guard after last 'word'
        self.PTT_OFF_FALLBACK_MS = 0    # if no PTT OFF seen within 12s, assume TX done and start wait
        self._ack_states = {}               # ack_id -> {"attempt":int, "timer":QTimer or None, "waiting":bool, "fb":QTimer or None}
        self._current_ack_id = None         # ack_id currently being transmitted
        # v1.3.8 PTT-driven ACK state machine
        self.ACK_LISTEN_MS = 6000         # fixed 6s wait after each TX
        self._ack_states = {}             # ack_id -> {"attempt":int, "timer":QTimer or None, "waiting":bool}
        self._current_ack_id = None       # ack_id currently being transmitted (for PTT OFF trigger)
        self._tx_active = False
        self._pending_ack_id_current_tx = None

        self.ack_counter = 1
        self.chat_items = []
        self.sent_by_ack = {}
        # v1.3.8 ACK timing config
        self.ACK_LISTEN_MS = 6000           # fixed 6s listen window after TX ends
        self.TX_CHAR_RATE_CPS = 12          # approx chars/sec on-air (tweak per mode)
        self.PTT_TAIL_MS = 0              # guard time after last byte before listen
        self.tx_end_timers = {}             # ack_id -> QTimer to begin ACK wait post-TX
        self._defer_initial_ack = {}        # ack_id -> True if initial schedule is deferred
    ...
    def open_port(self):
        try:
            ...
            try:
                self.reader_thread.ptt_changed.disconnect(self._on_ptt_changed)
            except Exception:
                pass
            try:
                self.reader_thread.ptt_changed.connect(self._on_ptt_changed)
            except Exception:
                pass
            try:
                self.reader_thread.ptt_changed.connect(self._on_ptt_changed)
    ...
    def close_port(self):
        try:
            ...
            for t in list(self.tx_end_timers.values()):
                try: t.stop()
                except Exception: pass
            self.tx_end_timers.clear()
            self._defer_initial_ack.clear()
...
# -------- APRS helpers --------
        ...
        if m:
            ...
            if ack_only:
                ...
                if item and my and to == my and frm == (item.get("to") or "").upper():
                    if base_callsign(to) != base_callsign(self.mycall_edit.text()):
                        return
            ...
            if tag and self.auto_ack_enabled and my:
                ...
                if allow_auto:
                    # Only ACK if the message is explicitly addressed to MYCALL
                    to_base = base_callsign(to)
                    my_base = base_callsign(my)
                    if not (my_base and to_base == my_base):
                        pass
                    else:
    ...
    def _add_chat_item(self, *, kind: str, text: str, to: str, frm: str, ack_id: str, ack: bool, style: str=None):
        ...
        self._rebuild_chat_view()
        if kind == "sent" and item["ack_id"]:
            # v1.3.8: defer initial ACK wait until TX finishes
            self._defer_initial_ack[item["ack_id"]] = True
    ...
    def _mark_ack_received(self, ack_id: str, from_callsign: str = ""):
        ...
        if item and not item.get("ack"):
            ...
            # stop any new PTT-driven ack timers and clear state
            self._clear_ack_timers(ack_id)
        try:
            st = self._ack_states.get(ack_id)
            if st and st.get('tx'):
                try:
                    st['tx'].stop()
                except Exception:
        ...
        try:
            ...
            try:
                t0 = self.tx_end_timers.pop(ack_id, None)
                if t0:
                    try:
                        t0.stop()
                    except Exception:
    ...
    # --- Word-count-based TX duration estimate ---
    def _tx_delay_ms_for(self, text: str) -> int:
        s = (text or "").strip()
        words = len([w for w in s.split() if w])
        ms = int(self.TX_BASE_MS + words * self.TX_MS_PER_WORD + self.PTT_TAIL_MS)
        # Clamp to sensible range
        if ms < 400: ms = 400
        if ms > 20000: ms = 20000
        return ms

    def _on_ptt_changed(self, is_on: bool):
        # If PTT timing disabled, ignore device PTT and rely on word-count timers
        if not getattr(self, "USE_PTT_TIMING", False):
            return
        # (The previous PTT-based logic can remain here if needed, but is gated off by the flag.)


    def _start_ack_window(self, ack_id: str):
        st = self._ack_states.get(ack_id)
        if not st:
            return
        # cancel any prior window timer
        self._clear_ack_timers(ack_id)
        timer = QTimer(self)
        timer.setSingleShot(True)
        timer.setInterval(int(self.ACK_LISTEN_MS))
        def _on_timeout(aid=ack_id):
            item = self.sent_by_ack.get(aid)
            if item and item.get("ack"):
                return  # already acked
            attempt = st.get("attempt", 1)
            max_attempts = item.get("max_attempts") if item else 3
            if attempt < max_attempts:
                # move to the next attempt
                st["attempt"] = attempt + 1
                self._sync_attempt_ui(aid)
                self.status_label.setText(f"No ACK in 6s. Retrying attempt {st['attempt']}…")
                self._retry_send(aid)
            else:
                # final failure after the 3rd 6s window
                self._final_fail(aid)
        timer.timeout.connect(_on_timeout)
        st["timer"] = timer
        self.status_label.setText(f"Attempt {st.get('attempt',1)}: waiting 6s for ACK…")
        timer.start()
        self._ack_states[ack_id] = st

    def _sync_attempt_ui(self, ack_id: str):
        """Copy attempt from _ack_states into the sent item and refresh the view."""
        st = self._ack_states.get(ack_id)
        if not st:
            return
        item = self.sent_by_ack.get(ack_id)
        if not item:
            return
        # Ensure max_attempts is present
        if not item.get("max_attempts"):
            item["max_attempts"] = 3
        # Update attempt (bounded 1..max)
        att = int(st.get("attempt", item.get("attempt", 1)))
        mmax = int(item.get("max_attempts", 3))
        if att < 1: att = 1
        if att > mmax: att = mmax
        item["attempt"] = att
        try:
            self._rebuild_chat_view()
        except Exception:
    ...
    def _clear_ack_timers(self, ack_id: str):
        st = self._ack_states.get(ack_id)
        if not st:
            return
        for key in ("timer","fb"):
            # also stop pending TX timer
            try:
                ttx = st.get('tx')
                st['tx'] = None
                if ttx:
    ...
    def _arm_ptt_fallback(self, ack_id: str):
        # If device doesn't print PTT OFF, fall back after PTT_OFF_FALLBACK_MS to start the 6s window
        st = self._ack_states.get(ack_id)
        if not st:
            return
        fb = QTimer(self)
        fb.setSingleShot(True)
        fb.setInterval(int(self.PTT_OFF_FALLBACK_MS))
        def _fallback(aid=ack_id):
            # If still waiting for PTT OFF, begin window anyway
            st2 = self._ack_states.get(aid)
            if st2 and st2.get("waiting", False):
                st2["waiting"] = False
                self._ack_states[aid] = st2
                self._start_ack_window(aid)
        fb.timeout.connect(_fallback)
        st["fb"] = fb
        fb.start()
        self._ack_states[ack_id] = st

    def _on_ptt_changed(self, is_on: bool):
        # When PTT drops (False), begin the 6s window for the current ack id (if any)
        if not is_on and self._current_ack_id:
            aid = self._current_ack_id
            self._current_ack_id = None
    ...
    def _start_ack_window(self, ack_id: str):
        st = self._ack_states.get(ack_id)
        if not st: return
        # ensure any prior timer cancelled
        self._clear_ack_timer(ack_id)
        timer = QTimer(self)
        timer.setSingleShot(True)
        timer.setInterval(int(self.ACK_LISTEN_MS))
        def _on_timeout(aid=ack_id):
            ...
            if item and item.get("ack"):
                return  # already acked
            # No ACK in the 6s window: retry or fail
            attempt = st.get("attempt", 1)
            max_attempts = item.get("max_attempts") if item else 3
            if attempt < max_attempts:
                st["attempt"] = attempt + 1
        ...
        timer.start()
        # Optional UI hint
        self.status_label.setText(f"Waiting {self.ACK_LISTEN_MS//1000}s for ACK {ack_id} (attempt {st.get('attempt',1)}).")

    def _clear_ack_timer(self, ack_id: str):
        st = self._ack_states.get(ack_id)
        if not st: return
        t = st.get("timer"); st["timer"] = None
        if t:
            try:
                t.stop()
            except Exception:
    ...
    def _on_ptt_changed(self, is_on: bool):
        self._tx_active = bool(is_on)
        if not is_on:
            # If a TX attempt just ended and we were deferring initial ACK wait, start it now
            aid = getattr(self, "_pending_ack_id_current_tx", None)
            if aid:
                # Clear pending and kick off 6s wait
                self._pending_ack_id_current_tx = None
                self._defer_initial_ack.pop(aid, None)
                self._schedule_retry(aid, initial=True)

    def _estimate_tx_ms(self, line_text: str) -> int:
        try:
            n = max(1, len(line_text or ""))
            est_ms = int((n / float(self.TX_CHAR_RATE_CPS)) * 1000)
            return est_ms + int(self.PTT_TAIL_MS)
        except Exception:
            return 1000 + int(self.PTT_TAIL_MS)

    def _begin_ack_wait(self, ack_id: str):
        if not ack_id:
            return
        # Clear defer flag and kick off first wait
        self._defer_initial_ack.pop(ack_id, None)
        self._schedule_retry(ack_id, initial=True)

    def _schedule_retry(self, ack_id: str, initial: bool = False):
        # v1.3.8: do not start initial wait until TX has ended
        if initial and self._defer_initial_ack.get(ack_id):
            return
        item = self.sent_by_ack.get(ack_id)
        ...
        a = item.get("attempt") or 1
        m = item.get("max_attempts") or 3
        delay_ms = int(self.ACK_LISTEN_MS)
    ...
    def _retry_send(self, ack_id: str):
        # Robust retry: ensure state exists and do not rely on PTT flags in word-count mode
        item = self.sent_by_ack.get(ack_id)
        if not item or item.get("ack"):
            return
        ...
        st = self._ack_states.get(ack_id)
        if not st:
            st = {"attempt": 2, "timer": None, "tx": None}
            self._ack_states[ack_id] = st

        attempt = int(st.get("attempt", 2))
        line = item.get("text", "")
        ...
        # Re-arm word-count TX timer; when it expires, begin the 6s ACK window
        try:
            if st.get("tx"):
                try:
                    st["tx"].stop()
            ...
            ttx = QTimer(self)
            ttx.setSingleShot(True)
            ttx.setInterval(self._tx_delay_ms_for(line))

            def _txdone(aid=ack_id):
                self._start_ack_window(aid)

            ttx.timeout.connect(_txdone)
            st["tx"] = ttx
            ttx.start()
        except Exception:
            pass
        ...
        # HUD + chat UI sync
        self.status_label.setText(f"Attempt {attempt}: transmitting…")
        try:
            self._sync_attempt_ui(ack_id)
        except Exception:
            pass
    ...
    def _final_fail(self, ack_id: str):
        ...
        if not item or item.get("ack"):
            return
        item["failed"] = True
        # Ensure attempt shown as max on failure
        try:
            st = self._ack_states.get(ack_id)
            if st:
                st["attempt"] = item.get("max_attempts", 3)
                self._sync_attempt_ui(ack_id)
        except Exception:
            pass

        # Stop and clear PTT-driven timers/state
        try:
            st = self._ack_states.pop(ack_id, None)
        ...
        # Stop and clear any legacy timers
        try:
            t = self.retry_timers.pop(ack_id, None)
            if t:
                try:
    ...
    def send_message(self):
        try:
            ...
            # v1.3.8: arm PTT-based start for ACK wait
            self._pending_ack_id_current_tx = ack_id
            tx_ok = self.send_user_text(line)
            if not tx_ok:
                self.status_label.setText("Send blocked or radio not connected (message shown locally).")

            # v1.3.8 — Start ACK wait AFTER TX finishes:
            try:
                ack_id  # ensure in scope
                line    # ensure in scope
            except NameError:
                pass
            else:
                if ack_id:
                    tx_ms = self._estimate_tx_ms(line)
                    t = QTimer(self)
                    t.setSingleShot(True)
                    t.setInterval(max(1, int(tx_ms)))
                    t.timeout.connect(lambda aid=ack_id: self._begin_ack_wait(aid))
                    self.tx_end_timers[ack_id] = t
                    t.start()
...
def main():
    print("Link500 Teensy Robust Chat v1.3.8 Beta (2025-09-12)")
