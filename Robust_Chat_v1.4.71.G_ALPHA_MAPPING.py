...
def diag_log(msg):
    try:
        ...
        with open('store/diagnostic.txt', 'a', encoding='utf-8') as _f:
            _f.write(f"[{_dt.datetime.utcnow().isoformat()}Z] {msg}\n")
    except Exception: pass
def _install_excepthook():
    def _hook(exc_type, exc, tb):
        try:
            _ensure_store_dir()
            with open('store/diagnostic.txt', 'a', encoding='utf-8') as _f:
                _f.write(f"[{_dt.datetime.utcnow().isoformat()}Z] UNCAUGHT {exc_type.__name__}: {exc}\n")
...
# ===== File Transfer (Upload → META/OK/PART/END with PING/PONG probes) =====
    ...
    def __init__(self):
        ...
        try:
            _install_offline_map_embedded(self)
        except Exception as e:
            pass
